...
try:
    launched = act('launch_app', {
        'path': str(Path(os.environ['SystemRoot'])/'System32/WindowsPowerShell/v1.0/powershell.exe'),
        'additional_arguments': ['-NoProfile','-STA','-WindowStyle','Hidden','-File',
            str(Path(__file__).with_name('computer-fixture.ps1').resolve()),
            '-ResultPath',str(root/'result.txt')],
    })
    ...
    act('set_value', {'pid':pid,'window_id':window,'element_token':text['element_token'],'value':'Marvi computer fixture'})
    state = targets(act('get_window_state', {'pid':pid,'window_id':window}))
    button = next(e for e in state['elements'] if e.get('label')=='Save fixture')
    act('click', {'pid':pid,'window_id':window,'element_token':button['element_token']})
    ...
    act('kill_app', {'pid':pid});pid=None
    print(json.dumps({'driver':'cua-driver 0.24.0','native_app_launch':True,
        'fresh_element_tokens':True,'background_button':True,'file_result_verified':True,
        'private_resume':True,'app_close':True,'timings_ms':timings}))
finally:
    if pid:
        service.control('resume')
