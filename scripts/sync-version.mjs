...
export function updateMarviLockPackages(source, version) {
  ...
  const updated = parts.map((block) => {
    if (
      !/^\[\[package\]\]/m.test(block) ||
      !/^name\s*=\s*"marvi-[^"]+"/m.test(block)
    ) {
      return block;
    }
    if (!/^version\s*=\s*"[^"]+"/m.test(block)) {
      throw new Error(
        `Marvi package lock entry has no version: ${block.slice(0, 100)}`,
      );
    }
    found += 1;
  ...
  if (found === 0)
    throw new Error("lockfile contains no Marvi package entries");
  return updated.join("");
}
...
export async function synchronize(root, version, check = false) {
  ...
  const jobs = [
    ...
    [
      ...
      (source) => updatePackageJson(source, version),
    ],
    ["package-lock.json", (source) => updatePackageLock(source, version)],
    [
      "apps/updater/Cargo.toml",
      (source) => updateUpdaterWorkspace(source, version),
    ],
    [
      "apps/updater/Cargo.lock",
      (source) => updateMarviLockPackages(source, version),
    ],
    [
      "apps/updater/src-tauri/tauri.conf.json",
      (source) => updateTauriConfig(source, version),
    ],
  ];

  for (const relative of await existingChildren(
    root,
    "services",
    "pyproject.toml",
  )) {
    jobs.push([
      relative,
      (source) => updateNamedTomlSection(source, "project", version),
    ]);
  }
  ...
  for (const relative of await existingChildren(root, "apps", "Cargo.toml")) {
    ...
    jobs.push([
      relative,
      (source) => updateNamedTomlSection(source, "package", version),
    ]);
  }
  ...
  for (const [relative, transform] of jobs) {
    ...
    try {
      ...
      if (
        error.code === "ENOENT" &&
        /(?:services|apps)[/\\][^/\\]+[/\\](?:uv\.lock|Cargo\.lock)$/.test(
          relative,
        )
      ) {
        continue;
...
async function main() {
  ...
  if (check && changed.length) {
    console.error(
      `Version ${version} is not synchronized in:\n${changed.map((file) => `  ${file}`).join("\n")}`,
    );
    process.exitCode = 1;
    return;
  }
  console.log(
    check
      ? `All product manifests match ${version}.`
      : `Synchronized ${version} in ${changed.length} files.`,
  );
