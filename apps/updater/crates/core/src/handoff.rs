...
///
/// It runs `python -m marvi_gateway.cli`, not the `marvi` console script.
/// Those are the same code, but the console script is `.venv/Scripts/marvi.exe`
/// — and `marvi setup` syncs Python projects in that same venv, which means uv
/// tries to rewrite the executable currently running the command:
///
/// ```text
/// failed to remove file `.venv/Lib/site-packages/../../Scripts/marvi.exe`:
/// The process cannot access the file because it is being used by another
/// process. (os error 32)
/// ```
///
/// Invoking the module leaves no `marvi.exe` running, so the sync has nothing
/// to fight over.
///
...
fn write_shim(
    ...
    let body = format!(
        "@echo off\r\n\
         ...
         \"{uv}\" run --project \"{root}\\services\\gateway\" python -m marvi_gateway.cli %*\r\n",
        uv = plain(&uv),
...
fn write_shim(
    ...
    std::fs::write(
        &shim,
        format!(
            "#!/bin/sh\n# Generated by the Marvi installer.\nexec \"{uv}\" run --project \"{root}/services/gateway\" python -m marvi_gateway.cli \"$@\"\n",
            uv = uv.display(),
