...
export function passwordExport(text: string): Login[] {
  ...
  return rows.map((row) => {
    ...
    try {
      ...
      throw new Error('Password export contains an invalid website')
    }
    if (
      ...
      throw new Error('Expected Chrome password CSV: url, username, password')
...
export class BrowserPasswords {
  ...
  private read(): Login[] {
    if (!this.encryption.isEncryptionAvailable())
      throw new Error('Windows password protection is unavailable')
  ...
  forOrigin(origin: string): Login[] {
    return this.read().filter((item) => item.origin === origin)
  }
}
...
export function fillLogin(login: Login): void {
  ...
  if (password.form?.action && new URL(password.form.action).origin !== login.origin)
    throw new Error('Login form has a different destination')
