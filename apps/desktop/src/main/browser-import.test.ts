...
describe('Chrome password export import', () => {
  ...
  it.each(['file:///secret', 'javascript:alert(1)', 'not a url'])(
    'refuses non-website credential destinations: %s',
    (url) => {
      expect(() =>
        passwordExport(`name,url,username,password\nExample,${url},user,password`)
      ).toThrow()
    }
  )
  it('rejects malformed or missing password data', () => {
    ...
    expect(() =>
      passwordExport('name,url,username,password\nExample,"https://example.com,user,password')
    ).toThrow()
  })
})
