...
const status: ComputerStatus = { enabled: true, installed: true, state: 'running', active: true, action: 'type_text', driver: 'cua-driver', version: '0.24.0' }
describe('computer-use Island', () => {
  ...
  it('distinguishes stopping from acknowledged private input', () => {
    expect(renderToStaticMarkup(<ComputerIsland status={{ ...status, state: 'stopping' }} />)).toContain('Marvi is stopping computer use')
    const html = renderToStaticMarkup(<ComputerIsland status={{ ...status, state: 'private', active: false }} />)
