    ...
    switch (page) {
      case 'Overview':
        return [
          ...
          { icon: Settings, label: 'Open settings', onSelect: () => setSettings('Preferences'), separatorBefore: true }
        ]
      case 'Voice':
        return [
          {
            icon: voice.phase === 'ready' || voice.phase === 'error' ? Play : Pause,
            label: voice.phase === 'ready' || voice.phase === 'error' ? 'Start voice session' : 'Stop voice session',
            onSelect: () => void (voice.phase === 'ready' || voice.phase === 'error' ? startVoice() : stopVoice())
          },
          { icon: Mic, label: 'Speech recognition', onSelect: () => setSettings('Speech recognition') },
          { icon: Waves, label: 'Wake word', onSelect: () => setSettings('Wake word') },
          { icon: Volume2, label: 'Voice synthesis', onSelect: () => setSettings('Voice synthesis') }
        ]
      case 'Chat':
        return [
          { icon: Box, label: 'Choose models', onSelect: () => setSettings('Models') },
          { icon: Gauge, label: 'View usage', onSelect: () => setSettings('Usage') },
          { icon: House, label: 'Return to Overview', onSelect: () => navigate('Overview'), separatorBefore: true }
        ]
      case 'Vision':
      case 'Room':
      case 'Activity':
        return [
          ...
          { icon: Activity, label: 'Open Activity', onSelect: () => navigate('Activity') },
          { icon: Settings, label: 'Capability settings', onSelect: () => setSettings('Plugins'), separatorBefore: true }
        ]
      case 'Identity':
      case 'Graph':
      case 'Mind':
        return [
          ...
          { icon: Settings, label: 'Memory settings', onSelect: () => setSettings('Memory'), separatorBefore: true }
        ]
      default:
        return [
  ...
  const contextActions = (surface: ShellMenuSurface): ShellMenuAction[] => {
    if (surface === 'sidebar') {
      return [
        ...
        { icon: House, label: 'Overview', onSelect: () => navigate('Overview'), separatorBefore: true },
