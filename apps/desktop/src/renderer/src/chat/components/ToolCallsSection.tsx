...
function words(value: string): string {
  return value.replaceAll(/[_-]+/g, ' ').trim().replace(/\b\w/g, (letter) => letter.toUpperCase())
}
...
function ToolIcon({ category }: { category: string }): React.JSX.Element {
  ...
  return <span className="chat-tool-icon" data-category={category}><Icon aria-hidden="true" size={15} strokeWidth={1.6} /></span>
}

export function ToolCallsSection({ messages }: { messages: ChatMessage[] }): React.JSX.Element | null {
  const [open, setOpen] = useState(false)
  const [expanded, setExpanded] = useState<Set<number>>(new Set())
  const tools = useMemo(() => messages.map(toolView), [messages])
  const categories = useMemo(
    () => tools.filter((tool, index) => tools.findIndex((item) => item.category === tool.category) === index),
    [tools]
  )
  ...
  return (
    <section className="chat-tool-section" aria-label={`${tools.length} tool calls`}>
      <button aria-expanded={open} className="chat-tool-section-head" onClick={() => setOpen((value) => !value)} type="button">
        <span className="chat-tool-stack" aria-hidden="true">
          {categories.slice(0, 10).map((tool, index) => (
            <span className="chat-tool-stack-item" key={tool.category} style={{ transform: `rotate(${categories.length > 1 ? (index % 2 ? -8 : 8) : 0}deg)`, zIndex: index + 1 }}>
              <ToolIcon category={tool.category} />
            </span>
          ))}
          {categories.length > 10 ? <span className="chat-tool-overflow">+{categories.length - 10}</span> : null}
        </span>
        <strong>Used {tools.length} tool{tools.length === 1 ? '' : 's'}</strong>
        <ChevronDown aria-hidden="true" className={open ? 'is-open' : ''} size={15} strokeWidth={1.6} />
      </button>
          ...
          {tools.map((tool, index) => {
            ...
            return (
              <div className="chat-tool-step" key={tool.message.id}>
                <div className="chat-tool-rail" aria-hidden="true"><ToolIcon category={tool.category} />{index < tools.length - 1 ? <i /> : null}</div>
                <div className="chat-tool-step-content">
                  <button aria-expanded={hasDetails ? callOpen : undefined} className="chat-tool-step-head" disabled={!hasDetails} onClick={() => hasDetails && toggle(tool.message.id)} type="button">
                    <strong>{words(tool.name)}</strong>
                    {hasDetails ? <ChevronDown aria-hidden="true" className={callOpen ? 'is-open' : ''} size={12} strokeWidth={1.6} /> : null}
                  </button>
                  <span className="chat-tool-category">{tool.categoryLabel}</span>
                  {callOpen && hasDetails ? (
                    <div className="chat-tool-detail">
                      {hasArguments ? <div><span>Input</span><pre>{JSON.stringify(tool.arguments, null, 2)}</pre></div> : null}
                      {hasOutput ? <div><span>Output</span><div className="chat-tool-body"><Markdown content={tool.message.content} /></div></div> : null}
                    </div>
                  ) : null}
                </div>
              </div>
