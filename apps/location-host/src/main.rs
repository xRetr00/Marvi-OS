...
fn locate(request: bool) -> windows::core::Result<Value> {
    ...
    let position = locator.GetGeopositionAsyncWithAgeAndTimeout(
        TimeSpan { Duration: 60 * 10_000_000 },
        TimeSpan { Duration: 20 * 10_000_000 },
    )?.join()?;
    ...
    Ok(json!({
        ...
        "source": format!("windows:{}", coord.PositionSource()?.0),
    }))
}
...
fn main() {
    ...
    let result = locate(request).unwrap_or_else(|error| {
        let status = if error.code().0 as u32 == 0x80070005 { "denied" } else { "unavailable" };
        json!({"status": status})
    });
    println!("{result}");
}
