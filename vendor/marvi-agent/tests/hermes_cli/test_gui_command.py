...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("hermes_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("hermes_cli.main._write_desktop_build_stamp"), \
             ...
             patch(
                 "hermes_cli.main._ensure_desktop_exe_launchable",
                 return_value=(packaged_exe, False),
             ), \
    ...
    resolved_npm = Path(mock_install.call_args.args[0])
    assert resolved_npm.name.lower() in {"npm", "npm.cmd", "npm.exe"}
    assert mock_install.call_args.args[1] == root
    ...
    assert mock_run.call_args_list[0].args[0] == [str(resolved_npm), "run", "pack"]
