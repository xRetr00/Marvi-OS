...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`marvi ${args.join(' ')}\` (legacy \`hermes ${args.join(' ')}\`)…`)
  ...
  if (result.error) {
    transcript.sys(`error launching marvi: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`marvi ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
