...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so Marvi Desktop cannot save remote gateway tokens. ' +
        'Either enable an OS keyring (for example GNOME Keyring or KWallet), confirm plain-text storage when prompted in Settings → Gateway, ' +
