...
const posixModeTest = process.platform === 'win32' ? test.skip : test
...
posixModeTest('writeSecretFileAtomic creates the file owner-only, not at the 0644 umask default', () => {
  withTempDir(dir => {
...
posixModeTest('writeSecretFileAtomic does not inherit loose bits from a stale temp file', () => {
...
posixModeTest('the written file is owner-only even where chmod does nothing', () => {
...
posixModeTest('the written file is owner-only even when a stale temp cannot be removed', () => {
...
posixModeTest('tightenSecretFileMode tightens a pre-existing world-readable config in place', () => {
...
posixModeTest('tightenSecretFileMode leaves a non-safeStorage token payload readable', () => {
...
posixModeTest('tightenSecretFileMode is idempotent and never throws on an unusable path', () => {
  withTempDir(dir => {
    const target = path.join(dir, 'connection.json')
...
posixModeTest('tightenSecretFileMode refuses to chmod a symlink instead of following it to its target', () => {
...
posixModeTest('tightenSecretFileMode only touches a regular file the current user owns', () => {
