...
fn main() {
    ...
    if let Some(c) = &commit {
        ...
        println!(
            "cargo:warning=marvi-bootstrap: pinning to commit {}",
            short(c)
        );
    }
    if let Some(b) = &branch {
        ...
        match &commit {
            Some(_) => println!("cargo:warning=marvi-bootstrap: pinning to branch {b}"),
            None => println!(
                "cargo:warning=marvi-bootstrap: following branch {b} HEAD (no commit pin; \
    ...
    if commit.is_none() && branch.is_none() {
        ...
        // build environment.
        println!(
            "cargo:warning=marvi-bootstrap: no pin resolved at build time; binary will fail at runtime without HERMES_SETUP_DEV_REPO_ROOT or runtime args"
        );
    }
