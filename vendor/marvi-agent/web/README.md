...
Browser-based dashboard for managing Marvi Agent configuration, API keys, and monitoring active sessions.
...
`marvi dashboard` on port 9119 serves the **built** bundle from `hermes_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (legacy `hermes dashboard` still works for existing installs) or use `web --no-open` + Vite as above.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Marvi Agent” — mixed case, no Mondwest, no `text-display` |
