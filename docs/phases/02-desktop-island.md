...
September 18 Chat gained six Gateway-validated commerce and travel cards
through its existing assistant-ui tool UI path. Each card requires matching
facts from a successful tool result in the same turn, with bounded fields and
optional public source links. Checkout and reservation actions remain owned by
the underlying Gateway tools.
