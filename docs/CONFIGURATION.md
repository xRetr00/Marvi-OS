...
Four stores have no variable because nothing has needed to move them: the
jobs board (`jobs.sqlite3`), the automation rules (`automations.sqlite3`),
meeting notes (`meetings.sqlite3`, with the recordings themselves under
`meetings\<id>\`), and the workspace snapshot store (`checkpoints\store`, one
bare git repository per workspace root). All sit beside the files above, under
`MARVI_HOME`.
...
| `MARVI_WEB_FETCHER` | `builtin` | page reader for `web_extract`: `builtin`, `trafilatura` (local extraction after Marvi's bounded fetch), or `jina` (hosted Jina Reader). Editable in Settings > Tools |
| `JINA_API_KEY` | unset | optional API key sent to hosted Jina Reader when `MARVI_WEB_FETCHER=jina` |
| `MARVI_LOCAL_ONLY` | off | every model call must go to a local provider (Ollama, LM Studio, llama.cpp); cloud providers are refused. Selecting a local provider in Models enables this automatically, so voice and auxiliary work keep working when the PC loses internet. Model calls only |
