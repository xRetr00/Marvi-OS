...
def instead_say(text: str, used: list[str] | None = None) -> str:
    """What to show in place of markup that could not be run.

    Names the tool, because "I tried to call terminal_run" is something the
    user can act on and "something went wrong" is not.

    And never claims that *nothing* was done. The first version said "Nothing
    was done" unconditionally, and it fired at the end of a turn that had just
    moved the cursor, read it back and clicked successfully -- five real
    actions, reported as none. Only the one call written out as text did not
    run; everything before it did.
    """
    tool = reached_for(text)
    what = f"call {tool}" if tool else "use a tool"
    did = _done_so_far(used)
    if did:
        return (
            f"I stopped before I could {what} — {did} Say continue and I will carry on "
            "from there."
        )
    return (
        f"I tried to {what} and it did not run, so that step was not done. Ask me "
        "again and I will retry it."
    )


def out_of_steps(text: str, used: list[str]) -> str:
    """The turn ran out of tool rounds while there was still work to do.

    Different from a malformed call, and it has to read differently: the model
    did nothing wrong. It was mid-task, the last round is tool-free by design,
    and it wrote the next call out because it had nothing to call it with.
    """
    tool = reached_for(text)
    nxt = f", and the next step was {tool}" if tool else ""
    did = _done_so_far(used)
    # A sentence of its own here, so it starts with a capital.
    did = f" {did[:1].upper()}{did[1:]}" if did else ""
    return (
        f"I ran out of steps for this turn before finishing{nxt}.{did} "
        "Say continue and I will pick up where I stopped."
    )


def _done_so_far(used: list[str] | None) -> str:
    """What actually ran this turn, in words. Empty when nothing did."""
    ran = [name for name in (used or []) if name]
    if not ran:
        return ""
    counts: dict[str, int] = {}
    for name in ran:
        counts[name] = counts.get(name, 0) + 1
    listed = ", ".join(f"{name} ({n} times)" if n > 1 else name for name, n in counts.items())
    return f"so far I ran {listed}."
