"""Centralized logging setup for Marvi Messaging.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.marvi/logs/`` (profile-aware via ``get_marvi_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``runtime_support.web_server``,
    ``runtime_support.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Marvi (TUI, gateway, ``hy_memory`` server, MCP
...
from marvi_constants import get_config_path, get_marvi_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_marvi_session_injector", False):
    ...
    _session_record_factory._marvi_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``marvi logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``marvi logs --component gateway``.
    "gateway": ("gateway", "marvi_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("runtime_support", "cli"),
    "cron": ("cron",),
    "learning": ("agent.learning",),
    "memory": ("agent.memory", "tools.memory_tool", "plugins.memory"),
    "gui": (
        "runtime_support.web_server",
        "runtime_support.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Marvi logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    marvi_home
        Override for the Marvi home directory.  Falls back to
        ``get_marvi_home()`` (profile-aware).
    log_level
    ...
    home = marvi_home or get_marvi_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_marvi_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._marvi_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the marvi group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from runtime_support.config import is_managed
...
# the module header): when several Marvi processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._marvi_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="marvi-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_marvi_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse runtime_support.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when runtime_support.config isn't importable (bare
        # marvi_logging consumers).
        try:
            from runtime_support.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
