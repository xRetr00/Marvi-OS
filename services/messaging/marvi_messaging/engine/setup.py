...
Marvi Messaging (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``MARVI_MESSAGING_NIX_BUILD=1`` on the
Marvi package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("MARVI_MESSAGING_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for marvi-messaging is not supported.\n"
    "Marvi is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://github.com/xRetr00/Marvi-OS/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Marvi Nix derivation sets MARVI_MESSAGING_NIX_BUILD=1. If it does, file a bug."
)
