"""CLI entry point for the marvi-messaging ACP adapter.

Loads environment variables from ``~/.marvi/.env``, configures logging
...
try:
    ...
    # shadowing Marvi's own modules — ``marvi acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from MARVI_MESSAGING_HOME (default ``~/.marvi``)."""
    from runtime_support.env_loader import load_marvi_dotenv

    marvi_home = get_marvi_home()
    loaded = load_marvi_dotenv(marvi_home=marvi_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", marvi_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="marvi-acp",
        description="Run Marvi Messaging as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Marvi version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Marvi provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.marvi/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import MarviACPAgent  # noqa: F401

    print("Marvi ACP check OK")


def _run_setup() -> None:
    from runtime_support.main import main as marvi_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "marvi", "model"]
        marvi_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting marvi-messaging ACP adapter")
    ...
    import acp
    from .server import MarviACPAgent
    ...
    if os.environ.get("MARVI_MESSAGING_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
