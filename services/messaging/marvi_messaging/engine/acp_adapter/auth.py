"""ACP auth helpers — detect and advertise Marvi authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "marvi-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Marvi runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Marvi can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Marvi.
    ...
    not have Marvi provider credentials configured yet, so Marvi always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Marvi using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Marvi provider",
            description=(
                "Open Marvi' interactive model/provider setup in a terminal. "
                "Use this when Marvi has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
