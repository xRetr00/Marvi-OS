...
- Give it a name like "marvi-messaging"
- Select scopes:
...
- Give it a title like "marvi-messaging-<machine-name>"
...
uv run python "${MARVI_MESSAGING_HOME:-$HOME/.marvi}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _marvi_env="${MARVI_MESSAGING_HOME:-$HOME/.marvi}/.env"; [ -f "$_marvi_env" ] && grep -q "^GITHUB_TOKEN=" "$_marvi_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_marvi_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${MARVI_MESSAGING_HOME:-$HOME/.marvi}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
