#!/usr/bin/env bash
# GitHub environment detection helper for Marvi Messaging skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _marvi_env="${MARVI_MESSAGING_HOME:-$HOME/.marvi}/.env"; [ -f "$_marvi_env" ] && grep -q "^GITHUB_TOKEN=" "$_marvi_env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_marvi_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    if [ -n "$GITHUB_TOKEN" ]; then
        GH_AUTH_METHOD="curl"
    fi
elif [ -f "$HOME/.git-credentials" ]; then
    GITHUB_TOKEN=$(uv run python3 "${MARVI_MESSAGING_HOME:-$HOME/.marvi}/skills/github/github-auth/scripts/git-credential-token.py")
    if [ -n "$GITHUB_TOKEN" ]; then
        GH_AUTH_METHOD="curl"
    fi
fi
