...
metadata:
  marvi:
    ...
    related_skills: [claude-code, marvi-messaging]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Marvi terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Marvi itself, `model.provider: openai-codex` uses Marvi-managed Codex
OAuth from `~/.marvi/auth.json` after `marvi auth add openai-codex`. For the
...
When invoking the Codex CLI from a Marvi gateway/service context (for example,
