...
Author widget apps for the Marvi TUI (`marvi --tui`): glanceable ambient
...
- The TUI must be in use (`marvi --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.marvi/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.marvi/logs/tui_gateway_crash.log` if a widget never appears.
