...
Set via `marvi model` (picker) or `marvi setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://github.com/xRetr00/Marvi-OS/docs/integrations/providers
...
| nous | OAuth device code | `marvi auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `marvi auth add openai-codex` |
| qwen-oauth | OAuth | `marvi auth add qwen-oauth` |
| minimax-oauth | OAuth | `marvi auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`marvi auth`).
Fallback chain when the primary fails: `marvi fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`runtime_support/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   marvi config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
