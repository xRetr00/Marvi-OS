...
## Layer 1 — Is this thing a Marvi plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Marvi process**. Its LLM calls go through Marvi's already-configured provider. | Already uses Portal if user's Marvi is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Marvi. That LLM call is already authenticated
via Marvi's provider config. The plugin's own server auth (e.g.
...
What Marvi Messaging has that *feels* like OAuth — `marvi login --provider nous`
opening a browser, user signs in, token lands in `~/.marvi/auth.json` — is a
**Marvi-specific browser flow**. Under the hood it produces a credential
Marvi uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Marvi's existing Portal credential out of `~/.marvi/auth.json`.
...
rides on the credential Marvi already holds.

Where this could live in Marvi:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Marvi itself.
...
1. First decide: Marvi plugin (runs inside Marvi) or separate app? If plugin,
   it already uses Portal via Marvi's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Marvi Messaging itself") is fully supported. Marvi treats agent
...
pitch in the Tool Gateway announcement is specifically about Marvi Messaging's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Marvi is configured to use Portal as its provider. It is
