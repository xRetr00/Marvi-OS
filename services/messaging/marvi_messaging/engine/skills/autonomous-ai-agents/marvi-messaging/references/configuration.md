...
Full reference: https://github.com/xRetr00/Marvi-OS/docs/user-guide/configuration
...
`marvi config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `marvi tools` (interactive) or `marvi tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_MARVI_MESSAGING_CORE_TOOLS` is the default bundle most platforms inherit).
