...
Write plugins for the Marvi desktop app: statusbar items, layout panes,
...
system (`~/.marvi/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$MARVI_MESSAGING_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$MARVI_MESSAGING_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Marvi desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$MARVI_MESSAGING_HOME/desktop-plugins/` (usually
  `~/.marvi/desktop-plugins/`).

## How to Run

1. Create `$MARVI_MESSAGING_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.marvi/...` by default, or `~/.marvi/profiles/<profile>/...` under a
...
The ONLY import surface is `@marvi/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from Marvi (use `host.notify` for the
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.marvi/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@marvi/plugin-sdk`, `react`, and
