# Themes / Skins — Author a Marvi Color Theme

Author a Marvi **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`runtime_support/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.marvi/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Marvi itself.
...
- Write access to the Marvi home dir — `~/.marvi` by default, or `$MARVI_MESSAGING_HOME`
  / the active profile's dir. Skins live in `<marvi-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `marvi config set`).
...
3. `write_file` it to `<marvi-home>/skins/<name>.yaml`.
...
`runtime_support/skin_engine.py`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `marvi config set display.skin default` (or they can `/skin default`).
...
```
marvi skin set <key> <hex>      # e.g. marvi skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `marvi skin set background "#08201f"` changes
only the background; `marvi skin use <name>` / `marvi skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.marvi`** when a profile is active — resolve the real home
  from `$MARVI_MESSAGING_HOME` first, falling back to `~/.marvi`.
...
- **Never hand-edit `config.yaml` to activate.** Use `marvi config set
...
- **You apply it, not the user.** `marvi config set display.skin <name>` is
...
- `read_file` the written `<marvi-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `marvi config get display.skin` and confirm it reports `<name>`.
