---
name: marvi-messaging
description: "Configure, extend, or contribute to Marvi Messaging."
version: 2.3.0
author: Marvi Messaging + Teknium
license: MIT
platforms: [linux, macos, windows]
metadata:
  marvi:
    tags: [marvi, marvi, setup, configuration, multi-agent, spawning, cli, gateway, development]
...
Marvi Messaging is an open-source AI agent framework by NeuRetro Labs Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Marvi works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

This skill keeps the legacy `marvi-messaging` ID so older prompts, profiles, and commands can still load it. Treat `marvi`, `MARVI_MESSAGING_*`, and `~/.marvi` as compatibility names for Marvi unless the user is explicitly asking about old branding.
...
**This skill helps you work with Marvi Messaging effectively** — setting it up, configuring features, spawning additional agent instances, troubleshooting issues, finding the right commands and settings, and understanding how the system works when you need to extend or contribute to it.
...
pip install marvi-messaging       # or: uv pip install marvi-messaging

# Interactive chat (default surface; set display.interface: tui to launch the Ink TUI instead)
marvi

# Single query
marvi chat -q "What is the capital of France?"
...
# Other surfaces
marvi desktop                 # launch the native desktop app (alias: marvi gui)
marvi dashboard               # web admin panel + embedded chat
marvi proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```
...
```
marvi setup [section]      Interactive wizard (model|terminal|gateway|tools|agent)
marvi model                Interactive model/provider picker
marvi config               View current config
marvi config edit          Open config.yaml in $EDITOR
marvi config set KEY VAL   Set a config value
marvi config path          Print config.yaml path
marvi config env-path      Print .env path
marvi config check         Check for missing/outdated config
marvi config migrate       Update config with new options
marvi doctor [--fix]       Check dependencies and config
marvi status [--all]       Show component status
```

Credentials (OAuth + API keys, with pooling) are managed under `marvi auth` — see the Credentials & Pools section below.

### Tools & Skills

```
marvi tools                Interactive tool enable/disable (curses UI)
marvi tools list           Show all tools and status
marvi tools enable NAME    Enable a toolset
marvi tools disable NAME   Disable a toolset

marvi skills list          List installed skills
marvi skills search QUERY  Search the skills hub
marvi skills install ID    Install a skill (ID can be a hub identifier OR a direct https://…/SKILL.md URL; pass --name to override when frontmatter has no name)
marvi skills inspect ID    Preview without installing
marvi skills config        Enable/disable skills per platform
marvi skills check         Check for updates
marvi skills update        Update outdated skills
marvi skills uninstall N   Remove a hub skill
marvi skills publish PATH  Publish to registry
marvi skills browse        Browse all available skills
marvi skills tap add REPO  Add a GitHub repo as skill source
```
...
marvi mcp add NAME         Add an MCP server (--url or --command)
marvi mcp remove NAME      Remove an MCP server
marvi mcp list             List configured servers
marvi mcp test NAME        Test connection
...
(`marvi mcp install <name>`): `skill_view(name="marvi-messaging", file_path="references/native-mcp.md")`.

### Gateway (Messaging Platforms)

```
marvi gateway run          Start gateway foreground
marvi gateway install      Install as background service
marvi gateway start/stop   Control the service
marvi gateway restart      Restart the service
marvi gateway status       Check status
marvi gateway setup        Configure platforms
```

Supported platforms (20+): Telegram, Discord, Slack, WhatsApp (Baileys bridge + official Business Cloud API), iMessage (Photon — `marvi photon setup`, the BlueBubbles successor with no Mac relay), Signal, Email, SMS, Matrix, Mattermost, Microsoft Teams, LINE, SimpleX, ntfy, Google Chat, Home Assistant, DingTalk, Feishu, WeCom, Weixin (WeChat), Raft (agent network), API Server, Webhooks. Open WebUI connects via the API Server adapter. Most adapters ship under `plugins/platforms/`, so new ones drop in without touching core.
...
```
marvi sessions list        List recent sessions
marvi sessions browse      Interactive picker
marvi sessions export OUT  Export to JSONL
marvi sessions rename ID T Rename a session
marvi sessions delete ID   Delete a session
marvi sessions prune       Clean up old sessions (--older-than N days)
marvi sessions stats       Session store statistics
```

### Cron Jobs

```
marvi cron list            List jobs (--all for disabled)
marvi cron create SCHED    Create: '30m', 'every 2h', '0 9 * * *'
marvi cron edit ID         Edit schedule, prompt, delivery
marvi cron pause/resume ID Control job state
marvi cron run ID          Trigger on next tick
marvi cron remove ID       Delete a job
marvi cron status          Scheduler status
```

### Webhooks

```
marvi webhook subscribe N  Create route at /webhooks/<name>
marvi webhook list         List subscriptions
marvi webhook remove NAME  Remove a subscription
marvi webhook test NAME    Send a test POST
```

Full setup, route config, payload templating, and event-driven agent-run
patterns: `skill_view(name="marvi-messaging", file_path="references/webhooks.md")`.

### Profiles

```
marvi profile list         List all profiles
marvi profile create NAME  Create (--clone, --clone-all, --clone-from)
marvi profile use NAME     Set sticky default
marvi profile delete NAME  Delete a profile
marvi profile show NAME    Show details
marvi profile alias NAME   Manage wrapper scripts
marvi profile rename A B   Rename a profile
marvi profile export NAME  Export to tar.gz
marvi profile import FILE  Import from archive
```
...
marvi auth add [PROVIDER]  Add OAuth or API-key credential
                            (e.g. nous, openai-codex, qwen-oauth, anthropic)
marvi auth list [PROVIDER] List pooled credentials
marvi auth remove P INDEX  Remove by provider + index
marvi auth reset PROVIDER  Clear exhaustion status
```
...
```
marvi insights [--days N]  Usage analytics
marvi update               Update to latest version
marvi desktop / gui        Launch the native desktop app
marvi dashboard            Web admin panel + embedded chat
marvi proxy                OpenAI-compatible local proxy backed by an OAuth provider
marvi portal               Quick setup / sign in via Nous Portal
marvi kanban <verb>        Multi-agent work-queue board (init/create/list/show/assign/…)
marvi pairing list/approve/revoke  DM authorization
marvi plugins list/install/remove  Plugin management
marvi secrets bitwarden …  External secret store (Bitwarden Secrets Manager)
marvi memory setup/status/off  Memory provider config
marvi send                 Send a one-off message through a gateway platform
marvi completion bash|zsh  Shell completions
marvi acp                  ACP server (IDE integration)
marvi claw migrate         Migrate from OpenClaw
marvi uninstall            Uninstall Marvi
```

For the full, authoritative command list run `marvi --help` (and `marvi <command> --help`). Plugin- and provider-supplied subcommands (e.g. `marvi photon setup` for iMessage) only appear once their plugin is installed/active.
...
The registry of record is `runtime_support/commands.py` — every consumer
...
/reload-skills       Re-scan ~/.marvi/skills/ for added/removed skills
...
```
~/.marvi/config.yaml       Main configuration
~/.marvi/.env              API keys and secrets (under $MARVI_MESSAGING_HOME if set)
$MARVI_MESSAGING_HOME/skills/        Installed skills
~/.marvi/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.marvi/state.db          Canonical session store (SQLite + FTS5)
~/.marvi/logs/             Gateway and error logs
~/.marvi/auth.json         OAuth tokens and credential pools
~/.marvi/marvi-messaging/     Source code (if git-installed)
```

Profiles use `~/.marvi/profiles/<name>/` with the same layout.

### Config Sections

Edit with `marvi config edit` or `marvi config set section.key value`.
...
20+ providers supported. Set via `marvi model` or `marvi setup`.

| Provider | Auth | Key env var |
|----------|------|-------------|
| OpenRouter | API key | `OPENROUTER_API_KEY` |
| Anthropic | API key | `ANTHROPIC_API_KEY` |
| Nous Portal | OAuth | `marvi auth` |
| OpenAI Codex | OAuth | `marvi auth` |
...
| Qwen OAuth | OAuth | `marvi auth add qwen-oauth` |
...
Enable/disable via `marvi tools` (interactive) or `marvi tools enable/disable NAME`.
...
Full enumeration lives in `toolsets.py` as the `TOOLSETS` dict; `_MARVI_MESSAGING_CORE_TOOLS` is the default bundle most platforms inherit from.
...
Common "why is Marvi doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `marvi` invocation) because they're read once at startup.
...
```bash
marvi config set security.redact_secrets true       # keep enabled globally
```

**Restart required.** `security.redact_secrets` is snapshotted at import time — toggling it mid-session (e.g. via `export MARVI_MESSAGING_REDACT_SECRETS=false` from a tool call) will NOT take effect for the running process. Tell the user to change it in config from a terminal, then start a new session. This is deliberate — it prevents an LLM from flipping the toggle on itself mid-task.

Disable only when you deliberately need raw credential-like strings for debugging or redactor development:
```bash
marvi config set security.redact_secrets false
```
...
```bash
marvi config set privacy.redact_pii true    # enable
marvi config set privacy.redact_pii false   # disable (default)
```
...
```bash
marvi config set approvals.mode smart       # recommended middle ground
marvi config set approvals.mode off         # bypass everything (not recommended)
```

Per-invocation bypass without changing config:
- `marvi --yolo …`
- `export MARVI_MESSAGING_YOLO_MODE=1`

Note: YOLO / `approvals.mode: off` does NOT turn off secret redaction. They are independent.

### Shell hooks allowlist

Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.marvi/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.

### Disabling the web/browser/image-gen tools

To keep the model away from network or media tools entirely, open `marvi tools` and toggle per-platform. Takes effect on next session (`/reset`). See the Tools & Skills section above.
...
Marvi keeps downstream features that may not exist in upstream Marvi:
...
Do not restore upstream's standalone "Hey Marvi" detector, bundled
`hey_marvi` models, top-level `wake_word` config, `/wake` command, `wake.*`
...
Python `--build-only` resolver must prefer those names. Legacy Marvi names are
...
Keep the `runtime_support/web_server.py` Mind contract: `/api/mind`,
...
| | `delegate_task` | Spawning a `marvi` compatibility process |
|-|-----------------|--------------------------|
...
```
terminal(command="marvi chat -q 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# Background for long tasks:
terminal(command="marvi chat -q 'Set up CI/CD for ~/myapp'", background=true)
```
...
```
# Start
terminal(command="tmux new-session -d -s agent1 -x 120 -y 40 'marvi'", timeout=10)
...
```
# Agent A: backend
terminal(command="tmux new-session -d -s backend -x 120 -y 40 'marvi -w'", timeout=10)
terminal(command="sleep 8 && tmux send-keys -t backend 'Build REST API for user management' Enter", timeout=15)

# Agent B: frontend
terminal(command="tmux new-session -d -s frontend -x 120 -y 40 'marvi -w'", timeout=10)
...
terminal(command="tmux new-session -d -s resumed 'marvi --continue'", timeout=10)

# Resume specific session
terminal(command="tmux new-session -d -s resumed 'marvi --resume 20260225_143052_a1b2c3'", timeout=10)
```
...
- **Use `marvi chat -q` for fire-and-forget** — no PTY needed
...
the `cronjob` tool, the `marvi cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `marvi curator <verb>` — `status`, `run`, `pause`, `resume`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `marvi curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.marvi/skills/.usage.json` holds
...
Users drive it via `marvi kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `MARVI_MESSAGING_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `MARVI_MESSAGING_KANBAN_BOARD` pinned in env); tenant is a soft namespace
...
- **Desktop app** (`marvi desktop` / `marvi gui`) — native Electron app
...
- **Web dashboard** (`marvi dashboard`) — full admin panel: configure
  ...
  an embedded `marvi --tui` chat. Secured behind an OAuth/token gate.
- **OpenAI-compatible proxy** (`marvi proxy`) — exposes a
...
- **Automation Blueprints** — pick a named automation and Marvi asks for
...
Marvi runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`marvi config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Marvi's env scrubber dropping
...
Marvi-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Marvi tool and most Windows APIs. Prefer forward slashes in code
...
### Tool not available
1. `marvi tools` — check if toolset is enabled for your platform
2. Some tools need env vars (check `.env`)
3. `/reset` after enabling tools

### Model/provider issues
1. `marvi doctor` — check config and dependencies
2. `marvi auth` — re-authenticate OAuth providers (or `marvi auth add <provider>`)
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `marvi model` → GitHub Copilot.
...
### Skills not showing
1. `marvi skills list` — verify installed
2. `marvi skills config` — check platform enablement
3. Load explicitly: `/skill name` or `marvi -s name`

### Gateway issues
Check logs first:
```bash
grep -i "failed to send\|error" ~/.marvi/logs/gateway.log | tail -20
```

Common gateway problems:
- **Gateway dies on SSH logout**: Enable linger: `sudo loginctl enable-linger $USER`
- **Gateway dies on WSL2 close**: WSL2 requires `systemd=true` in `/etc/wsl.conf` for systemd services to work. Without it, gateway falls back to `nohup` (dies when session closes).
- **Gateway crash loop**: Reset the failed state: `systemctl --user reset-failed marvi-gateway`
...
```bash
marvi config set auxiliary.vision.provider <your_provider>
marvi config set auxiliary.vision.model <model_name>
```
...
|----------------|----------|
| Config options | `marvi config edit` or [Configuration docs](https://github.com/xRetr00/Marvi) |
| Available tools | `marvi tools list` or [Tools reference](https://github.com/xRetr00/Marvi) |
| Slash commands | `/help` in session or [Slash commands reference](https://github.com/xRetr00/Marvi) |
| Skills catalog | `marvi skills browse` or [Skills catalog](https://github.com/xRetr00/Marvi) |
| Provider setup | `marvi model` or [Providers guide](https://github.com/xRetr00/Marvi) |
| Platform setup | `marvi gateway setup` or [Messaging docs](https://github.com/xRetr00/Marvi) |
| MCP servers | `marvi mcp list` or [MCP guide](https://github.com/xRetr00/Marvi) |
| Profiles | `marvi profile list` or [Profiles docs](https://github.com/xRetr00/Marvi) |
| Cron jobs | `marvi cron list` or [Cron docs](https://github.com/xRetr00/Marvi) |
| Memory | `marvi memory status` or [Memory docs](https://github.com/xRetr00/Marvi) |
| Env variables | `marvi config env-path` or [Env vars reference](https://github.com/xRetr00/Marvi) |
| CLI commands | `marvi --help` or [CLI reference](https://github.com/xRetr00/Marvi) |
| Gateway logs | `~/.marvi/logs/gateway.log` |
| Session files | `marvi sessions browse` (reads state.db) |
| Source code | `~/.marvi/marvi-messaging/` |
...
├── cli.py                # Interactive CLI (MarviCLI)
├── marvi_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── runtime_support/           # CLI subcommands, config, setup, commands
...
Config: `~/.marvi/config.yaml` (settings), `~/.marvi/.env` (API keys) — both under `$MARVI_MESSAGING_HOME` when it is set.
...
`_MARVI_MESSAGING_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_marvi_home()` for paths,
never hardcode `~/.marvi`. For custom/local-only tools, write a plugin in
`~/.marvi/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `runtime_support/commands.py`
...
- Tests auto-redirect `MARVI_MESSAGING_HOME` to temp dirs — never touch real `~/.marvi/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/runtime_support/test_auth_toctou_file_modes.py`)
...
- Use `get_marvi_home()` from `marvi_constants` for all paths (profile-safe)
