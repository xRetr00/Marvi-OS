...
This skill is separate from the Marvi Email gateway adapter. The gateway
adapter lets people email the agent and uses Marvi' built-in IMAP/SMTP
...
To reply non-interactively from Marvi, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Marvi)** — pipe the message via stdin:
