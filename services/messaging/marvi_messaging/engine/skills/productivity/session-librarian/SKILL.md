...
| List/filter by metadata (age, source, cost, tokens, workspace) | `marvi sessions list` / `stats` via terminal |
| Rename | `marvi sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `marvi sessions archive <filters>` |
| Delete (destructive) | `marvi sessions delete` / `marvi sessions prune <filters>` |
| Export before deleting anything valuable | `marvi sessions export --session-id <id> --format md` |
...
`marvi sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `marvi sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `marvi sessions list`
