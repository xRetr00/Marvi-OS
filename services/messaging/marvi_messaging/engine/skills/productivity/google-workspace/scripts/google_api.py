#!/usr/bin/env python3
"""Google Workspace API CLI for Marvi Messaging.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Marvi-facing JSON contract and falls back to the Python client
...
from _marvi_home import get_marvi_home
...
def _gws_binary() -> str | None:
    override = os.getenv("MARVI_MESSAGING_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Marvi Messaging")
