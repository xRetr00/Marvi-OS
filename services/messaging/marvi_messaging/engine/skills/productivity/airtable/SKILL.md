...
4. Store the token in `${MARVI_MESSAGING_HOME:-~/.marvi}/.env` (or via `marvi setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Marvi. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${MARVI_MESSAGING_HOME:-~/.marvi}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
