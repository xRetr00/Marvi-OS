...
Ledger path resolution (first wins):
  --ledger PATH
  $MARVI_MESSAGING_CITATION_LEDGER
  $MARVI_MESSAGING_HOME/cache/citations/ledger.json
"""
...
from _marvi_home import get_marvi_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("MARVI_MESSAGING_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_marvi_home() / "cache" / "citations" / "ledger.json"
