...
`$MARVI_MESSAGING_HOME/config.yaml` (defaults to `~/.marvi/config.yaml` when `MARVI_MESSAGING_HOME` is unset)
...
Restart the Marvi session for changes to take effect. The MCP connection is
