...
MARVI_MESSAGING_HOME_DIR="${MARVI_MESSAGING_HOME:-$HOME/.marvi}"
MARVI_MESSAGING_CFG="${MARVI_MESSAGING_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Marvi config has twozero_td MCP entry ──
if [[ ! -f "$MARVI_MESSAGING_CFG" ]]; then
    echo -e " ${FAIL} Marvi config not found at ${MARVI_MESSAGING_CFG}"
    manual_steps+=("Create ${MARVI_MESSAGING_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$MARVI_MESSAGING_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Marvi config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Marvi config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$MARVI_MESSAGING_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${MARVI_MESSAGING_CFG} manually"); }
    manual_steps+=("Restart Marvi session to pick up config change")
fi
