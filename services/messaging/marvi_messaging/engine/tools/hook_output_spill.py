...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <MARVI_MESSAGING_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  MARVI_MESSAGING_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        base = Path(get_marvi_home()) / "hook_outputs"
