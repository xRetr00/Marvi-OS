...
etc.) surface as specific blocked checks via `marvi computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and xRetr00/Marvi-OS#67052.
    """
...
# fetches the latest release, so a `MARVI_MESSAGING_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `MARVI_MESSAGING_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "MARVI_MESSAGING_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Marvi should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Marvi should disable cua-driver telemetry for this user.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Marvi runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Marvi process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Marvi session's YOLO choice
    ...
    def __init__(self, driver_cmd: str, permission_mode: str) -> None:
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\marvi-cua-{token}"
        else:
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of xRetr00/Marvi-OS#47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Marvi an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``MARVI_MESSAGING_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Marvi Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `marvi computer-use doctor` succeeds from a login shell.
    """
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every Marvi process is restarted.
            os.path.join(
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``MARVI_MESSAGING_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `marvi computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  marvi computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `marvi tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of xRetr00/Marvi-OS#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of xRetr00/Marvi-OS#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Marvi-managed secrets out of the child env.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `marvi computer-use doctor` and check "
                f"{display_marvi_home()}/logs/agent.log for the phase timings."
            )
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    xRetr00/Marvi-OS#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of xRetr00/Marvi-OS#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Marvi run, with
        ...
        #     concurrent Marvi runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Marvi run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"marvi-{uuid.uuid4().hex[:12]}"
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of Marvi' minimal core.
    ...
    def capture(
        ...
        Maps marvi `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of xRetr00/Marvi-OS#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of xRetr00/Marvi-OS#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See xRetr00/Marvi-OS#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # xRetr00/Marvi-OS#47072). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Marvi adapter, and uses cua-driver's
