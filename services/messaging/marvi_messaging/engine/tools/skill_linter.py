...
Design contract (matches the Marvi "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    marvi_meta = meta.get("marvi") if isinstance(meta, dict) else None
    if not isinstance(marvi_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.marvi.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in marvi_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.marvi.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("marvi", "agent", "Marvi Messaging") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Marvi Messaging' (proper caps) "
