...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``marvi_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from marvi_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where marvi previously hard-rejected an oversized read
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Marvi.
...
_marvi_config_resolved: str | None = None
_marvi_config_resolved_loaded = False


def _get_marvi_config_resolved() -> str | None:
    """Return the resolved absolute path of the Marvi config file (cached)."""
    global _marvi_config_resolved, _marvi_config_resolved_loaded
    if _marvi_config_resolved_loaded:
        return _marvi_config_resolved
    _marvi_config_resolved_loaded = True
    try:
        from runtime_support.config import get_config_path
        _marvi_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _marvi_config_resolved = str(Path(_expand_tilde("~/.marvi/config.yaml")).resolve())
        except Exception:
            _marvi_config_resolved = None
    return _marvi_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Marvi config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    marvi_config = _get_marvi_config_resolved()
    if marvi_config and (resolved == marvi_config or normalized == marvi_config):
        return (
            f"Refusing to write to Marvi config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.marvi/config.yaml directly or use 'marvi config' instead."
        )
    return None
...
# .cursorrules (or a project-local .marvi config tree) outlives the current
...
_real_marvi_home_cached: str | None = None
_real_marvi_home_loaded = False


def _get_real_marvi_home() -> str | None:
    """Return the realpath of the authoritative Marvi home (cached)."""
    global _real_marvi_home_cached, _real_marvi_home_loaded
    if _real_marvi_home_loaded:
        return _real_marvi_home_cached
    _real_marvi_home_loaded = True
    try:
        from marvi_constants import get_marvi_home
        _real_marvi_home_cached = os.path.realpath(str(get_marvi_home()))
    except Exception:
        try:
            _real_marvi_home_cached = os.path.realpath(_expand_tilde("~/.marvi"))
        except Exception:
            _real_marvi_home_cached = None
    return _real_marvi_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from runtime_support.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.marvi home is governed by its own guards
    ...
    # ``.marvi`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_marvi_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .marvi config dirs (e.g. <repo>/.marvi/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.marvi`` — matching
        # any ancestor named .marvi would gate every write inside a
        # checkout that happens to live under ~/.marvi (e.g. the
        # marvi-messaging repo itself at ~/.marvi/marvi-messaging).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".marvi":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Marvi mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Marvi
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Marvi state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.marvi/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Marvi process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.marvi/…`` path.

    Returns ``None`` when the write is in-scope or outside Marvi scope.
    ...
    # in a session that cd'd into ``~/.marvi/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Marvi internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under MARVI_MESSAGING_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == MARVI_MESSAGING_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Marvi-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Marvi-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Marvi profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Marvi profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with marvi_tools.write_file() for very "
            "large files."
        )
