...
Lets the LLM write a Python script that calls Marvi tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `marvi_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `marvi_tools.py` with file-based RPC stubs
...
# available on every platform Marvi itself runs on.
...
# match a safe prefix, the operational MARVI_MESSAGING_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Marvi
# code can lose the DB-layer Kanban mutation guard while still inheriting
# MARVI_MESSAGING_HOME.
#
# NB: the broad "MARVI_MESSAGING_" prefix was deliberately removed (#27303) — it leaked
# MARVI_MESSAGING_*-named config that lacks a secret substring (e.g. MARVI_MESSAGING_BASE_URL,
# MARVI_MESSAGING_KANBAN_DB, MARVI_MESSAGING_*_WEBHOOK).  The child only needs the few
# location/profile vars in _MARVI_MESSAGING_CHILD_ALLOWED below; MARVI_MESSAGING_RPC_SOCKET /
# MARVI_MESSAGING_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational MARVI_MESSAGING_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set runtime_support treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_MARVI_MESSAGING_CHILD_ALLOWED = frozenset({
    "MARVI_MESSAGING_HOME",
    "MARVI_MESSAGING_PROFILE",
    "MARVI_MESSAGING_CONFIG",
    "MARVI_MESSAGING_ENV",
    "MARVI_MESSAGING_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational MARVI_MESSAGING_* vars (_MARVI_MESSAGING_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret MARVI_MESSAGING_* vars dropped by the tightened allowlist (#27303). The
    # broad "MARVI_MESSAGING_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like MARVI_MESSAGING_KANBAN_DB / MARVI_MESSAGING_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _MARVI_MESSAGING_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("MARVI_MESSAGING_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped MARVI_MESSAGING_* var.
            _dropped_marvi.append(k)
    if _dropped_marvi:
        logger.debug(
            "execute_code: dropped %d non-allowlisted MARVI_MESSAGING_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Marvi-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Marvi-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    marvi_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'marvi_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_marvi_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the marvi_tools.py stub module.
...
"""Auto-generated Marvi tools RPC stubs."""
...
def _connect():
    ...
    MARVI_MESSAGING_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["MARVI_MESSAGING_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("MARVI_MESSAGING_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Marvi tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("MARVI_MESSAGING_RPC_DIR") or os.path.join(tempfile.gettempdir(), "marvi_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated marvi_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/marvi_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_marvi_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/marvi_tools.py", tools_src)
        ...
        env_prefix = (
            f"MARVI_MESSAGING_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"MARVI_MESSAGING_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("MARVI_MESSAGING_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with marvi_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="marvi_sandbox_")
    ...
    # selector from MARVI_MESSAGING_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"marvi_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated marvi_tools module.
        ...
        tools_src = generate_marvi_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "marvi_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  MARVI_MESSAGING_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["MARVI_MESSAGING_RPC_SOCKET"] = rpc_endpoint
        child_env["MARVI_MESSAGING_RPC_TOKEN"] = rpc_token
        ...
        # MARVI_MESSAGING_TIMEZONE is an internal Marvi setting and must not leak
        # into child processes.
        _tz_name = os.getenv("MARVI_MESSAGING_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("MARVI_MESSAGING_TIMEZONE", None)
        ...
        # ``marvi_tools.py`` always lives in the staging directory, so that
        # directory must be importable even when project mode changes CWD.
        # Marvi's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Marvi's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        _marvi_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir]
        if _uses_marvi_python_environment(_child_python):
            _pp_parts.append(_marvi_root)
        elif _child_python not in _external_env_logged:
            # Import behavior changes silently otherwise — surface it (once
            # per interpreter path) so "import marvi_constants suddenly
            # fails" reports are diagnosable without log spam.
            _external_env_logged.add(_child_python)
            logger.info(
                "execute_code: child interpreter %s is outside the Marvi "
                "environment; marvi root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.marvi/.env')).
...
def _load_config() -> dict:
    ...
    ``marvi --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (marvi-messaging's python). Reproducible and the
...
# Interpreter paths already reported as outside the Marvi environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the marvi root from every
...
def _uses_marvi_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Marvi's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``marvi tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and marvi-messaging's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.marvi/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Marvi tools programmatically. "
        ...
        f"Available via `from marvi_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from marvi_tools import {import_str}` "
