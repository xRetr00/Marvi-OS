...
``runtime_support/web_server.py`` and
...
(``runtime_support.web_server._duplex_run_deep_task``).
...
- **Runtime**: ``auxiliary.voice_instant.{provider,model,base_url,api_key,
  ...
  path ``runtime_support.web_server`` already uses to emit one error event and
...
class RollingTranscript:
    ...
    ``runtime_support/web_server.py``'s ``_duplex_run_deep_task``).
    """
    ...
    # (runtime_support.web_server) to report deferred_context=ready load_ms=N.
...
class InstantLaneUnavailable(RuntimeError):
    ...
    ``runtime_support.web_server._drive_instant_lane_sync`` already treats as
...
def _configured_main_provider(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    (``model.provider``, then ``MARVI_MESSAGING_INFERENCE_PROVIDER``) -- the
    ...
    import ``runtime_support.runtime_provider.resolve_requested_provider``'s
    ...
    if provider and provider != "auto":
        return provider
    return os.environ.get("MARVI_MESSAGING_INFERENCE_PROVIDER", "").strip().lower()
...
def resolve_instant_runtime(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    same config namespace, same ``runtime_support.runtime_provider.resolve_runtime_provider``
    ...
    handled by ``runtime_support.web_server``, see :class:`InstantLaneUnavailable`).
    """
    from runtime_support.config import cfg_get, load_config
...
def warm_instant_lane(
    ...
    ``runtime_support.web_server._DuplexSession.start``), so the first real
...
def stream_instant_reply(
    ...
    *,
    ...
    time spent doing so. Used by ``runtime_support.web_server`` for
    ...
    if _LOCAL_TIME_QUERY_RE.search(utterance):
        from marvi_time import now as marvi_now
