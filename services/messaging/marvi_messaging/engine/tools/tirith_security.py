...
it is automatically downloaded from GitHub releases to $MARVI_MESSAGING_HOME/bin/tirith.
...
def _get_marvi_home() -> str:
    """Return the Marvi home directory, respecting MARVI_MESSAGING_HOME env var."""
    return str(get_marvi_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_marvi_home(), ".tirith-install-failed")
...
def _marvi_bin_dir() -> str:
    """Return $MARVI_MESSAGING_HOME/bin, creating it if needed."""
    d = os.path.join(_get_marvi_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $MARVI_MESSAGING_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_marvi_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $MARVI_MESSAGING_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $MARVI_MESSAGING_HOME/bin/tirith
    ...
    marvi_bin = os.path.join(_marvi_bin_dir(), "tirith")
    if os.path.isfile(marvi_bin) and os.access(marvi_bin, os.X_OK):
        _resolved_path = marvi_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return marvi_bin
