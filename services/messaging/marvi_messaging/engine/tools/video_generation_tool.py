...
provider** — turn on a backend by enabling a plugin (``marvi plugins
enable video_gen/<name>``) and selecting it in ``marvi tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/marvi-messaging-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `marvi tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `marvi plugins list` to see "
            f"installed video gen backends, or `marvi tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `marvi tools` → "
...
# `marvi tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`marvi tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `marvi tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
