...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.marvi/config.yaml``. Marvi
...
Configuration is loaded from ~/.marvi/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``runtime_support.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from runtime_support.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``marvi auth add <provider_id>``).
...
from tools.xai_http import marvi_xai_user_agent
...
# Base URL now resolved via runtime_support.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_marvi_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.marvi/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.marvi/config.yaml.
    ...
    try:
        ...
        logger.debug("runtime_support.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``marvi tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into Marvi without any Python code
...
#           output_format: wav
#
# Marvi writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__MARVI_MESSAGING_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Marvi secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Marvi secrets (salvage of #56332) while still
    ...
    scrubbed = marvi_subprocess_env(inherit_credentials=False)
...
# file, so retired models disappear from marvi the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``runtime_support.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `marvi setup` to configure, "
    ...
    from runtime_support.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `marvi model` or set XAI_API_KEY.")
    ...
    # output_format when Marvi actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": marvi_xai_user_agent(),
        },
        json=payload,
        timeout=60,
        stream=True,
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            ...
            _marvi_version = str(_runtime_support.__version__)
        except Exception:
            _marvi_version = "0.0.0"
        # Include Marvi client context following Gemini's partner
        # integration guidance:
        # https://ai.google.dev/gemini-api/docs/partner-integration
        headers["X-Goog-Api-Client"] = f"marvi-messaging/{_marvi_version}"
...
def _check_pockettts_available() -> bool:
    ...
    try:
        ...
        # ``marvi update`` deliberately installs only the lean ``[all]``
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Marvi caches Piper voice models.

    Resolves to ``~/.marvi/cache/piper-voices/`` under the active
    MARVI_MESSAGING_HOME so voice downloads follow profile boundaries.
    """
    from marvi_constants import get_marvi_dir
    root = Path(get_marvi_dir("cache/piper-voices", "piper_voices_cache"))
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("MARVI_MESSAGING_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'marvi-messaging[mistral]'"
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'marvi tools' and select Piper under TTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "PocketTTS provider selected but 'pocket-tts' package is not installed. "
                             "Run 'marvi tools' and select PocketTTS under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_marvi_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
