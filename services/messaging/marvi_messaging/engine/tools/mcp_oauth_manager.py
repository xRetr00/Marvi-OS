...
`mcp_tool.py`, and `runtime_support/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# MarviMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_marvi_provider_class() -> Optional[type]:
    ...
    class MarviMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._marvi_server_name = server_name
            self._marvi_home = ""
            ...
            self._marvi_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`MarviTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, MarviTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._marvi_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import MarviTokenStorage
                        if isinstance(storage, MarviTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._marvi_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import MarviTokenStorage
            if not isinstance(storage, MarviTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``marvi mcp reauth``.
            ...
            back to ``marvi mcp reauth``.
            """
            try:
                if self._marvi_preregistered:
                    return
    ...
    return MarviMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_MARVI_MESSAGING_PROVIDER_CLS: Optional[type] = _make_marvi_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._marvi_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`MarviMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _MARVI_MESSAGING_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `marvi mcp login "
        ...
        return _MARVI_MESSAGING_PROVIDER_CLS(
    ...
    def remove(
        ...
        *,
        ...
        Called by ``marvi mcp remove <name>`` and (indirectly) by
        ``marvi mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, marvi_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, marvi_home=marvi_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, marvi_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, marvi_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, marvi_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(marvi_home) / f"{_safe_filename(server_name)}.json"
            try:
