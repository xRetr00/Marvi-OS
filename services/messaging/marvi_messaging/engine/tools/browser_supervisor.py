...
One ``CDPSupervisor`` runs per Marvi ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Marvi logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "marvi-dialog-bridge.invalid"
...
(() => {
  if (window.__marviDialogBridgeInstalled) return;
  window.__marviDialogBridgeInstalled = true;
  const ENDPOINT = "http://marvi-dialog-bridge.invalid/";
