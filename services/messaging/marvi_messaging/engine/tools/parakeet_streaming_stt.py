...
def _load_parakeet_model(config: ParakeetStreamingConfig) -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run: marvi tools post-setup parakeet_stt"
        ) from exc
...
def _native_root() -> Path:
    ...
    return get_marvi_home() / "cache" / "parakeet-cpp"
...
def parakeet_venv_python(stt_config: dict[str, Any] | None = None) -> Path:
    ...
    return get_marvi_home() / "parakeet-venv" / ("Scripts/python.exe" if sys.platform == "win32" else "bin/python")
    ...
    def _transcribe_current(self) -> str:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                prefix="marvi-parakeet-", suffix=".wav", dir=self._temp_dir, delete=False
            ) as tmp:
                temp_path = tmp.name
