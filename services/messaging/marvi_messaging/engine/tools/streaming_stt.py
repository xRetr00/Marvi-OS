...
def _import_sherpa_onnx():
    try:
        ...
        raise WakeWordUnavailable(
            "sherpa-onnx is not installed. Run `marvi tools post-setup sherpa_onnx` "
...
def _import_livekit_wakeword_model():
    ...
    try:
        ...
        raise WakeWordUnavailable(
            "livekit-wakeword is not installed. Run `marvi tools post-setup livekit_wakeword` "
...
def _model_cache_dir(model_id: str) -> Path:
    return Path(get_marvi_dir(f"cache/sherpa-onnx/{model_id}", "sherpa_onnx_cache"))


def _livekit_model_cache_dir(model_id: str) -> Path:
    return Path(get_marvi_dir(f"cache/livekit-wakeword/{model_id}", "livekit_wakeword_cache"))
...
def _extract_tar_bz2(archive: Path, target_dir: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="marvi-kws-") as tmp_name:
...
def _write_wake_keywords_file(cfg: WakeWordConfig, files: dict[str, str]) -> str:
    ...
    if not cli:
        raise WakeWordUnavailable(
            "sherpa-onnx-cli is not available. Re-run `marvi tools post-setup sherpa_onnx` "
...
class LiveKitWakeWordSpotter:
    ...
    def _log_decision(
        ...
        *,
        ...
        try:
            log_path = get_marvi_home() / "logs" / _WAKEWORD_TELEMETRY_LOG
