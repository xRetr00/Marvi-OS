...
Copies bundled skills from the repo's skills/ directory into ~/.marvi/skills/
...
The manifest lives at ~/.marvi/skills/.bundled_manifest.
"""
...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
from marvi_constants import get_bundled_skills_dir, get_marvi_home, get_optional_skills_dir
...
SKILLS_DIR = MARVI_MESSAGING_HOME / "skills"
MANIFEST_FILE = SKILLS_DIR / ".bundled_manifest"

# Marker file written by `marvi profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.marvi profile).
# When present in MARVI_MESSAGING_HOME, sync_skills() is a no-op so neither the
# installer, `marvi update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# runtime_support.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks MARVI_MESSAGING_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.marvi/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.marvi/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``marvi skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `marvi skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.marvi/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.marvi) that wrote the
    ...
    if (MARVI_MESSAGING_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.marvi/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `marvi update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `marvi skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``MARVI_MESSAGING_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.marvi/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # path join, a missing ``MARVI_MESSAGING_HOME`` default, a malicious
    # bundled-manifest entry, or a mid-flight exception that leaves a
    # stale path in scope — this guard turns the resulting
    # ``shutil.rmtree(~/.marvi)`` into a loud, recoverable ``ValueError``
    ...
    # ``MARVI_MESSAGING_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `marvi skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `marvi update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``marvi update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``marvi update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``marvi skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `marvi skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes MARVI_MESSAGING_HOME/.no-bundled-skills so the
    installer, ``marvi update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``marvi skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = MARVI_MESSAGING_HOME / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            MARVI_MESSAGING_HOME.mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding "
                "(`marvi skills opt-out`).\n"
            ...
            changed = existed
            message = (
                "Opted back in. The next `marvi update` (or `marvi skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (MARVI_MESSAGING_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.marvi/skills/ ...")
