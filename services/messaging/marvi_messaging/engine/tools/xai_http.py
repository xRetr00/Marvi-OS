...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``marvi tools`` repaint, tool-registration scans,
    ...
    2. ``~/.marvi/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``marvi auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_marvi_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.marvi/.env`` first, then ``os.environ``.

    Wraps :func:`runtime_support.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from runtime_support.config import get_env_value as _marvi_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _marvi_get_env_value(name)
    return value if value is not None else default


def marvi_xai_user_agent() -> str:
    """Return a stable Marvi-specific User-Agent for xAI HTTP calls."""
    try:
        from runtime_support import __version__
    except Exception:
        __version__ = "unknown"
    return f"Marvi Messaging/{__version__}"


def marvi_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Marvi Messaging,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": marvi_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Marvi config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Marvi home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_marvi_home() / "state"
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Marvi-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``runtime_support.config.get_env_value`` so keys
    stored in ``~/.marvi/.env`` (the standard Marvi location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("MARVI_MESSAGING_XAI_BASE_URL")
