...
_FORCE_SYNC_ENV = "MARVI_MESSAGING_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.marvi") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.marvi to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``MARVI_MESSAGING_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, marvi_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.marvi/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(marvi_home=marvi_home)

    def _sync_back_transaction(self, marvi_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="marvi-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.marvi/skills/a.md`` →
        ``~/.marvi/skills/a.md``, a new remote file at
        ``/root/.marvi/skills/b.md`` maps to ``~/.marvi/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
