...
This module provides the glue:
    - ``MarviTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Marvi Messaging"
"""
...
from marvi_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__marvi_user_skipped__"
...
def _get_token_dir(marvi_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses MARVI_MESSAGING_HOME so each profile gets its own OAuth tokens.
    Layout: ``MARVI_MESSAGING_HOME/mcp-tokens/``
    """
    from marvi_constants import get_marvi_home

    base = Path(marvi_home) if marvi_home is not None else Path(get_marvi_home())
...
def _cached_redirect_port(storage: "MarviTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Marvi restarts and chooses a
...
def _cached_redirect_uri(storage: "MarviTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``marvi mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `marvi mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# MarviTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class MarviTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        MARVI_MESSAGING_HOME/mcp-tokens/<server_name>.json         -- tokens
        MARVI_MESSAGING_HOME/mcp-tokens/<server_name>.client.json   -- client info
        MARVI_MESSAGING_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, marvi_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._marvi_home = Path(marvi_home) if marvi_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._marvi_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._marvi_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._marvi_home) / f"{self._server_name}.meta.json"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Marvi records an absolute wall-clock ``expires_at`` alongside the
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._marvi_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Marvi.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `marvi mcp login <server>` later to "
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = MarviTokenStorage(server_name, marvi_home=marvi_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "MarviTokenStorage | None" = None,
) -> int:
...
#   "Claude Code" → 200
#   "Codex"       → 200
    #   "Marvi Messaging" / "Marvi" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Marvi Messaging")
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: marvi mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Marvi auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Marvi is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = MarviTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`marvi mcp login {server_name}` interactively first to complete "
