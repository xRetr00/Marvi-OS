...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/marvi-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/marvi-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/marvi-results"
HEREDOC_MARKER = "MARVI_MESSAGING_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/marvi-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"MARVI_MESSAGING_PERSIST_{uuid.uuid4().hex[:8]}"
