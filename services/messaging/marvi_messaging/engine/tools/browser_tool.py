...
from runtime_support.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Marvi secret
...
def _build_browser_env() -> dict:
    ...
    Strips Marvi-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``marvi_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = marvi_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    marvi_node_bin = str(marvi_home / "node" / "bin")
    marvi_node_root = str(marvi_home / "node")
    marvi_nm_bin = str(marvi_home / "node_modules" / ".bin")
    return [marvi_node_bin, marvi_node_root, marvi_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``marvi doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.marvi/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Marvi profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.marvi/plugins/browser/<vendor>/``) participate
...
# comparison sites (four here, plus runtime_support/tools_config.py and
# runtime_support/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Marvi silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Marvi-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Marvi CLI where sys.stdout and sys.stderr both report
...
def _allow_private_urls() -> bool:
    ...
    if get_marvi_home_override() is not None:
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-marvi_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed marvi processes — this way every clean marvi exit sweeps
    ...
    # Sweep orphans from other crashed marvi processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live marvi processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current marvi PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live marvi process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning marvi process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced marvi PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent marvi instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-marvi_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live marvi process.
            continue
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Marvi-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Marvi-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_marvi_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Marvi-managed
    ...
    # node_modules binary, which disappears on the next ``marvi update`` and
    ...
    # Build an extended search PATH including Marvi-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_marvi_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_marvi_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_marvi_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``marvi update`` (or ``marvi doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Marvi holds, while running registry-fetched
    npm code on every ``marvi update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `marvi update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this marvi PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_marvi_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = marvi_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Marvi
    ...
    from marvi_constants import get_marvi_dir
    screenshots_dir = get_marvi_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/marvi-messaging:latest")
                else:
