#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Marvi Skills Hub.
...
Used by runtime_support/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _marvi_home() -> Path:
    return get_marvi_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _marvi_home() / "skills"
...
def _marvi_index_cache_file() -> Path:
    return _index_cache_dir() / "marvi-index.json"


_DYNAMIC_PATH_RESOLVERS = {
    "MARVI_MESSAGING_HOME": _marvi_home,
    ...
    "MARVI_MESSAGING_INDEX_CACHE_FILE": _marvi_index_cache_file,
}

MARVI_MESSAGING_HOME = _DynamicPath("MARVI_MESSAGING_HOME", _marvi_home)
...
MARVI_MESSAGING_INDEX_CACHE_FILE = _DynamicPath("MARVI_MESSAGING_INDEX_CACHE_FILE", _marvi_index_cache_file)
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            marvi_meta = metadata.get("marvi", {})
            if isinstance(marvi_meta, dict):
                tags = marvi_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "xRetr00/Marvi-OS"
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``marvi update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                marvi_meta = meta_block.get("marvi", {})
                if isinstance(marvi_meta, dict):
                    tags = marvi_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``runtime_support.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Marvi centralized index source
# ---------------------------------------------------------------------------

MARVI_MESSAGING_INDEX_URL = "https://raw.githubusercontent.com/xRetr00/Marvi/main/website/static/api/skills-index.json"
MARVI_MESSAGING_INDEX_TTL = 6 * 3600  # 6 hours


def _load_marvi_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for MARVI_MESSAGING_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    marvi_index_cache_file = _marvi_index_cache_file()
    if marvi_index_cache_file.exists():
        try:
            age = time.time() - marvi_index_cache_file.stat().st_mtime
            if age < MARVI_MESSAGING_INDEX_TTL:
                return json.loads(marvi_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            resp = httpx.get(
                MARVI_MESSAGING_INDEX_URL,
                timeout=15,
                follow_redirects=True,
                headers={"Accept-Encoding": accept_encoding},
            )
            if resp.status_code != 200:
                logger.debug("Marvi index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Marvi index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Marvi index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        marvi_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        marvi_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class MarviIndexSource(SkillSource):
    """Skill source backed by the centralized Marvi Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "marvi-index")
                bundle.identifier = identifier
                return bundle
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "marvi-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        MarviIndexSource(auth=auth), # Centralized index (search + resolved install paths)
