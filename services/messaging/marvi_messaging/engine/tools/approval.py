...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("MARVI_MESSAGING_YOLO_MODE", ""))
...
# Marvi session id (observability identity, distinct from the gateway
...
# os.environ["MARVI_MESSAGING_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export MARVI_MESSAGING_INTERACTIVE.
_marvi_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "marvi_interactive",
    default=None,
)


def set_marvi_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["MARVI_MESSAGING_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``MARVI_MESSAGING_INTERACTIVE`` env var for legacy callers.
    """
    return _marvi_interactive_ctx.set("1" if interactive else "")


def reset_marvi_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_marvi_interactive_context`."""
    _marvi_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``MARVI_MESSAGING_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _marvi_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("MARVI_MESSAGING_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Marvi session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("MARVI_MESSAGING_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("MARVI_MESSAGING_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("MARVI_MESSAGING_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("MARVI_MESSAGING_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("MARVI_MESSAGING_CRON_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set MARVI_MESSAGING_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind MARVI_MESSAGING_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds MARVI_MESSAGING_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("MARVI_MESSAGING_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``MARVI_MESSAGING_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $MARVI_MESSAGING_HOME, or by the resolved absolute
# active profile home path such as /home/marvi/.marvi/config.yaml. The
# resolved-absolute form is folded into the ~/.marvi/ patterns at detection
...
# go stale when MARVI_MESSAGING_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_MARVI_MESSAGING_ENV_PATH = (
    r'(?:~\/\.marvi/|'
    r'(?:\$home|\$\{home\})/\.marvi/|'
    r'(?:\$marvi_home|\$\{marvi_home\})/)'
    r'\.env\b'
)
# ~/.marvi/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _MARVI_MESSAGING_ENV_PATH; matches the MARVI_MESSAGING_HOME override form as
# well as ~/.marvi/.
_MARVI_MESSAGING_CONFIG_PATH = (
    r'(?:~\/\.marvi/|'
    r'(?:\$home|\$\{home\})/\.marvi/|'
    r'(?:\$marvi_home|\$\{marvi_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_MARVI_MESSAGING_ENV_PATH}|'
    rf'{_MARVI_MESSAGING_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_marvi_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Marvi: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Marvi
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]marvi[^\n]*\.env\b', "access to Marvi secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `marvi` and `gateway` (e.g. `marvi -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bmarvi\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart marvi gateway (kills running agents)"),
    (r'\bmarvi\s+update\b', "marvi update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `marvi gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `marvi ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart marvi-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart marvi-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(marvi|gateway|cli\.py)\b', "kill marvi/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill marvi` but not
    # `kill -9 $(pgrep -f marvi)` because the substitution is opaque
    ...
    # the `marvi gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.marvi.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(marvi|ai\.marvi)\b', "stop/restart marvi launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Marvi file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.marvi/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Marvi-managed security file (~/.marvi/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_MARVI_MESSAGING_CONFIG_PATH}|{_MARVI_MESSAGING_ENV_PATH})', "in-place edit of Marvi config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_MARVI_MESSAGING_CONFIG_PATH}|{_MARVI_MESSAGING_ENV_PATH})', "in-place edit of Marvi config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_MARVI_MESSAGING_CONFIG_PATH}|{_MARVI_MESSAGING_ENV_PATH})', "in-place edit of Marvi config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.marvi/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / MARVI_MESSAGING_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Marvi home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\marvi), so folding the user home
    # first would eat the prefix the Marvi-home fold needs.
    command = _rewrite_resolved_marvi_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / MARVI_MESSAGING_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.marvi``); the matched
...
def _rewrite_resolved_marvi_home(command: str) -> str:
    """Rewrite the resolved absolute Marvi home prefix to ``~/.marvi/``.

    Resolves the active ``MARVI_MESSAGING_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.marvi/`` so the static ``_MARVI_MESSAGING_CONFIG_PATH`` / ``_MARVI_MESSAGING_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/marvi/.marvi/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$MARVI_MESSAGING_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from marvi_constants import get_marvi_home
        home = get_marvi_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.marvi")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``marvi send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Marvi ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"marvi-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Marvi YOLO.
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["MARVI_MESSAGING_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "MARVI_MESSAGING_SPINNER_PAUSE" in os.environ:
            del os.environ["MARVI_MESSAGING_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Marvi approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``MARVI_MESSAGING_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            MARVI_MESSAGING_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set MARVI_MESSAGING_INTERACTIVE or "
                "MARVI_MESSAGING_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set MARVI_MESSAGING_INTERACTIVE or "
            "MARVI_MESSAGING_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("MARVI_MESSAGING_EXEC_ASK"):
        ...
        # can see (MARVI_MESSAGING_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``MARVI_MESSAGING_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from runtime_support.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from runtime_support.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("MARVI_MESSAGING_EXEC_ASK")
        ...
        if _is_cron_approval_context():
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from runtime_support.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (MARVI_MESSAGING_EXEC_ASK) still takes this path even when INTERACTIVE
