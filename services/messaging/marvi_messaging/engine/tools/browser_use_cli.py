...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Marvi's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Marvi's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_marvi_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `marvi tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Marvi' own bin dir ($MARVI_MESSAGING_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from marvi_constants import get_marvi_home

        return str(Path(get_marvi_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Marvi' own ``$MARVI_MESSAGING_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    Marvi-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Marvi' managed uv (bootstrapped on demand via
    ``runtime_support.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$MARVI_MESSAGING_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Marvi-managed copy, or resolution
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_marvi_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(env: dict, task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "switch backends via `marvi tools` → Browser Automation."
        )
