"""Central registry for all marvi-messaging tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (marvi_constants itself is stdlib-only, so no cycle).
        from marvi_constants import get_marvi_home

        return Path(get_marvi_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``marvi tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Marvi-home override for every
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``marvi tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved MARVI_MESSAGING_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("marvi_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``marvi_plugins.{name}``), not the exact module string.
                # A handler defined in ``marvi_plugins.pkg.handlers`` is
                # still owned by the ``marvi_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``marvi tools enable foo``)
