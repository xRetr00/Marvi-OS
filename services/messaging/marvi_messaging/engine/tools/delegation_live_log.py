...
per child under::
    ...
    <marvi_home>/cache/delegation/live/<delegation_id>/task-<n>.log
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.marvi)."""
    from marvi_constants import get_marvi_dir

    return get_marvi_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        try:
            ...
            header = [
                "=== Marvi subagent live transcript ===",
