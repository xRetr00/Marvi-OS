...
`marvi skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.marvi.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.marvi.blueprint`` is a mapping containing
    ...
    marvi = meta.get("marvi") if isinstance(meta, dict) else None
    blueprint = marvi.get("blueprint") if isinstance(marvi, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.marvi.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.marvi.blueprint`` block) they can hand
    to ``marvi skills publish`` to share. ``body`` is the plain-language
