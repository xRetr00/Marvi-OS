...
``marvi mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Marvi desktop app. Use the "
            "terminal instead: `marvi mcp install <name>` for catalog entries, "
            "`marvi mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Marvi desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `marvi mcp catalog` "
