...
def auth_json_path():
    """Return the Marvi auth store path, respecting MARVI_MESSAGING_HOME overrides."""
    return get_marvi_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`marvi tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Marvi release, and
