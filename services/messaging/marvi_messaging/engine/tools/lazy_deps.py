"""
Lazy dependency installer for opt-in Marvi Messaging backends.

Many Marvi features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``marvi-messaging[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``marvi tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``MARVI_MESSAGING_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/marvi`` read-only), setting
  ``MARVI_MESSAGING_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Marvi, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (MARVI_MESSAGING_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (marvi trace upload / /upload-trace).
    ...
    # Optional everywhere: the subconscious sync cron job and `marvi composio
...
_LAZY_TARGET_ENV = "MARVI_MESSAGING_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``MARVI_MESSAGING_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("MARVI_MESSAGING_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``marvi update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Marvi under WSL to use Matrix on Windows."
        )
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``marvi update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="marvi-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``runtime_support.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = marvi_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $MARVI_MESSAGING_HOME/bin is never on PATH, so a bare
        # which() misses the uv Marvi installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Marvi
    ...
    # deployment sets MARVI_MESSAGING_MANAGED=true *and* MARVI_MESSAGING_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from runtime_support.config import get_managed_system
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Marvi cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Marvi instead)."
            )
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> Optional[str]:
    ...
    (default install, ``MARVI_MESSAGING_HOME`` overrides, profile installs) and
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``MARVI_MESSAGING_DISABLE_LAZY_INSTALLS=1``) and sets
      ``MARVI_MESSAGING_LAZY_INSTALL_TARGET``, installs are redirected to the writable
    ...
    * **Gated** — honors ``security.allow_lazy_installs`` and refuses to run
    ...
    if not _allow_lazy_installs():
        target = _lazy_install_target()
        if os.environ.get("MARVI_MESSAGING_DISABLE_LAZY_INSTALLS") == "1" and target is None:
            reason = (
                "runtime installs are disabled on this deployment: the agent "
                "environment is immutable and no writable install target is "
                "configured (MARVI_MESSAGING_LAZY_INSTALL_TARGET)"
            )
        else:
...
def active_features() -> list[str]:
    ...
    Used by ``marvi update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``marvi update``. Never raises; lazy-install failures
