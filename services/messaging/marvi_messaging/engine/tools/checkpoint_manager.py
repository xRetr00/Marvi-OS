...
-----------------------------------------------------------------------------
    ...
    ~/.marvi/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/marvi/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_marvi_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/marvi"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Marvi convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("MARVI_MESSAGING_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Marvi infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``marvi checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `marvi checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "marvi@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Marvi Checkpoint"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "MARVI_MESSAGING_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``MARVI_MESSAGING_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at MARVI_MESSAGING_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "MARVI_MESSAGING_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Marvi changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Marvi changed", not an exact per-session ledger.
...
# ---------------------------------------------------------------------------
# Public helpers for `marvi checkpoints` CLI
# ---------------------------------------------------------------------------
