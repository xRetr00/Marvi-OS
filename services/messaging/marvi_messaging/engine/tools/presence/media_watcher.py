...
Managed by `marvi presence setup` / `pause` / `resume` (see
runtime_support/presence_cmd.py), which spawns this as a detached background
process and tracks its PID under ~/.marvi/presence/media_watcher.json.
"""
...
WINSDK_INSTALL_HINT = (
    ...
    "only). Install with: pip install \"marvi-messaging[presence]\" "
