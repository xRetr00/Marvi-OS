...
(see ``runtime_support`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<MARVI_MESSAGING_HOME>/pending/{memory,skills}/<id>.json``
...
def _pending_dir(subsystem: str) -> Path:
    return get_marvi_home() / "pending" / subsystem
