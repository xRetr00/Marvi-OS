...
Extracted verbatim from marvi_state.py so the SessionDB mixin modules
(marvi_state_search / marvi_state_schema / marvi_state_portability) can
reference them without importing marvi_state (which would be a cycle).
marvi_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``marvi sessions optimize-storage``. A legacy DB sits at
...
# `marvi sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `marvi db optimize`. This is
