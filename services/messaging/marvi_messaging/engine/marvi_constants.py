"""Shared constants for Marvi Messaging.
...
PRODUCT_NAME = "Marvi Messaging"
PRODUCT_SHORT_NAME = "Marvi"
LEGACY_PRODUCT_NAME = "Marvi Messaging"
LEGACY_PRODUCT_SHORT_NAME = "Marvi"
MARVI_MESSAGING_HOME_ENV = "MARVI_MESSAGING_HOME"
MARVI_HOME_ENV = "MARVI_HOME"

_profile_fallback_warned: bool = False
_UNSET = object()
_MARVI_MESSAGING_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_MARVI_MESSAGING_HOME_OVERRIDE", default=_UNSET
)
...
def set_marvi_home_override(path: str | Path | None) -> Token:
    """Set a context-local Marvi home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _MARVI_MESSAGING_HOME_OVERRIDE.set(value)


def reset_marvi_home_override(token: Token) -> None:
    """Restore the previous context-local Marvi home override."""
    _MARVI_MESSAGING_HOME_OVERRIDE.reset(token)


def get_marvi_home_override() -> str | None:
    """Return the active context-local Marvi home override, if any."""
    override = _MARVI_MESSAGING_HOME_OVERRIDE.get()
...
def _marvi_home_from_env() -> Path:
    """Resolve Marvi's home from the process environment only.

    Reads the ``MARVI_MESSAGING_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_marvi_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_marvi_home`
    and :func:`get_process_marvi_home` so the two never drift.
    """
    val = os.environ.get(MARVI_MESSAGING_HOME_ENV, "").strip()
    if val:
        return Path(val)
    val = os.environ.get(MARVI_HOME_ENV, "").strip()
    if val:
        return Path(val)
    return _get_platform_default_marvi_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``MARVI_MESSAGING_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_marvi_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[MARVI_HOME fallback] MARVI_HOME/MARVI_MESSAGING_HOME are unset but active "
...
def get_marvi_home() -> Path:
    """Return the Marvi home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_marvi_home_override`) → ``MARVI_MESSAGING_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``MARVI_MESSAGING_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``MARVI_MESSAGING_HOME`` explicitly (see the systemd
    template in ``runtime_support/gateway.py`` and the kanban dispatcher in
    ``runtime_support/kanban_db.py``).  See https://github.com/xRetr00/Marvi-OS/issues/18594.
    """
    ...
    if not (
        os.environ.get(MARVI_MESSAGING_HOME_ENV, "").strip()
        or os.environ.get(MARVI_HOME_ENV, "").strip()
    ):
        _warn_profile_fallback_once()

    return _marvi_home_from_env()


def marvi_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Marvi home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_marvi_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_marvi_home() -> Path:
    """Return the Marvi home for the running process, ignoring task overrides.

    Unlike :func:`get_marvi_home`, this never follows the context-local
    override set by :func:`set_marvi_home_override`.  It resolves only the
    process ``MARVI_MESSAGING_HOME`` env var (falling back to the platform default),
...
def get_default_marvi_root() -> Path:
    """Return the root Marvi directory for profile-level operations.

    In standard deployments this is the platform-native Marvi home
    (``~/.marvi`` on POSIX, ``%LOCALAPPDATA%\\marvi`` on native Windows).

    In Docker or custom deployments where ``MARVI_MESSAGING_HOME`` points outside
    ``~/.marvi`` (e.g. ``/opt/data``), returns ``MARVI_MESSAGING_HOME`` directly
    — that IS the root.

    In profile mode where ``MARVI_MESSAGING_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.marvi/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_marvi_home()
    env_home = (
        os.environ.get(MARVI_MESSAGING_HOME_ENV, "").strip()
        or os.environ.get(MARVI_HOME_ENV, "").strip()
    )
    ...
    try:
        ...
        # MARVI_MESSAGING_HOME is under ~/.marvi (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — MARVI_MESSAGING_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``MARVI_MESSAGING_OPTIONAL_SKILLS``.
    """
    override = os.getenv("MARVI_MESSAGING_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_marvi_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``MARVI_MESSAGING_OPTIONAL_MCPS``.
    """
    override = os.getenv("MARVI_MESSAGING_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_marvi_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``MARVI_MESSAGING_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<MARVI_MESSAGING_HOME>/skills`` last-resort
    """
    override = os.getenv("MARVI_MESSAGING_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_marvi_home() / "skills"
...
def get_marvi_dir(
    ...
    *,
    ...
    """Resolve a Marvi subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to MARVI_MESSAGING_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to MARVI_MESSAGING_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Marvi home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local MARVI_MESSAGING_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_marvi_home()
...
def iter_marvi_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Marvi-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\marvi\\node``. POSIX installs use
    ``$MARVI_MESSAGING_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_marvi_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with marviManagedNodePathEntries() in
...
_MARVI_MESSAGING_NODE_TARGET_MAJOR = int(os.environ.get("MARVI_MESSAGING_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Marvi-managed Node trees live under ``$MARVI_MESSAGING_HOME/node`` (or a profile's
    ``MARVI_MESSAGING_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_marvi_node_executable``
    used to trust file presence alone, so ``marvi update`` would pick that
...
def marvi_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Marvi-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_marvi_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%MARVI_MESSAGING_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_marvi_home()
    index_url = f"https://nodejs.org/dist/latest-v{_MARVI_MESSAGING_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_MARVI_MESSAGING_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$MARVI_MESSAGING_HOME/node`` on POSIX.
    ...
    ``MARVI_MESSAGING_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
    ...
    try:
        result = subprocess.run(
            ...
            env={
                **os.environ,
                "MARVI_MESSAGING_HOME": str(get_marvi_home()),
                ...
                "MARVI_MESSAGING_NODE_SKIP_LINKS": "1",
            },
            capture_output=True,
            timeout=600,
            check=False,
...
def bootstrap_marvi_managed_node() -> str | None:
    """Install a Marvi-managed Node tree and return its npm path.
    ...
    Marvi never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$MARVI_MESSAGING_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_marvi_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_marvi_node_dirs():
...
def heal_marvi_managed_node() -> bool:
    """Redownload Marvi-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not marvi_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MARVI_MESSAGING_HOME": str(get_marvi_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_marvi_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_MARVI_MESSAGING_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_marvi_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _MARVI_MESSAGING_NODE_TARGET_MAJOR
    return False


def find_marvi_node_executable(command: str) -> str | None:
    """Return a Marvi-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_MARVI_MESSAGING_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_marvi_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Marvi-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Marvi-managed installs.

    This is for Marvi-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_marvi_node_executable(command)
    if managed:
        return managed
    if marvi_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_marvi_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Marvi-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_marvi_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``marvi update`` — leaving a **dangling symlink** that ``which`` still
...
def display_marvi_home() -> str:
    """Return a user-friendly display string for the current MARVI_MESSAGING_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.marvi``
        profile:  ``~/.marvi/profiles/coder``
        custom:   ``/opt/marvi-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.marvi``.  For code that needs a real ``Path``, use
    :func:`get_marvi_home` instead.
    """
    home = get_marvi_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``MARVI_MESSAGING_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/xRetr00/Marvi-OS/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{MARVI_MESSAGING_HOME}/home`` when the profile-home directory exists."""
    marvi_home = get_marvi_home_override() or (env or {}).get("MARVI_MESSAGING_HOME") or os.getenv("MARVI_MESSAGING_HOME")
    if not marvi_home:
        return None
    profile_home = os.path.join(marvi_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("MARVI_MESSAGING_REAL_HOME") or os.getenv("MARVI_MESSAGING_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Marvi profile HOME.

    ``MARVI_MESSAGING_HOME`` scopes Marvi state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={MARVI_MESSAGING_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{MARVI_MESSAGING_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{MARVI_MESSAGING_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Marvi' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["MARVI_MESSAGING_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Marvi itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under MARVI_MESSAGING_HOME.

    Replaces the ``get_marvi_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, marvi_logging.py, marvi_time.py, etc.).
    """
    return get_marvi_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under MARVI_MESSAGING_HOME."""
    return get_marvi_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under MARVI_MESSAGING_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_marvi_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._marvi_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``runtime_support`` modules using three different Windows
    ...
    A few sites outside ``runtime_support`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``runtime_support.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Marvi itself. An ImportError
...
# `runtime_support.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Marvi.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("marvi_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `marvi update` is exactly
    ...
    return [
        ...
        "    marvi update",
        "If that also fails, reinstall: https://github.com/xRetr00/Marvi-OS",
    ]
