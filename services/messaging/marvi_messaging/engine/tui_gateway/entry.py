...
# from shadowing Marvi's own top-level modules.  ``marvi_bootstrap`` lives at
...
import marvi_bootstrap

marvi_bootstrap.harden_import_path()
...
# shared owner in runtime_support.mcp_startup — the wait/in-flight/join helpers
...
# runtime_support.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `MARVI_MESSAGING_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("MARVI_MESSAGING_TUI_SIDECAR_URL")
...
# tested; override via ``MARVI_MESSAGING_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("MARVI_MESSAGING_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``MARVI_MESSAGING_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``marvi --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``runtime_support.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from runtime_support.mcp_startup import _resolve_discovery_timeout
    ...
    # → runtime_support.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's MARVI_MESSAGING_HOME first), so a launch profile with no
    ...
    try:
        from runtime_support.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``marvi --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``marvi dashboard`` spawn theirs via
    ``runtime_support.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``runtime_support.mcp_startup`` thread used by the
    ...
    try:
        from runtime_support.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from runtime_support.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's MARVI_MESSAGING_HOME override — the shared owner in
    ``runtime_support.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins Marvi activates mid-conversation.
