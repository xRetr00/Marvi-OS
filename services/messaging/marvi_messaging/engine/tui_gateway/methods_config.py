...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from runtime_support.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_marvi_home), "display": display_marvi_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _marvi_home / "config.yaml"
        try:
...
def _(rid, params: dict) -> dict:
    try:
        from runtime_support.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Marvi provider is configured.",
                },
            )
