...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind MARVI_MESSAGING_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # custom `MARVI_MESSAGING_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        _enable_gateway_prompts()
        home_token = (
            set_marvi_home_override(str(profile_home)) if profile_home is not None else None
        )
        secret_token = (
        ...
        with _session_resume_lock:
            ...
            try:
                init_home_token = (
                    set_marvi_home_override(str(profile_home))
                    if profile_home is not None
                    else None
                )
                init_secret_token = (
                ...
                try:
                    ...
                    if init_home_token is not None:
                        reset_marvi_home_override(init_home_token)
                ...
                if sid in _sessions:
                    ...
                    # Remember the profile home so each turn re-binds MARVI_MESSAGING_HOME (the
        ...
        # atexit.register(_drain_token_queue_at_exit) (marvi_state.py), which only
...
def _(rid, params: dict) -> dict:
    ...
    # ``marvi --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params) as db:
        ...
        if profile_home is not None:
            ...
            sessions_dir = get_marvi_home() / "sessions"
        try:
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``marvi gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from runtime_support.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from runtime_support.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from runtime_support.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from runtime_support.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    from runtime_support.nous_billing import BillingError, post_subscription_preview
...
def _(rid, params: dict) -> dict:
    ...
    billing:manage.
    """
    from runtime_support.nous_billing import BillingError, put_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    """
    from runtime_support.nous_billing import BillingError, delete_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    from runtime_support.nous_billing import BillingError, post_subscription_upgrade
...
def _(rid, params: dict) -> dict:
    ...
    """
    from runtime_support.nous_billing import BillingError, post_charge
    from agent.billing_view import new_idempotency_key
...
def _(rid, params: dict) -> dict:
    ...
    """
    from runtime_support.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from runtime_support.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from runtime_support.auth import step_up_nous_billing_scope
        from runtime_support.nous_billing import BillingError
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Marvi profile home
    # (~/.marvi/sessions/saved/) rather than the project/workspace CWD, and
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if parent_home:
            ...
            branch_owns_db = True
        home_token = (
            set_marvi_home_override(parent_home) if parent_home else None
        )
