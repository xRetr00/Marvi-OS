...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.marvi/.env`` into the gateway process via
    ``runtime_support.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m runtime_support.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "runtime_support.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m runtime_support.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=marvi_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from runtime_support.moa_config import moa_usage, normalize_moa_config
    ...
    if name == "loop":
        ...
        if not session:
            return _err(rid, 4001, "no active session")
        try:
            from runtime_support.loops import LoopManager, dispatch_loop_command
        except Exception as exc:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("MARVI_MESSAGING_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("MARVI_MESSAGING_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_marvi_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``marvi journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    if profile:
        try:
            ...
            from marvi_constants import set_marvi_home_override
    ...
    try:
        ...
        if action == "install":
            from runtime_support.skills_hub import do_install
        ...
        if action == "browse":
            from runtime_support.skills_hub import browse_skills
        ...
        if action == "inspect":
            from runtime_support.skills_hub import inspect_skill
        ...
        if token is not None:
            try:
                from marvi_constants import reset_marvi_home_override
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `marvi mcp` offers, so
    ...
    try:
        ...
        from runtime_support import mcp_catalog
...
# (runtime_support/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes MARVI_MESSAGING_HOME via set_marvi_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses runtime_support/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from runtime_support.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from runtime_support.config import load_config, save_config, save_env_value
        from runtime_support.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from runtime_support.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.marviDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``marvi mcp login`` uses
    ...
    try:
        ...
        marvi_home = str(get_marvi_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(marvi_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``marvi plugins`` / the dashboard, so the three surfaces
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Marvi plugin.
