...
def _default_registry_path() -> Path:
    return get_marvi_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_marvi_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_marvi_home = expected_marvi_home if expected_marvi_home is not None else str(get_marvi_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["MARVI_MESSAGING_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("marvi_home") or "")
        if got_home and got_home != self.expected_marvi_home:
            raise RuntimeError(f"compute host MARVI_MESSAGING_HOME mismatch: {got_home} != {self.expected_marvi_home}")
