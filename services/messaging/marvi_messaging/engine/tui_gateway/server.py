...
load_marvi_dotenv(
    marvi_home=_marvi_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.marvi/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_marvi_home, "logs", "tui_gateway_crash.log")
...
try:
    from runtime_support.banner import prefetch_update_check
...
try:
    _slash_timeout = float(os.environ.get("MARVI_MESSAGING_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("MARVI_MESSAGING_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("MARVI_MESSAGING_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("marvi_gateway_runtime_session_record", default=None)
)
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Marvi' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Marvi-managed CLIs (browser-use, uvx, uv) even when the parent
    ...
    try:
        ...
        managed_bin = str(Path(get_marvi_home()) / "bin")
    except Exception:
        pass
...
class _SlashWorker:
    """Persistent MarviCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Marvi Messaging → needs provider credentials.
        ...
        # gateway's launch MARVI_MESSAGING_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["MARVI_MESSAGING_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            marvi_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"MARVI_MESSAGING_HOME": str(profile_home)} if profile_home else None,
        )
        # Prepend the Marvi venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Marvi-managed CLIs
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from runtime_support.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from runtime_support.lifecycle import invoke_hook
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `marvi --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("MARVI_MESSAGING_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `marvi --tui`, so it must NOT count as dead here (doing
...
def _reap_idle_sessions() -> None:
    ...
    try:
        from runtime_support.mem_trim import trim_memory
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from runtime_support.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from runtime_support.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its MARVI_MESSAGING_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_marvi_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s MARVI_MESSAGING_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_marvi_home``).
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from runtime_support.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``marvi dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("MARVI_MESSAGING_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `marvi` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # MARVI_MESSAGING_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_marvi_home_override(profile_home)
                try:
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``marvi`` / ``marvi --tui`` / CLI) is started from a
    ...
    try:
        ...
        from marvi_state import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from marvi_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``runtime_support.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg_raw() -> dict:
    ...
    try:
        ...
        # launch profile's _marvi_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_marvi_home_override()
        home = override if isinstance(override, str) and override else _marvi_home
...
def _load_cfg() -> dict:
    ...
    ``runtime_support.config.load_config_readonly`` — managed-scope overlay and
...
def _apply_managed(cfg: dict) -> dict:
    ...
    runtime_support.config.load_config, so without this a managed skin / reasoning_effort
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read MARVI_MESSAGING_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # MARVI_MESSAGING_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["MARVI_MESSAGING_GATEWAY_SESSION"] = "1"
    os.environ["MARVI_MESSAGING_EXEC_ASK"] = "1"
...
def resolve_skin() -> dict:
    try:
        from runtime_support.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``marvi config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_marvi_home_override()
    return Path(override if isinstance(override, str) and override else _marvi_home)
...
def _ensure_skin_watcher() -> None:
    ...
    Marvi activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="marvi-change-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("MARVI_MESSAGING_MODEL", "")
        or os.environ.get("MARVI_MESSAGING_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from runtime_support.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``MARVI_MESSAGING_DESKTOP=1`` and ``MARVI_MESSAGING_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``MARVI_MESSAGING_DESKTOP_TERMINAL=1`` → "tui"
        (``marvi --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``marvi --tui``.)
    """
    if is_truthy_value(os.environ.get("MARVI_MESSAGING_DESKTOP")) and not is_truthy_value(
        os.environ.get("MARVI_MESSAGING_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads MARVI_MESSAGING_MODEL /
    MARVI_MESSAGING_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`marvi --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads MARVI_MESSAGING_MODEL /
    # MARVI_MESSAGING_INFERENCE_MODEL, which `marvi --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("MARVI_MESSAGING_TUI_PROVIDER", "").strip()
    ...
    try:
        from runtime_support.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("MARVI_MESSAGING_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
from marvi_state import _BARE_BILLING_PROVIDERS
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from runtime_support.runtime_provider import (
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind MARVI_MESSAGING_HOME to the session's profile so load_soul_md() and
    ...
    profile_home = session.get("profile_home")
    home_token = (
        set_marvi_home_override(profile_home) if profile_home else None
    )
    try:
...
def _load_approval_mode() -> str:
    ...
    canonical ``runtime_support.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`marvi_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("MARVI_MESSAGING_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_MARVI_MESSAGING_CORE_TOOLS`` — every other
    ...
    paths run with ``MARVI_MESSAGING_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the Marvi desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("MARVI_MESSAGING_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Marvi): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `marvi --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] MARVI_MESSAGING_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from runtime_support.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown MARVI_MESSAGING_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in MARVI_MESSAGING_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid MARVI_MESSAGING_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _MARVI_MESSAGING_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off marvi-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid MARVI_MESSAGING_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    cfg = None
    try:
        from runtime_support.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (MARVI_MESSAGING_MODEL /
    # MARVI_MESSAGING_INFERENCE_MODEL / MARVI_MESSAGING_TUI_PROVIDER / MARVI_MESSAGING_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `marvi --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # MARVI_MESSAGING_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("MARVI_MESSAGING_DEV_CREDITS")):
        try:
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from runtime_support.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from runtime_support import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from runtime_support.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from runtime_support.banner import get_update_result
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Marvi window, and answers
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from runtime_support.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via runtime_support.personality (single owner)."""
    from runtime_support.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as runtime_support.personality.resolve_personality — (name,
    ...
    """
    from runtime_support.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to runtime_support.personality (single owner).
    """
...
def _build_plugin_world_context(profile_home: str | Path | None = None) -> str:
    ...
    try:
        if profile_home is not None:
            home_token = set_marvi_home_override(str(profile_home))
        from runtime_support.plugins import build_plugin_context_blocks, discover_plugins
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("MARVI_MESSAGING_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("MARVI_MESSAGING_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``MarviCLI.__init__`` and ``gateway/run.py``:
...
def _resolve_runtime_with_fallback(
    ...
    """
    from runtime_support.auth import AuthError
    from runtime_support.runtime_provider import resolve_runtime_provider
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from runtime_support.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses runtime_support.mcp_startup; TUI stdio keeps
    ...
    cfg = _load_cfg()
    from runtime_support.config import resolve_ephemeral_system_prompt_from_config
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("MARVI_MESSAGING_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("MARVI_MESSAGING_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("MARVI_MESSAGING_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("MARVI_MESSAGING_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped MARVI_MESSAGING_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / MARVI_MESSAGING_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The MARVI_MESSAGING_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_marvi_home)
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Marvi has to support both the legacy 8-row petdex atlas and the current
...
def _pet_gen_root():
    ...
    root = get_marvi_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("MARVI_MESSAGING_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
# (agent/billing_view.py + runtime_support/nous_billing.py) — same as /topup.


def _serialize_billing_error(exc) -> dict:
    """Map a BillingError into the result.error envelope the TUI branches on."""
    from runtime_support.nous_billing import (
...
#
# Layout:  $MARVI_MESSAGING_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_marvi_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _maybe_fire_tui_loop_tick(sid: str, session: dict) -> None:
    ...
    """
    try:
        from runtime_support.loops import LoopManager, goal_blocks_loop_tick
    except Exception:
        return
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=MARVI_MESSAGING_SESSION_KEY`` (see
    ...
    # DB when MARVI_MESSAGING_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``MARVI_MESSAGING_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn MARVI_MESSAGING_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_marvi_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from runtime_support.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from runtime_support.voice import is_continuous_active
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: runtime_support.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific MARVI_MESSAGING_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile MARVI_MESSAGING_HOME
    override, so ``get_marvi_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _marvi_home
    return base / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile MARVI_MESSAGING_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from runtime_support.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from runtime_support.models import resolve_fast_mode_overrides
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("MARVI_MESSAGING_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["MARVI_MESSAGING_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("MARVI_MESSAGING_YOLO_MODE", None)
                    nv = "0"
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through runtime_support.personality (single owner) and never
                # touches the user-owned global system prompt.
                from runtime_support.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under MARVI_MESSAGING_HOME (~/.marvi by
    ...
    marvi_home = os.path.realpath(str(get_marvi_home()))

    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == marvi_home
        or real.startswith(marvi_home + os.sep)
    )
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    MARVI_MESSAGING_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: MARVI_MESSAGING_HOME itself and the dirs in
    ...
    marvi_home = os.path.normcase(os.path.realpath(str(get_marvi_home())))
    return real in _non_workspace_dirs() or real == marvi_home
...
def _discover_repos_payload(
    ...
    repos even with zero marvi sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (marvi home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Marvi sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Marvi
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `marvi` is interactive — use `/marvi chat -q …` or run `marvi` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`marvi setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`marvi gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`marvi sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`marvi config edit` needs $EDITOR in a real terminal"
    return None
...
def _model_picker_context(agent):
    ...
    from runtime_support.inventory import load_picker_context
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Marvi" if role == "assistant" else role.title()
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("MARVI_MESSAGING_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("MARVI_MESSAGING_VOICE_TTS", "").strip() == "1"


def _any_session_running() -> bool:
    """True while any session's agent turn is in flight.

    Registered as the voice busy-probe (``runtime_support.voice.set_voice_busy_probe``)
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["MARVI_MESSAGING_VOICE"] = "0"
                    os.environ["MARVI_MESSAGING_VOICE_TTS"] = "0"
                    try:
                        from runtime_support.voice import stop_continuous
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via runtime_support.voice.speak_text with spoken barge-in.
    ...
    """
    from runtime_support.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["MARVI_MESSAGING_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["MARVI_MESSAGING_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from runtime_support.voice import start_continuous
            ...
            # are tolerated.
            try:
                from runtime_support.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import runtime_support.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from runtime_support.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from runtime_support.browser_connect import DEFAULT_BROWSER_CDP_URL
