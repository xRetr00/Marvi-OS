...
def _(rid, params: dict) -> dict:
    ...
    def _profile_mention_items(prefix: str) -> list[dict]:
        ...
        under the 'marvi' alias when no real profile claims that name.
        """
        out: list[dict] = []
        try:
            ...
            if "marvi".startswith(prefix.lower()) and "marvi" not in seen:
                out.append(
                    {
                        "text": "@marvi",
                        "display": "@marvi",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from runtime_support.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        from runtime_support.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from runtime_support.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `marvi model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.marvi/.env via the unified credential lifecycle
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from runtime_support.auth import PROVIDER_REGISTRY, clear_provider_auth
