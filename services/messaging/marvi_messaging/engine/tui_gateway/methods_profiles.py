...
handlers delegate to the same `runtime_support.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Marvi profiles (name, path, model, description, skill count).
...
def _(rid, params: dict) -> dict:
    ...
    ``marvi setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (runtime_support.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local MARVI_MESSAGING_HOME override — the same
        ...
        try:
            ...
            token = set_marvi_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from runtime_support.web_routers.profiles import _write_profile_model
        ...
        try:
            from runtime_support.config import load_config_readonly, read_user_config_raw
            from runtime_support.web_routers.profiles import _write_profile_model
            from marvi_constants import (
                reset_marvi_home_override,
                set_marvi_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the MARVI_MESSAGING_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from runtime_support.skills_config import get_disabled_skills
            ...
            # Toolsets: the same filtered universe the `marvi tools`
            ...
            # composites (marvi-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from runtime_support.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `marvi tools` / Settings is where
            ...
            description = ""
            try:
                from runtime_support.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from runtime_support.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from runtime_support.skills_config import save_disabled_skills
