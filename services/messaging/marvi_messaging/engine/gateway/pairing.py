...
PAIRING_DIR = get_marvi_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``runtime_support.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from runtime_support.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{MARVI_MESSAGING_HOME}/pairing`` while newer code/docs may
    write ``{MARVI_MESSAGING_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own MARVI_MESSAGING_HOME using the same legacy/consolidated layout rules
    as ``marvi -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current MARVI_MESSAGING_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp MARVI_MESSAGING_HOME
        ...
        if profile:
            ...
            # ``marvi -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `marvi` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> marvi pairing approve ...` as root, "
                    "re-run with `docker exec -u marvi <container> ...` and "
                    "chown the existing file to the marvi user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``marvi
