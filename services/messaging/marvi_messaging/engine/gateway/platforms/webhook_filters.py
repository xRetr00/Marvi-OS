...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.marvi to the active profile home."""
    ...
    if raw == "~/.marvi":
        return marvi_home
    if raw.startswith("~/.marvi/"):
        return marvi_home / raw.removeprefix("~/.marvi/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return marvi_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under MARVI_MESSAGING_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from marvi_constants import get_marvi_home

    scripts_root = (get_marvi_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.marvi" or raw_text.startswith("~/.marvi/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__marvi_ignore__") is True
        ):
            return False, None
