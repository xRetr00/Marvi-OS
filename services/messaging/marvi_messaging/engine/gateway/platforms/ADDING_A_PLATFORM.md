...
There are two ways to add a platform to the Marvi gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.marvi/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Marvi code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `marvi gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `runtime_support/config.py` so the setup
...
Checklist for integrating a platform directly into the Marvi core.
...
And add it to the `marvi-gateway` composite:

```python
"marvi-gateway": {
    "includes": [..., "marvi-your-platform"]
}
```
...
## 12. Status Display (`runtime_support/status.py`)
...
## 13. Gateway Setup Wizard (`runtime_support/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ runtime_support/ toolsets.py \
  --include="*.py" -l | sort -u
