...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Marvi Messaging*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to MARVI_MESSAGING_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/marvi), this function
    mirrors the bridge source to a writable MARVI_MESSAGING_HOME location and returns that
    ...
    # Try MARVI_MESSAGING_HOME location first
    marvi_home = get_marvi_home()
    marvi_home_bridge = marvi_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to MARVI_MESSAGING_HOME if needed
    if marvi_home_bridge.exists():
        return marvi_home_bridge

    # Mirror the bridge source to MARVI_MESSAGING_HOME
    try:
        marvi_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            marvi_home_bridge,
            dirs_exist_ok=False,
