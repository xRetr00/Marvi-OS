...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        marvi_home_str = os.environ.get("MARVI_MESSAGING_HOME")
        if marvi_home_str:
            takeover_path = Path(marvi_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(marvi_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded marvi-messaging but never re-ran ``marvi setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../marvi-gateway.service"
    ...
    # that's the common case for marvi.
    ...
    # constant in runtime_support/gateway.py.
    headroom = 30.0
