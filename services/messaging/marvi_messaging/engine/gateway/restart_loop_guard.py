...
Defenses 1 and 2 (the ``_MARVI_MESSAGING_GATEWAY`` guard on ``marvi gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.marvi.gateway")``,
...
State lives in ``<MARVI_MESSAGING_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_marvi_home() / "gateway" / "restart_loop.json"
