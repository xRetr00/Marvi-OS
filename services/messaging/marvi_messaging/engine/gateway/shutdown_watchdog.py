...
2. An event-loop heartbeat file at ``<MARVI_MESSAGING_HOME>/state/gateway.heartbeat`` so
...
def _process_marvi_home() -> Path:
    """MARVI_MESSAGING_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<MARVI_MESSAGING_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_marvi_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from marvi_logging import drain_log_queue
