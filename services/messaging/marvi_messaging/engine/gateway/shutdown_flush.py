...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<marvi_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active MARVI_MESSAGING_HOME."""
    from marvi_constants import get_marvi_home

    flush_dir = get_marvi_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<marvi_home>/pending_messages/``), so a runtime cap rotation no
