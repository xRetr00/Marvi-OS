...
class PlatformEntry:
    ...
    # status displays (``marvi setup``, ``marvi status``, the dashboard
    ...
    # Env vars this platform needs (for ``marvi setup`` display).
    ...
    # built-ins).  Used by ``marvi gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved MARVI_MESSAGING_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `marvi`
        # invocation -- including plain `marvi chat`, which never touches any
        ...
        # `marvi setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``marvi setup``/``gateway status``, channel
