...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{MARVI_MESSAGING_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``MARVI_MESSAGING_HOME`` is a **durable** store — on Marvi
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``marvi gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting MARVI_MESSAGING_HOME."""
    base = home if home is not None else get_marvi_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable MARVI_MESSAGING_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (MARVI_MESSAGING_HOME is a
    durable Fly volume on Marvi Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    MARVI_MESSAGING_HOME volume, or an expired same-epoch orphan (#85433), is
