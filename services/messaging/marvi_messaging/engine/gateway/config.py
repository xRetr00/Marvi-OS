...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from runtime_support.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (runtime_support/web_server.py) so
...
class PlatformConfig:
    ...
    # "Marvi is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_marvi_home() / "sessions")
    ...
    # gateway behaves exactly as before — single MARVI_MESSAGING_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``marvi config set gateway.multiplex_profiles true``.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.marvi/config.yaml (primary user-facing config)
    3. ~/.marvi/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_marvi_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through runtime_support.config.load_config, so without this a managed
            ...
            # (e.g. via `marvi config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `marvi config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``marvi config set gateway.streaming.*``
