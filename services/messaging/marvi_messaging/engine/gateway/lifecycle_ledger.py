...
``<MARVI_MESSAGING_HOME>/state/gateway.lifecycle.json``:
...
def _process_marvi_home() -> Path:
    """MARVI_MESSAGING_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<MARVI_MESSAGING_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``runtime_support.container_boot`` to annotate ``container-boot.log``.
    """
    try:
