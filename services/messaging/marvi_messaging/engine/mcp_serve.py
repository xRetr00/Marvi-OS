"""
Marvi MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Marvi-specific extra)

Usage:
    marvi mcp serve
    marvi mcp serve --verbose
...
logger = logging.getLogger("marvi.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using MARVI_MESSAGING_HOME."""
    try:
        from marvi_constants import get_marvi_home
        return get_marvi_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("MARVI_MESSAGING_HOME", Path.home() / ".marvi")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from marvi_constants import get_marvi_home
        directory_file = get_marvi_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("MARVI_MESSAGING_HOME", Path.home() / ".marvi")
...
class EventBridge:
    ...
    This is the Marvi equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline(self) -> None:
        ...
        try:
            from marvi_constants import get_marvi_home
            db_file = get_marvi_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("MARVI_MESSAGING_HOME", Path.home() / ".marvi")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Marvi MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "marvi",
        instructions=(
            "Marvi Messaging messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Marvi MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
