...
from runtime_support.config import get_env_value, load_config
...
# coverage category (runtime_support.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `marvi tools` picker scope its entitlement gate to the selected backend, so a
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Marvi-managed Node dirs (Windows installer / POSIX $MARVI_MESSAGING_HOME/node)
        ...
        managed_path = with_marvi_node_path().get("PATH", "")
        if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("MARVI_MESSAGING_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `marvi model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("MARVI_MESSAGING_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("MARVI_MESSAGING_LOCAL_STT_COMMAND"))
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`marvi tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``marvi tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``marvi model``'s Nous
    login, this:
