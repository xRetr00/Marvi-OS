"""
Dump command for marvi CLI.

Outputs a compact, plain-text summary of the user's Marvi setup
...
from runtime_support.config import get_marvi_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.marvi/.env.
    ...
    exports. ``marvi debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.marvi_build_sha`` by the Dockerfile's
    ``MARVI_MESSAGING_GIT_SHA`` build-arg (see ``runtime_support/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``marvi dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from runtime_support.gateway import get_gateway_runtime_snapshot
...
def _count_skills(marvi_home: Path) -> int:
...
def _cron_summary(marvi_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = marvi_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"marvi_home:      {display_marvi_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.marvi/.env: a managed
        ...
        # A credential added via `marvi auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `marvi auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["marvi-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(marvi_home)}")
    lines.append(f"  skills:             {_count_skills(marvi_home)}")
