...
_TASK_NAME_DEFAULT = "Marvi_Gateway"
_TASK_DESCRIPTION = "Marvi Messaging Gateway - Messaging Platform Integration"
...
def _preserve_marvi_home_path(path: str | Path) -> str:
    """Render Marvi-owned paths under the configured MARVI_MESSAGING_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\marvi`` as a symlink/junction to
    ...
    path lives under MARVI_MESSAGING_HOME.
    """
    candidate = Path(path)
    try:
        from runtime_support.config import get_marvi_home

        home = Path(get_marvi_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Marvi profile."""
    from runtime_support.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "runtime_support.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("MARVI_MESSAGING_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("MARVI_MESSAGING_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("MARVI_MESSAGING_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["MARVI_MESSAGING_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["MARVI_MESSAGING_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["MARVI_MESSAGING_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("MARVI_MESSAGING_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("MARVI_MESSAGING_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("MARVI_MESSAGING_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Marvi_Gateway``
    Named profile X: ``Marvi_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during runtime_support boot.
    from runtime_support.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\marvi\\gateway-service\\<task_name>.cmd``
    (or ``<MARVI_MESSAGING_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Marvi installs stay self-contained).
    """
    _assert_windows()
    from runtime_support.config import get_marvi_home

    script_dir = Path(get_marvi_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``MARVI_MESSAGING_HOME`` whenever it
    ...
    source checkout only if ``MARVI_MESSAGING_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports MARVI_MESSAGING_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m runtime_support.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``marvi gateway run``.
    ...
    lines.append(f'set "MARVI_MESSAGING_HOME={marvi_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "MARVI_MESSAGING_GATEWAY_DETACHED=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports marvi_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_marvi_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_marvi_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_marvi_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [python_exe_path, "-m", "runtime_support.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_marvi_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_marvi_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('MARVI_MESSAGING_HOME')}) = {_quote_vbs_string(marvi_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('MARVI_MESSAGING_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_marvi_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_marvi_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    marvi_home = str(Path(get_marvi_home()))
    profile_arg = _profile_arg(marvi_home)

    content = _build_gateway_cmd_script(python_path, working_dir, marvi_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, marvi_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Marvi builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_marvi_home_path(get_python_path())
    )
    project_root = _preserve_marvi_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    marvi_home = str(Path(get_marvi_home()))
    profile_arg = _profile_arg(marvi_home)

    argv = [python_exe, "-m", "runtime_support.main"]
    if profile_arg:
        argv.extend(profile_arg.split())
    argv.extend(["gateway", "run"])

    env_overlay = {
        "MARVI_MESSAGING_HOME": marvi_home,
        "PYTHONIOENCODING": "utf-8",
        "MARVI_MESSAGING_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_marvi_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_marvi_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (MARVI_MESSAGING_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m runtime_support.main
    ...
    from runtime_support.config import get_marvi_home
    from runtime_support.gateway import PROJECT_ROOT
    ...
    try:
        marvi_home = str(Path(get_marvi_home()).resolve())
    except Exception:
        marvi_home = ""
    ...
    if marvi_home:
        env_overlay["MARVI_MESSAGING_HOME"] = marvi_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m runtime_support.main gateway run`` directly — NOT
    ...
    log_dir = Path(get_marvi_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("MARVI_MESSAGING_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("MARVI_MESSAGING_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from runtime_support.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `marvi -p <profile> gateway install` must be safe.
    ...
    from runtime_support.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"marvi {profile_arg} gateway start" if profile_arg else "marvi gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: marvi gateway start")
        return
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                ...
                if start_now:
                    ...
                    print("  Approve the Windows UAC prompt, then run: marvi gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: marvi gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from runtime_support.config import get_marvi_home
        print(f"    type {Path(get_marvi_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_marvi_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from runtime_support.config import get_marvi_home

    marvi_home = Path(get_marvi_home())
    print()
    print("Next steps:")
    print("  marvi gateway status                      # Check status")
    print(f"  type {marvi_home}\\logs\\gateway.log       # View logs")
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: marvi gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: marvi gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `marvi gateway status`."
        )
