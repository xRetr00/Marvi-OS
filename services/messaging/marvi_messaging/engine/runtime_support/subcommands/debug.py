...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Marvi Messaging. Use 'marvi debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    marvi debug share              Upload debug report (asks for confirmation)
    marvi debug share --yes        Skip confirmation (for scripts/CI)
    marvi debug share --lines 500  Include more log lines
    marvi debug share --expire 30  Keep paste for 30 days
    marvi debug share --local      Print report locally (no upload)
    marvi debug share --no-redact  Disable upload-time secret redaction
    marvi debug share --nous       Upload to Nous-internal storage (private)
    marvi debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'marvi debug share'",
    )
