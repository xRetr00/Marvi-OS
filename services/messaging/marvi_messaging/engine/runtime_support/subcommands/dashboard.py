"""``marvi dashboard`` / ``marvi serve`` subcommand parsers.
...
``runtime_support/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `marvi dashboard` / `marvi serve` cmdlines and SIGTERM them directly —
    # the same path `marvi update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Marvi web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Marvi web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Marvi Messaging web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Marvi desktop app shells (<= 0.15.x) spawn the
    # backend as `marvi dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Marvi couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Marvi Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Marvi backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Marvi backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports MARVI_MESSAGING_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `marvi dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.marvi/.env.
    # Nested subparser so bare `marvi dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "MARVI_MESSAGING_DASHBOARD_OAUTH_CLIENT_ID into ~/.marvi/.env, and prints "
            "how to engage the login gate. Requires being logged in (marvi setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://marvi.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via MARVI_MESSAGING_DASHBOARD_PORTAL_URL. Mainly for "
