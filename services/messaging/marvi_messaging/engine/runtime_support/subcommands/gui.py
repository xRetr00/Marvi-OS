...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Marvi Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any marvi CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--marvi-root",
        help="Override the Marvi source root used by Desktop (sets MARVI_MESSAGING_DESKTOP_MARVI_MESSAGING_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets MARVI_MESSAGING_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
