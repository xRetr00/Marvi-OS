"""``marvi sync`` subcommand parser — Skill Sync.

Cloned from ``runtime_support/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    marvi sync status                 show gate/opt-in/head state
    marvi sync pull                   pull and materialize opted-in skills
    marvi sync push                   push opted-in skills
    marvi sync now                    reconcile: pull then push
    marvi sync enable <skill>         opt a skill into sync
    marvi sync disable <skill>        opt a skill out of sync
    marvi sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    marvi sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  marvi sync status            what is synced, and from where\n"
            "  marvi sync enable my-skill   include a skill in your sync\n"
            "  marvi sync now               pull, then push\n"
            "  marvi sync propose my-skill  share a skill with your team\n"
        ),
