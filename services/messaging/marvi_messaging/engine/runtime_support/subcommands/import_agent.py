"""``marvi import-agent`` subcommand parser.

Follows the ``marvi claw`` pattern (see ``runtime_support/subcommands/claw.py``):
parser building lives here, the handler is injected to avoid importing
``main``, and the import logic itself lives in ``runtime_support/agent_import.py``.
"""
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into Marvi",
        description=(
            "One-command import of another coding agent's setup into Marvi. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Marvi equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'marvi setup' for those."
        ),
    )
    parser.add_argument(
        "agent",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing Marvi items on name conflicts (default: skip)",
    )
    parser.add_argument(
