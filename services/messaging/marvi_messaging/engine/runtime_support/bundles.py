"""Implementation of the ``marvi bundles`` CLI subcommand.

Mirrors the structure of ``runtime_support/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `marvi bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  marvi bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``marvi bundles`` argparse tree.

    Called from ``runtime_support/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `marvi bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``marvi bundles <subcommand>`` to the right handler."""
