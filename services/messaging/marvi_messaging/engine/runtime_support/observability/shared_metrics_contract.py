"""Bounded product contract for the first Marvi shared-metrics slice."""
...
SCHEMA_KEY = "marvi.metrics.schema_version"
SCHEMA_VERSION = "marvi.metrics.event.v2"
MODEL_CALL_SCOPE = "marvi.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "marvi.task_run"
TOOL_CALL_SCOPE = "marvi.tool_call"
CLIENT_ACTIVE_MARK = "marvi.client.active"
TOOL_APPROVAL_MARK = "marvi.tool_approval"
SKILL_LIFECYCLE_MARK = "marvi.skill.lifecycle"
SKILL_LOAD_MARK = "marvi.skill.load"
SUBSCRIBER_NAME = "marvi.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "marvi.client.active"
LEGACY_MODEL_CALL_METRIC = "marvi.model_call.count"
MODEL_ROUTE_METRIC = "marvi.model_route.count"
TASK_STARTED_METRIC = "marvi.task_run.started"
TASK_FINISHED_METRIC = "marvi.task_run.finished"
TOOL_CALL_METRIC = "marvi.tool_call.count"
TOOL_APPROVAL_METRIC = "marvi.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "marvi.skill.lifecycle.count"
SKILL_LOAD_METRIC = "marvi.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Marvi installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(marvi_version or "").strip()
    ...
    return {
        ...
        "marvi_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("marvi_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Marvi logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("marvi.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from runtime_support.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Marvi terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Marvi registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "marvi-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Marvi tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Marvi."""
