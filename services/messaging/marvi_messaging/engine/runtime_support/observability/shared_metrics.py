"""Durable aggregation and local export for Marvi shared metrics."""
...
from runtime_support.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "marvi.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_marvi_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "marvi_version": row["marvi_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Marvi processes.
            with write_txn(connection):
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "marvi_version": period_row["marvi_version"],
