"""Pydantic request/response models for the Marvi dashboard web server.

Extracted verbatim from ``runtime_support/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from runtime_support.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``marvi model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `marvi import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`marvi -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the MARVI_MESSAGING_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under MARVI_MESSAGING_HOME.
    output: str = ""
