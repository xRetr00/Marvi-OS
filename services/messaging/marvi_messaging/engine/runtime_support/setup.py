...
from marvi_constants import get_optional_skills_dir
...
# display_marvi_home imported lazily at call sites (stale-module safety during marvi update)
...
from runtime_support.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from runtime_support.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Marvi using environment variables or config commands:")
    print_info("  marvi config set model.provider custom")
    print_info("  marvi config set model.base_url http://localhost:8080/v1")
    print_info("  marvi config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'marvi setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``MARVI_MESSAGING_NONINTERACTIVE=1`` (see ``runtime_support/web_server.py``). In that
    ...
    """
    return os.environ.get("MARVI_MESSAGING_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``MARVI_MESSAGING_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from runtime_support.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'marvi setup')")


def _print_setup_summary(config: dict, marvi_home):
    ...
    try:
        from runtime_support.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Marvi cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    marvi model            (pick any provider/model)")
        print_info("    marvi setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'marvi setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from runtime_support.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `marvi tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from runtime_support.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        tool_status.append(("Text-to-Speech (removed local provider)", False, "run 'marvi setup tts' and choose PocketTTS"))
        ...
        if pockettts_ok:
            ...
            tool_status.append(("Text-to-Speech (PocketTTS — not installed)", False, "run 'marvi setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'marvi tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'marvi setup terminal'"))
    ...
    # Spotify (OAuth via marvi auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'marvi setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {marvi_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('marvi setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('marvi setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('marvi setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('marvi setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('marvi setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('marvi config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('marvi config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('marvi config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('marvi', Colors.GREEN)}              Start chatting")
    print(f"   {color('marvi gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('marvi doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `marvi tools` and `marvi setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``marvi model``)
    ...
    provider added to ``marvi model`` is automatically available here.
    ...
    # Delegate to the shared marvi model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from runtime_support.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: marvi model")
    ...
    # on demand via `marvi auth add`, `marvi setup` vision, and
    # `marvi setup tts`. This keeps both quick and full setup thin.
...
def _install_parakeet_stt_deps() -> bool:
    ...
    try:
        ...
        print_info("Try manually: marvi tools post-setup parakeet_stt")
        return False
...
def _install_semantic_turn_deps() -> bool:
    ...
    try:
        ...
        print_info("Try manually: marvi tools post-setup semantic_turn")
        return False
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``marvi model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from runtime_support.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.marvi/.env."
            )
    ...
    if selected == "elevenlabs":
        ...
        # via the Marvi auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via marvi setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
        ...
        if selected == "pockettts":
            ...
            if pocket_cfg["device"] == "cuda":
                from runtime_support.tools_config import _ensure_cuda_torch
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'marvi setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `marvi setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `marvi setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `marvi egress setup` then `marvi egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `marvi egress setup`."
            )
        ...
        if use_managed_modal:
            ...
            try:
                ...
                from runtime_support.tools_config import _pip_install
        ...
        print_info("Requires the optional SDK: pip install 'marvi-messaging[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $MARVI_MESSAGING_HOME/bin is never on PATH, so a bare
            # which() misses the uv Marvi installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'marvi-messaging[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into MARVI_MESSAGING_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("MARVI_MESSAGING_MAX_ITERATIONS")
    ...
    print_info("  Run `marvi setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # MARVI_MESSAGING_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.marvi/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from runtime_support.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_marvi_home(Path(get_marvi_home()))
    except Exception:
        pass
...
def _profile_name_from_marvi_home(marvi_home) -> str | None:
    """Return the active profile name when MARVI_MESSAGING_HOME is a profile dir."""
    if marvi_home.parent.name == "profiles":
        return marvi_home.name
    return None
...
def _setup_bluebubbles():
    ...
    print_info("Connects Marvi to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from runtime_support.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  marvi config edit")
    print_info("   Open config in your editor:  marvi config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from runtime_support.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if not selected:
        print_info("No platforms selected. Run 'marvi setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     marvi config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `marvi import` or `marvi setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `marvi setup tools` and `marvi tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Marvi profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``runtime_support.auth`` — lists every supported
    ...
    """
    try:
        from runtime_support.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from runtime_support.config
    ...
    if section_key == "model":
        ...
        from runtime_support.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_marvi.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_marvi migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_marvi", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Marvi.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Marvi to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Marvi at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Marvi at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Marvi at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Marvi at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Marvi equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Marvi config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Marvi.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Marvi's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(marvi_home: Path) -> bool:
    ...
    print_info("Marvi can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: marvi claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=marvi_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: marvi claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Marvi configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Marvi config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Marvi (use marvi claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``marvi setup --portal`` and ``marvi portal``. This is the
    ...
    Marvi session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``marvi setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``marvi model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``marvi portal`` always offers a model picker,
    ...
    # setup calls, so `marvi portal` == quick setup's Nous step.
    try:
        from runtime_support.main import _model_flow_nous
        ...
        print_info("  You can retry later with `marvi portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `marvi portal`: %s", exc)
        print()
    ...
    print_info("  Run `marvi portal info` to inspect routing.")
    print_info("  Run `marvi` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      marvi setup           — full or quick (auto-detected)
      marvi setup model     — just model/provider
      marvi setup tts       — just text-to-speech
      marvi setup terminal  — just terminal backend
      marvi setup gateway   — just messaging platforms
      marvi setup tools     — just tool configuration
      marvi setup telemetry — just local shared metrics
      marvi setup agent     — just agent settings
    """
    from runtime_support.config import is_managed, managed_error
    if is_managed():
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, marvi_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Marvi configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'marvi setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(marvi_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Marvi?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, marvi_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, marvi_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {marvi_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'marvi config edit'")
    ...
    # Tune later with `marvi setup agent`.
    if not is_existing:
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from runtime_support.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    _print_setup_summary(config, marvi_home)


def _run_first_time_quick_setup(config: dict, marvi_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``marvi setup <section>``
    or switch providers with ``marvi model``.
    """
    from runtime_support.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'marvi setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `marvi import` from another machine).
        from runtime_support.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    marvi setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  marvi setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``marvi tools`` (which
    ...
    try:
        ...
        from runtime_support.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``marvi setup agent`` /
    ``marvi config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, marvi_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `marvi update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        marvi tools")
        print_info("  Seed skills:         marvi skills opt-in --sync")
        print_info("  Add MCP servers:     marvi mcp add")
        print_info("  Enable plugins:      marvi plugins")
        print_info("  Tune agent settings: marvi setup agent")
        print()
        _print_setup_summary(config, marvi_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, marvi_home)


def _blank_slate_walkthrough(config: dict, marvi_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`marvi update` runs from re-injecting them. Opt back in any")
            print_info("time with `marvi skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `marvi tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `marvi plugins list` / `marvi plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `marvi plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `marvi mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `marvi mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   marvi tools")
    print_info("  Seed skills:         marvi skills opt-in --sync")
    print_info("  Add MCP servers:     marvi mcp add")
    print_info("  Tune agent settings: marvi setup agent")
    print()
...
def _run_quick_setup(config: dict, marvi_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'marvi setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Marvi to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'marvi setup gateway'.")
