"""marvi webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    marvi webhook subscribe <name> [options]
    marvi webhook list
    marvi webhook remove <name>
    marvi webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.marvi/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _marvi_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: marvi gateway run
"""
...
def webhook_command(args):
    """Entry point for 'marvi webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: marvi webhook {subscribe|list|remove|test}")
        print("Run 'marvi webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (marvi gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: marvi webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from marvi webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (marvi gateway run)")
