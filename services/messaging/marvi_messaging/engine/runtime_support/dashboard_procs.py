"""Dashboard process-hygiene helpers — extracted from ``runtime_support/main.py``.
...
``_detect_concurrent_marvi_instances``) are lifted verbatim. References to
helpers that STAY in ``runtime_support.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``runtime_support.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``runtime_support.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``runtime_support.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``marvi dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``marvi update`` replaces files on disk, the running
    ...
    ``marvi-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Marvi Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``marvi serve`` as
    ...
    environment variable ``MARVI_MESSAGING_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`marvi serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "marvi.*dashboard"`) keeps us consistent
            # with `runtime_support.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``marvi dashboard`` / ``marvi serve`` processes.

    Called at the end of ``marvi update`` (default ``reason``) and also
    from ``marvi dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``marvi update`` path), a detected
    ``marvi-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``marvi-serve.service``) has its owning unit
    ...
    # When the Marvi Desktop Electron app spawns this dashboard as a
    # backend child, it sets MARVI_MESSAGING_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("MARVI_MESSAGING_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `marvi update` path) — `--stop` must
    ...
    #    remote backend (marvi serve) under Restart=on-failure never comes
...
def _detect_concurrent_marvi_instances(
    ...
    The Marvi Desktop Electron app spawns ``marvi.EXE`` as a backend child,
    so during ``marvi update`` the user-invoked process and the desktop's
    ...
    shims (``marvi.exe`` / ``marvi-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``marvi update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``marvi.exe``) is a separate process from the Python
    ...
    for shim in _m()._marvi_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated marvi.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``marvi update``
    ...
    #      marvi.exe sitting *under* a non-Marvi parent (e.g. a Marvi
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        marvi serve --host 127.0.0.1 --port 0
        marvi serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "marvi" not in cmd and "runtime_support" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (MARVI_MESSAGING_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("MARVI_MESSAGING_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``marvi serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``marvi serve`` on a Mac
...
def _marvi_home_dir() -> Path:
    """Resolved Marvi home (MARVI_MESSAGING_HOME override or ~/.marvi)."""
    override = os.environ.get("MARVI_MESSAGING_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "marviPath", "marviHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated MARVI_MESSAGING_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{marvi_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _marvi_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local marvi serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    lock_owned_pids_fn=None,
) -> dict[str, list]:
    """Kill leftover Desktop-local ``marvi serve`` backends with no parent.
    ...
    running under MARVI_MESSAGING_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never MARVI_MESSAGING_DESKTOP_CHILD_PID entries
