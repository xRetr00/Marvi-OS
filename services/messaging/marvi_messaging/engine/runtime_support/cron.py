...
# commands at execution time when ``_MARVI_MESSAGING_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: marvi gateway install", Colors.DIM))
    print(color("                    sudo marvi gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  marvi cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'marvi cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: marvi gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec marvi "
                        "marvi cron ...`). Fix ownership to match the "
        ...
        print("    marvi gateway install    # Install as a user service")
        print("    sudo marvi gateway install --system  # Linux servers: boot-time system service")
        print("    marvi gateway            # Or run in foreground")
...
def cron_notepad(args) -> int:
    """Handle ``marvi cron notepad <job_id> [get|set|delete|list]``.
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: marvi cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: marvi cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: marvi cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: marvi cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
