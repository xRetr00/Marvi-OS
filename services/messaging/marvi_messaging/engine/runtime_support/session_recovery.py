"""Offline, non-destructive recovery for a damaged Marvi session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``runtime_support.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``marvi_state._backup_db_file``.
    """
    from runtime_support.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Marvi process using this profile and retry. "
                "This includes the interactive `marvi` CLI session this "
                ...
                "Marvi process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `marvi` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from runtime_support.session_lost_and_found import (
