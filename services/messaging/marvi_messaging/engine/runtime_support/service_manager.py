...
the existing module-level functions in runtime_support.gateway and
runtime_support.gateway_windows directly. This protocol is a thin facade
...
``marvi gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``marvi gateway start/stop/restart``).
    """
    ...
    # `marvi gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged marvi
    ...
    detection always fails. Since every Marvi runtime call inside the
    container drops to marvi via ``s6-setuidgid``, that silent failure
...
# in ``runtime_support.gateway`` (systemd/launchd) and ``runtime_support.gateway_windows``
...
# active (set via the ``marvi -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in runtime_support.gateway.
    ...
    def start(self, name: str) -> None:
        from runtime_support.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from runtime_support.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from runtime_support.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from runtime_support.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in runtime_support.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from runtime_support.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from runtime_support.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from runtime_support.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from runtime_support.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``runtime_support.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `marvi profile create`
# runs inside the container (Phase 4). Static services (main-marvi, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's MARVI_MESSAGING_HOME blindly;
    derive the shared profile root from the current MARVI_MESSAGING_HOME and map the
    ...
    marvi_home = Path(os.environ.get("MARVI_MESSAGING_HOME", "/opt/data"))
    if marvi_home.parent.name == "profiles":
        root = marvi_home.parent.parent
    else:
        root = marvi_home
...
# UID/GID of the in-image ``marvi`` user. Hardcoded to match what
...
_MARVI_MESSAGING_UID = 10000
_MARVI_MESSAGING_GID = 10000


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create the ``supervise/`` and top-level ``event/`` skeleton
    inside a service directory, owned by the marvi user.
    ...
    (the ``marvi`` user — UID 10000 — running every Marvi runtime
    ...
    is dispatched as the marvi user.
    ...
    with marvi ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           marvi:marvi, 0755 (parent must already exist)
    ``svc_dir/event/``                     marvi:marvi, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 marvi:marvi, 0755
    ``svc_dir/supervise/event/``           marvi:marvi, 03730
    ``svc_dir/supervise/control``          marvi:marvi, 0660    (FIFO)
    ...
    slot's supervise/ was marvi-owned.
    ...
    def _chown_if_available(path: Path) -> None:
        ...
        if chown is None:
            return
        try:
            chown(path, _MARVI_MESSAGING_UID, _MARVI_MESSAGING_GID)
        except PermissionError:
            # Running as the marvi user already — ownership is effectively
    ...
    # when the parent slot's supervise/ is marvi-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``marvi -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`marvi profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-marvi, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources MARVI_MESSAGING_HOME (and any extra env) via with-contenv —
             so e.g. ``-e MARVI_MESSAGING_HOME=/data/marvi`` is honored at run
          ...
          4. Drops to the marvi user and exec's
             ``marvi -p <profile> gateway run`` (or just ``marvi
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``marvi gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root MARVI_MESSAGING_HOME profile" (the implicit profile that exists at
        the top of $MARVI_MESSAGING_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``runtime_support.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$MARVI_MESSAGING_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``MARVI_MESSAGING_HOME``,
        ...
        allocator in ``runtime_support.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export MARVI_MESSAGING_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's MARVI_MESSAGING_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``marvi gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-MARVI_MESSAGING_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The MARVI_MESSAGING_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own MARVI_MESSAGING_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "marvi gateway run --replace"
        else:
            gateway_cmd = f"marvi -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid marvi {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${MARVI_MESSAGING_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the MARVI_MESSAGING_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e MARVI_MESSAGING_HOME=/data/marvi``
        gets its logs under /data/marvi/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``marvi logs`` reads and what
        ...
        return (
            ...
            f': "${{MARVI_MESSAGING_HOME:=/opt/data}}"\n'
            f'log_dir="$MARVI_MESSAGING_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as marvi when
            # this script starts as root. Never chown or unlink marvi-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is marvi-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded marvi-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid marvi mkdir -p "$log_dir"\n'
            f'  s6-setuidgid marvi rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid marvi s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``marvi gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``marvi -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with marvi ownership
            ...
            # the marvi user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is marvi-owned (see ``_seed_supervise_skeleton``), and on
