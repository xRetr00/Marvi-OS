"""
Top-level argparse construction for the marvi CLI.
...
# argparse runs (it sets ``MARVI_MESSAGING_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``runtime_support.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    marvi                        Start interactive chat
    marvi chat -q "Hello"        Single query mode
    marvi --tui                  Launch the modern TUI (or set display.interface: tui)
    marvi --cli                  Force the classic REPL (overrides display.interface: tui)
    marvi -c                     Resume the most recent session
    marvi -c "my project"        Resume a session by name (latest in lineage)
    marvi --resume <session_id>  Resume a specific session by ID
    marvi --resume latest        Resume the most recent session (same as -c)
    marvi --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    marvi setup                  Run setup wizard
    marvi logout                 Clear stored authentication
    marvi auth add <provider>    Add a pooled credential
    marvi auth list              List pooled credentials
    marvi auth remove <p> <t>    Remove pooled credential by index, id, or label
    marvi auth reset <provider>  Clear exhaustion status for a provider
    marvi model                  Select default model
    marvi fallback [list]        Show fallback provider chain
    marvi fallback add           Add a fallback provider (same picker as `marvi model`)
    marvi fallback remove        Remove a fallback provider from the chain
    marvi config                 View configuration
    marvi config edit            Edit config in $EDITOR
    marvi config set model gpt-4 Set a config value
    marvi gateway                Run messaging gateway
    marvi -s marvi-messaging-dev,github-auth
    marvi -w                     Start in isolated git worktree
    marvi gateway install        Install gateway background service
    marvi sessions list          List past sessions
    marvi sessions browse        Interactive session picker
    marvi sessions rename ID T   Rename/title a session
    marvi logs                   View agent.log (last 50 lines)
    marvi logs -f                Follow agent.log in real time
    marvi logs errors            View errors.log
    marvi logs --since 1h        Lines from the last hour
    marvi debug share             Upload debug report for support
    marvi console                Open the safe Marvi command console
    marvi update                 Update to latest version
    marvi dashboard              Start web UI dashboard (port 9119)
    marvi dashboard --stop       Stop running dashboard processes
    marvi dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="marvi",
        description="Marvi Messaging - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `marvi chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via MARVI_MESSAGING_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `marvi setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to MARVI_MESSAGING_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.marvi/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Marvi Messaging",
    )
    ...
    # top-level parser: when the user writes `marvi -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/runtime_support/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also MARVI_MESSAGING_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.marvi/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Marvi itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
