"""
Single source of truth for provider identity in Marvi Messaging.
...
2. **Marvi overlays** — transport type, auth patterns, aggregator flags,
...
# -- Marvi overlay ----------------------------------------------------------
# Marvi-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class MarviOverlay:
    """Marvi-specific provider metadata layered on top of models.dev."""
...
MARVI_MESSAGING_OVERLAYS: Dict[str, MarviOverlay] = {
    "moa": MarviOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": MarviOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": MarviOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": MarviOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": MarviOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": MarviOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="MARVI_MESSAGING_QWEN_BASE_URL",
    ),
    "lmstudio": MarviOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": MarviOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": MarviOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": MarviOverlay(
    ...
    "kimi-for-coding": MarviOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": MarviOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": MarviOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": MarviOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": MarviOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": MarviOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": MarviOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": MarviOverlay(
        transport="openai_chat",
    ...
    "opencode": MarviOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": MarviOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": MarviOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": MarviOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "marvi", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Marvi overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Marvi overlay
    ...
    overlay = MARVI_MESSAGING_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + marvi extra
    ...
    if overlay is not None:
        # Marvi-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Marvi overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in MARVI_MESSAGING_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Marvi provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from runtime_support.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
