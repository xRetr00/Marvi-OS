"""
Configuration management for Marvi Messaging.

Config files are stored in ~/.marvi/ for easy access:
- ~/.marvi/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.marvi/.env         - API keys and secrets

This module provides:
- marvi config          - Show current configuration
- marvi config edit     - Open config in editor
- marvi config get      - Print a resolved configuration value
- marvi config set      - Set a specific value
- marvi config unset    - Remove a user configuration value
- marvi config wizard   - Re-run setup wizard
"""
...
from runtime_support.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``marvi config set`` (which rewrites
    ...
    ``config.yaml`` in place: marvi never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.marvi/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``marvi logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  marvi config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Marvi makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Marvi itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``marvi update``, the TUI build.
...
#   on every plugin install / ``marvi update``.
...
# * ``MARVI_MESSAGING_HOME`` / ``MARVI_MESSAGING_PROFILE`` / ``MARVI_MESSAGING_CONFIG`` /
#   ``MARVI_MESSAGING_ENV`` — Marvi runtime location flags. Writing these into
...
#
# IMPORTANT: ``MARVI_MESSAGING_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (MARVI_MESSAGING_LANGFUSE_PUBLIC_KEY,
# MARVI_MESSAGING_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Marvi runtime location — never via dashboard env writer.
    # NOT a MARVI_MESSAGING_* blanket: integration credentials (MARVI_MESSAGING_GEMINI_*,
    # MARVI_MESSAGING_LANGFUSE_*, MARVI_MESSAGING_SPOTIFY_*, ...) ARE allowed.
    "MARVI_MESSAGING_HOME", "MARVI_MESSAGING_PROFILE", "MARVI_MESSAGING_CONFIG", "MARVI_MESSAGING_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Marvi runtime location "
            "(MARVI_MESSAGING_HOME, MARVI_MESSAGING_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # MARVI_MESSAGING_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `marvi plugins enable
    # observability/langfuse` or `marvi tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "MARVI_MESSAGING_LANGFUSE_ENV",
    "MARVI_MESSAGING_LANGFUSE_RELEASE",
    "MARVI_MESSAGING_LANGFUSE_SAMPLE_RATE",
    "MARVI_MESSAGING_LANGFUSE_MAX_CHARS",
    "MARVI_MESSAGING_LANGFUSE_CAPTURE",
    "MARVI_MESSAGING_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "MARVI_MESSAGING_ACP_AUTH_METHOD",
    "MARVI_MESSAGING_ACP_AUTO_APPROVE",
    "MARVI_MESSAGING_COPILOT_ACP_COMMAND",
    "MARVI_MESSAGING_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from runtime_support.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set MARVI_MESSAGING_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("MARVI_MESSAGING_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_marvi_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Marvi is running in package-manager-managed mode.

    Two signals: the MARVI_MESSAGING_MANAGED env var (set by the systemd service),
    or a .managed marker file in MARVI_MESSAGING_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Marvi through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``runtime_support/`` — i.e. the git checkout for source
    installs, ``/opt/marvi`` inside the published image. It is a property of
    the running interpreter, NOT of ``$MARVI_MESSAGING_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Marvi was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$MARVI_MESSAGING_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. MARVI_MESSAGING_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.marvi``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$MARVI_MESSAGING_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.marvi:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$MARVI_MESSAGING_HOME``, a
    ...
    ``docker`` and ``marvi update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/marvi-messaging`` image bakes a ``docker``
        stamp into ``/opt/marvi`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $MARVI_MESSAGING_HOME.
    try:
    ...
    #    host install whose shared $MARVI_MESSAGING_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``marvi update``.
    try:
        method = (
            (get_marvi_home() / ".install_method")
    ...
    # detect Nix installs that don't set MARVI_MESSAGING_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``marvi_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$MARVI_MESSAGING_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/marvi`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull nousresearch/marvi-messaging:latest"
    return "marvi update"
...
# banner, the TUI/desktop session info panel, and ``marvi update``. NixOS
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``marvi update`` / ``marvi update --check`` so the wording — and the
...
# Long-form text for ``marvi update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# runtime_support/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Marvi, it doesn't update
...
✗ ``marvi update`` doesn't apply inside the Docker container.

Marvi Messaging runs as a published image (nousresearch/marvi-messaging), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/marvi-messaging:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate marvi-messaging
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/marvi-messaging:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/marvi-messaging/tags
  • Your config and session history live under ``$MARVI_MESSAGING_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``marvi update`` inside Docker.
...
def format_managed_message(action: str = "modify this Marvi installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("MARVI_MESSAGING_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Marvi installation is managed by NixOS "
            f"(MARVI_MESSAGING_MANAGED={env_hint}).\n"
            "Edit services.marvi-messaging.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this Marvi installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Marvi."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from MARVI_MESSAGING_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, marvi_bin
    or None if container mode is not active, we're already inside the
    container, or MARVI_MESSAGING_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("MARVI_MESSAGING_DEV") == "1":
        return None
    ...
    container_mode_file = get_marvi_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "marvi-messaging")
    exec_user = info.get("exec_user", "marvi")
    marvi_bin = info.get("marvi_bin", "/data/current-package/bin/marvi")
...
# Re-export from marvi_constants — canonical definition lives there.
from marvi_constants import get_marvi_home, get_process_marvi_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_marvi_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_marvi_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_marvi_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the MARVI_MESSAGING_UID / MARVI_MESSAGING_GID env vars set by Docker deployments.

    Docker containers running Marvi commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level MARVI_MESSAGING_HOME once, but
    subdirectories created at runtime by ``ensure_marvi_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("MARVI_MESSAGING_UID", "").strip()
    gid_str = os.environ.get("MARVI_MESSAGING_GID", "").strip()
    try:
...
def _chown_to_marvi_uid(path) -> None:
    """Chown ``path`` to ``MARVI_MESSAGING_UID:MARVI_MESSAGING_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_marvi_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_marvi_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the marvi group can
    share state with the gateway service.

    The mode can be overridden via the MARVI_MESSAGING_HOME_MODE environment variable
    (e.g. MARVI_MESSAGING_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse MARVI_MESSAGING_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``MARVI_MESSAGING_UID``/``MARVI_MESSAGING_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("MARVI_MESSAGING_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_marvi_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Marvi runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("MARVI_MESSAGING_CONTAINER") or os.environ.get("MARVI_MESSAGING_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set MARVI_MESSAGING_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into MARVI_MESSAGING_HOME, upgrading legacy empty templates.
...
# ensure_marvi_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_MARVI_MESSAGING_HOME_ENSURED: set = set()


def ensure_marvi_home():
    """Ensure ~/.marvi directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``MARVI_MESSAGING_HOME`` we only re-run
    ...
    ``get_marvi_home()`` and therefore re-run for the new path.
    """
    home = get_marvi_home()
    key = str(home)

    if key in _MARVI_MESSAGING_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``marvi profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old MARVI_MESSAGING_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_marvi_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _MARVI_MESSAGING_HOME_ENSURED.add(key)


def _ensure_marvi_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"MARVI_MESSAGING_HOME {home} does not exist. "
...
from runtime_support.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `marvi config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.marvi.config`` entries, then checks
    ...
    try:
        ...
        # should never break `marvi update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Marvi' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Marvi versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``runtime_support.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (runtime_support/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (runtime_support/setup.py)
    "platform_toolsets",     # written by the setup wizard (runtime_support/setup.py)
    "known_plugin_toolsets", # written/read by runtime_support/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'marvi setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Marvi won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, marvi send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if os.environ.get("MARVI_MESSAGING_RUNTIME") == "1":
        lines.append("  \033[2mOpen Marvi OS Maintenance for diagnostics.\033[0m")
    else:
        lines.append("  \033[2mRun 'marvi doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "marvi update / marvi -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from runtime_support.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ marvi config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in runtime_support.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from runtime_support.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from runtime_support.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: marvi config set <key> <value>")
    ...
    # `marvi update`, but only the version bump is persisted.
    ...
    # metadata.marvi.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.marvi/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_marvi_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``marvi config set``
    ...
    backends) — while display paths (``marvi status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _marvi_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in runtime_support.personality.
from runtime_support.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`runtime_support.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from runtime_support.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from runtime_support.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``MARVI_MESSAGING_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`runtime_support.personality` (single owner).
    """
    from runtime_support.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.marvi/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_marvi_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.marvi/config.yaml.
    ...
    (which change ``MARVI_MESSAGING_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Marvi host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/marvi/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_marvi_dotenv()
...
#   openai-codex (OAuth — marvi auth) — OpenAI Codex
#   nous         (OAuth — marvi auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.marvi/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Marvi writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.marvi/.env.
    ...
    interactive menu render (`marvi tools`, `marvi setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `marvi tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.marvi/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.marvi/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``MARVI_MESSAGING_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"MARVI_MESSAGING_CUSTOM_{slug}_API_KEY" if slug else "MARVI_MESSAGING_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.marvi/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.marvi/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Marvi — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Marvi vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.marvi/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.marvi/.env`` over ``os.environ``.

    Used for Marvi-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Marvi Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from runtime_support.auth import get_anthropic_key
    ...
    # Warn on stale MARVI_MESSAGING_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("MARVI_MESSAGING_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale MARVI_MESSAGING_MAX_ITERATIONS={_env_ghost} "
                f"(run 'marvi doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from runtime_support.personality import active_personality_name
    ...
    print(color("  marvi config edit     # Edit config file", Colors.DIM))
    print(color("  marvi config set <key> <value>", Colors.DIM))
    print(color("  marvi setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``MARVI_MESSAGING_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("MARVI_MESSAGING_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `marvi cron list`, then pin the "
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (runtime_support/plugins_cmd.py, runtime_support/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `marvi config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``marvi config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``marvi config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``marvi config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (marvi config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``marvi config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``marvi config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    marvi config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    marvi config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `marvi config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_marvi_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `marvi config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `marvi config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: marvi config get <key> [--json]")
            print()
            print("Examples:")
            print("  marvi config get model")
            print("  marvi config get terminal.backend")
            print("  marvi config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: marvi config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  marvi config set model anthropic/claude-sonnet-4")
            print("  marvi config set terminal.backend docker")
            print("  marvi config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: marvi config unset <key>")
            print()
            print("Examples:")
            print("  marvi config unset model")
            print("  marvi config unset terminal.backend")
            print("  marvi config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'marvi config migrate' to add them")
        ...
        print("  marvi config           Show current configuration")
        print("  marvi config edit      Open config in editor")
        print("  marvi config get <key>          Print a resolved config value")
        print("  marvi config set <key> <value>   Set a config value")
        print("  marvi config unset <key>        Remove a config value")
        print("  marvi config check     Check for missing/outdated config")
        print("  marvi config migrate   Update config with new options")
        print("  marvi config path      Show config file path")
        print("  marvi config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``marvi config`` UI so users
