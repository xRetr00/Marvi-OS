"""``marvi portal`` — the human-readable entry point for Nous Portal.

Running ``marvi portal`` with no subcommand performs the one-shot Portal
...
``marvi auth add nous --type oauth`` (which still works), is identical to
``marvi setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``marvi auth`` or ``marvi tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `marvi model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `marvi portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `marvi auth add nous --type
    oauth`. It reuses the exact wiring behind `marvi setup --portal` (which in
    ...
    """
    from runtime_support.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `marvi portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `marvi portal` is the
        # human-readable alias for `marvi auth add nous --type oauth` /
        # `marvi setup --portal`.
    ...
    print("Run `marvi portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `marvi portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `marvi portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `marvi auth add "
            "nous --type oauth`, identical to `marvi setup --portal`). "
