...
``runtime_support.config.migrate_config``. Each step is a function
...
``runtime_support.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``runtime_support.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``runtime_support.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``runtime_support.config`` (e.g.
``patch("runtime_support.config.read_raw_config", ...)``) keep working, because
...
#: MARVI_MESSAGING_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_marvi_home()}/config.yaml and run `marvi setup` to "
...
def _cfg():
    """Return the live ``runtime_support.config`` module (lazy, cycle-free)."""
    from runtime_support import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_marvi_home = _c.get_marvi_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$MARVI_MESSAGING_HOME/plugins/`` for currently installed user plugins.
        grandfathered: List[str] = []
        try:
            user_plugins_dir = get_marvi_home() / "plugins"
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `marvi plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `marvi curator status` has no
    ...
    #   3. Creates `~/.marvi/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_marvi_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_marvi_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `marvi config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `marvi config set`"
                )
...
# behavior set it to true explicitly via `marvi config set`.
