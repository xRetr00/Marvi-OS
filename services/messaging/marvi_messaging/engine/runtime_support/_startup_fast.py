...
This module is imported by ``runtime_support/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no runtime_support.config,
...
``runtime_support/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(marvi_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect MARVI_MESSAGING_HOME?"""
    active_profile = os.path.join(marvi_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    marvi_home = os.environ.get("MARVI_MESSAGING_HOME", "").strip()
    if marvi_home:
        return marvi_home
    return os.path.join(os.path.expanduser("~"), ".marvi")
...
def container_mode_may_be_active() -> bool:
    ...
    marvi_home = os.environ.get("MARVI_MESSAGING_HOME", "").strip()
    if marvi_home:
        if os.path.exists(os.path.join(marvi_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(marvi_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(marvi_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".marvi")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect MARVI_MESSAGING_HOME.
    """
...
def print_fast_version_info() -> None:
    from runtime_support import __release_date__, __version__

    print(f"Marvi Messaging v{__version__} ({__release_date__})")
    ...
    print("Run 'marvi version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``marvi --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the MARVI_MESSAGING_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("MARVI_MESSAGING_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
