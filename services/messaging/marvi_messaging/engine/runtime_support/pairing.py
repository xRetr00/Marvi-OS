...
Usage:
    marvi pairing list              # Show all pending + approved users
    marvi pairing approve <platform> <request-id|code>  # Approve a pairing request
    marvi pairing revoke <platform> <user_id> # Revoke user access
    marvi pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle marvi pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: marvi pairing {list|approve|revoke|clear-pending}")
        print("Run 'marvi pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: marvi pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.marvi/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'marvi pairing list' to see pending requests.\n")
