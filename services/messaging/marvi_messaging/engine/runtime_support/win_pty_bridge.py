"""Windows ConPTY bridge for the `marvi dashboard` chat tab.

Drop-in counterpart to ``runtime_support.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``runtime_support.web_server`` consumes: ``spawn``, ``read``,
