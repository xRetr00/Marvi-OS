"""Shell completion script generation for marvi CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_marvi_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Marvi Messaging bash completion
# Add to ~/.bashrc:
#   eval "$(marvi completion bash)"

_marvi_profiles() {{
    local profiles_dir="$HOME/.marvi/profiles"
...
_marvi_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_marvi_profiles)" -- "$cur"))
        return
    fi
...
complete -F _marvi_completion marvi
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef marvi
# Marvi Messaging zsh completion
# Add to ~/.zshrc:
#   eval "$(marvi completion zsh)"

_marvi_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.marvi/profiles" ]]; then
        profiles+=($HOME/.marvi/profiles/*(N/:t))
    fi
...
_marvi() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_marvi_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'marvi command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Marvi Messaging fish completion",
        "# Add to your config:",
        "#   marvi completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __marvi_profiles",
        "    echo default",
        "    if test -d $HOME/.marvi/profiles",
        "        for d in $HOME/.marvi/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c marvi -f -s p -l profile"
        " -d 'Profile name' -xa '(__marvi_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__marvi_profiles)' -d 'Profile name'"
                )
