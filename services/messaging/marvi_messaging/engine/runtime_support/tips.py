...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or marvi -c.",
    ...
    "Ctrl+Z suspends Marvi to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "marvi -c resumes your most recent CLI session. marvi -c \"project name\" resumes by title.",
    "marvi -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "marvi -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "marvi chat -t web,terminal enables only specific toolsets for a focused session.",
    "marvi chat -s github-pr-workflow preloads a skill at launch.",
    "marvi chat -q \"query\" runs a single non-interactive query and exits.",
    "marvi chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "marvi chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "marvi --yolo bypasses all dangerous command approval prompts for the entire session.",
    "marvi chat --source telegram tags the session for filtering in marvi sessions list.",
    "marvi -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "marvi doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "marvi dump outputs a compact setup summary — great for bug reports.",
    "marvi config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "marvi config edit opens config.yaml in your default editor.",
    "marvi config check scans for missing or stale configuration options.",
    "marvi sessions browse opens an interactive session picker with search.",
    "marvi sessions stats shows session counts by platform and database size.",
    "marvi sessions prune --older-than 30 cleans up old sessions.",
    "marvi skills search react --source skills-sh searches the skills.sh public directory.",
    "marvi skills check scans installed hub skills for upstream updates.",
    "marvi skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "marvi skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "marvi mcp add github --command npx adds MCP servers from the command line.",
    "marvi mcp serve runs Marvi itself as an MCP server for other agents.",
    "marvi auth add lets you add multiple API keys for credential pool rotation.",
    "marvi completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "marvi logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "marvi backup creates a zip backup of your entire Marvi home directory.",
    "marvi profile create coder creates an isolated profile that becomes its own command.",
    "marvi profile create work --clone copies your current config and keys to a new profile.",
    "marvi update syncs new bundled skills to ALL profiles automatically.",
    "marvi gateway install sets up Marvi as a system service (systemd/launchd).",
    "marvi memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "marvi webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: marvi tools disables unused tools, marvi skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "marvi models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as marvi -w).",
    ...
    "MARVI_MESSAGING_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Marvi tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'marvi profile create coder' creates the 'coder' command.",
    "marvi profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Marvi prints a resume command with session ID and stats.",
    "marvi sessions export backup.jsonl exports all sessions for backup or analysis.",
    "marvi -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "marvi skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .marvi/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: marvi cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Marvi runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "marvi gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Marvi via a local macOS server.",
    ...
    "Context files (.marvi.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Marvi loads project context from .marvi.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "marvi mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "marvi mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "marvi mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — marvi tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.marvi/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "marvi chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "marvi plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "marvi acp runs Marvi as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "MARVI_MESSAGING_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "marvi auth add nous or marvi auth add openai-codex sets up OAuth-based providers.",
    ...
    "marvi status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.marvi/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Marvi your own.",
    ...
    "Before a gateway session reset, Marvi auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.marvi/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Marvi state via MARVI_MESSAGING_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "MARVI_MESSAGING_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in MARVI_MESSAGING_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.marvi/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'marvi profile create ls' would be rejected.",
    "marvi profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "marvi claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "MARVI_MESSAGING_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via marvi tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.marvi/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.marvi/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Marvi auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Marvi config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Marvi is working.',
    ...
    '/reload reloads ~/.marvi/.env into the running session — pick up new API keys without restarting.',
    ...
    "MARVI_MESSAGING_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.marvi/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.marvi/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'marvi curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "marvi curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'marvi curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'marvi auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'MARVI_MESSAGING_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "MARVI_MESSAGING_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'marvi dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'marvi dashboard embeds the full Marvi TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.marvi/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.marvi/dashboard-plugins/ — no npm build required.',
    ...
    'MARVI_MESSAGING_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'MARVI_MESSAGING_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'MARVI_MESSAGING_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'MARVI_MESSAGING_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.marvi/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.marvi/skills/ so drop-in skills appear without restarting the session.',
    ...
    'marvi -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'marvi chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'marvi chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'marvi chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "marvi chat --source tool tags programmatic chats so they don't clutter marvi sessions list.",
    'marvi dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'marvi sessions rename <ID> "new title" renames any past session; marvi sessions delete <ID> removes one.',
    'marvi import restores a full Marvi backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'marvi fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'marvi pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'marvi setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'marvi status --deep runs the full health sweep across every component; plain marvi status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'MARVI_MESSAGING_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'MARVI_MESSAGING_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.marvi/plugins/ — trust-gated by design.',
    "MARVI_MESSAGING_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'MARVI_MESSAGING_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'MARVI_MESSAGING_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'MARVI_MESSAGING_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'MARVI_MESSAGING_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'MARVI_MESSAGING_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'MARVI_MESSAGING_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'MARVI_MESSAGING_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'MARVI_MESSAGING_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'MARVI_MESSAGING_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'MARVI_MESSAGING_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Marvi block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from marvi sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.marvi/dashboard-plugins/.',
]
