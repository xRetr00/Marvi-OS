...
configured for "cli" in `marvi tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (MARVI_MESSAGING_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `marvi chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    - MARVI_MESSAGING_INFERENCE_MODEL
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"marvi -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "marvi -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"marvi -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "marvi -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "marvi -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to MARVI_MESSAGING_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("MARVI_MESSAGING_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "marvi -z: --provider requires --model (or MARVI_MESSAGING_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"marvi -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("marvi -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``marvi -z`` / oneshot mode.

    Oneshot bypasses ``MarviCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when marvi is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from runtime_support.config import load_config
    from runtime_support.models import detect_provider_for_model
    ...
    env_model = os.getenv("MARVI_MESSAGING_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from runtime_support import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("MARVI_MESSAGING_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # MarviCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    from runtime_support.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                MARVI_MESSAGING_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via MARVI_MESSAGING_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via MARVI_MESSAGING_YOLO_MODE=1
