"""Runtime-backed validation behind ``marvi plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary MARVI_MESSAGING_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="marvi-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            os.environ,
            {
                "MARVI_MESSAGING_HOME": str(home),
                "MARVI_MESSAGING_BUNDLED_PLUGINS": str(bundled),
                "MARVI_MESSAGING_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    from runtime_support.plugins import PluginManager
    ...
    modules_before = {
        ...
        if name == "marvi_plugins" or name.startswith("marvi_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Marvi discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "marvi_plugins" or name.startswith("marvi_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_marvi_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".marvi" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from runtime_support.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Marvi supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Marvi never auto-installs plugin dependencies; "
    ...
    if schema:
        from runtime_support.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Marvi' real scanner and registration path."""
    try:
