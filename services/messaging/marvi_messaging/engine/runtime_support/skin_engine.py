"""Marvi skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Marvi itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.marvi/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.marvi/skins/ therefore
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Marvi Messaging"           # Banner title, status display
...
.. code-block:: python
    ...
    from runtime_support.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Marvi Messaging"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.marvi/skins/
...
from marvi_constants import PRODUCT_NAME, PRODUCT_SHORT_NAME, get_marvi_home
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces MARVI_MESSAGING_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces MARVI_MESSAGING_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        ...
        # and the TUI render the same Marvi gold.
        "colors": {
