"""
Backup and import commands for marvi CLI.

`marvi backup` creates a zip archive of the entire ~/.marvi/ directory
(excluding the marvi-messaging repo and transient files).

`marvi import` restores from a backup zip, overlaying onto the current
MARVI_MESSAGING_HOME root.
"""
...
from marvi_constants import get_default_marvi_root, get_marvi_home, display_marvi_home

# Shared formatter; the private alias is kept because claw.py and the backup
# tests import ``_format_size`` from this module.
from runtime_support.sizefmt import format_bytes as _format_size
...
# ``marvi-messaging`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/marvi-messaging/``
...
# MARVI_MESSAGING_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "marvi-messaging",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under MARVI_MESSAGING_HOME) —
...
# File names that ``marvi import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE MARVI_MESSAGING_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Marvi backup slot."""
...
@contextmanager
def _backup_operation_lock(marvi_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = marvi_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Marvi backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside MARVI_MESSAGING_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to marvi root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``marvi-messaging`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/marvi-messaging/`` — must be preserved.
        if part == "marvi-messaging" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside MARVI_MESSAGING_HOME.
...
# to the user as a hung `marvi update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # runtime_support.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Marvi home directory."""
    marvi_root = get_default_marvi_root()

    if not marvi_root.is_dir():
        print(f"Error: Marvi home directory not found at {marvi_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(marvi_root):
            _run_backup_locked(args, marvi_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, marvi_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"marvi-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"marvi-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_marvi_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(marvi_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(marvi_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``marvi-messaging`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "marvi-messaging" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(marvi_root)
    ...
    # outside MARVI_MESSAGING_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_marvi_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: marvi import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Marvi backup.
    ...
    # Look for telltale files that a marvi home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Marvi backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.marvi/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a marvi dir name
        if prefix in {".marvi", "marvi"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_marvi_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (marvi_root / "config.yaml").exists()
        has_env = (marvi_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Marvi configuration.")
        ...
        marvi_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under MARVI_MESSAGING_HOME.
            ...
            target = marvi_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(marvi_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_marvi_home()}."
            )
        ...
        profiles_dir = marvi_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # runtime_support.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: marvi profile list  (after installing marvi)")

        # Guidance
        print()
        if not (marvi_root / "marvi-messaging").is_dir():
            print("Note: The marvi-messaging codebase was not included in the backup.")
            print("  If this is a fresh install, run: marvi update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  marvi -p {pname} gateway install")
        ...
        try:
            from runtime_support.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Marvi configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and marvi backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to MARVI_MESSAGING_HOME).
...
# — `marvi update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $MARVI_MESSAGING_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside MARVI_MESSAGING_HOME and the entry is silently
...
def _quick_snapshot_root(marvi_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: marvi snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(marvi_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside MARVI_MESSAGING_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``marvi update``.
    ...
    Args:
        ...
        marvi_home: Override for the Marvi home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(marvi_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for marvi backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_marvi_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, marvi_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(marvi_root):
            return _write_full_zip_backup_locked(out_path, marvi_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, marvi_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``marvi_root`` to ``out_path``.
...
def _pre_update_backup_dir(marvi_home: Optional[Path] = None) -> Path:
    home = marvi_home or get_marvi_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of MARVI_MESSAGING_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<MARVI_MESSAGING_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``marvi update``) should continue even if the backup fails.
    """
    marvi_root = marvi_home or get_default_marvi_root()
    if not marvi_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(marvi_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, marvi_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `marvi claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of MARVI_MESSAGING_HOME under ``backups/`` before a
    ``marvi claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``marvi import <archive>``.  Writes to
    ``<MARVI_MESSAGING_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `marvi import` and the
