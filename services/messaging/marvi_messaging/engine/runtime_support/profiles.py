"""
Profile management for multiple isolated Marvi instances.

Each profile is a fully independent MARVI_MESSAGING_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.marvi/profiles/<name>/`` by default.

The "default" profile is ``~/.marvi`` itself — backward compatible,
zero migration needed.

Usage::

    marvi profile create coder          # fresh profile + bundled skills
    marvi profile create coder --clone  # also copy config, .env, SOUL.md, skills
    marvi profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    marvi -p coder chat                 # or via flag
    marvi profile use coder             # set as sticky default
    marvi profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See marvi_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.marvi``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   marvi-messaging  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `marvi profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `marvi
...
# `marvi skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Marvi infrastructure directories that only the default profile
         (``~/.marvi``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_marvi_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.marvi) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "marvi-messaging",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when MARVI_MESSAGING_HOME equals the
...
# into the export. The set below identifies the *known Marvi profile
# artifacts* at the root of MARVI_MESSAGING_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``marvi_constants``.
...
_RESERVED_NAMES = frozenset({
    "marvi", "default", "test", "tmp", "root", "sudo",
})

# Marvi subcommands that cannot be used as profile names/aliases
_MARVI_MESSAGING_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the marvi root, NOT to the current MARVI_MESSAGING_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where MARVI_MESSAGING_HOME points outside
    ``~/.marvi``, profiles live under ``MARVI_MESSAGING_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_marvi_home() / "profiles"


def _get_default_marvi_home() -> Path:
    """Return the default (pre-profile) MARVI_MESSAGING_HOME path.

    In standard deployments this is ``~/.marvi``.
    In Docker/custom deployments where MARVI_MESSAGING_HOME is outside ``~/.marvi``
    (e.g. ``/opt/data``), returns MARVI_MESSAGING_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_marvi_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Marvi installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its MARVI_MESSAGING_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_marvi_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, marvi subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _MARVI_MESSAGING_SUBCOMMANDS:
        return f"'{canon}' conflicts with a marvi subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            marvi_exe = shutil.which("marvi") or "marvi"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(marvi_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Marvi. If we
    ...
    try:
        ...
        # not be migrated. The next `marvi doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``marvi -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``marvi profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``marvi -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``marvi profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``MARVI_MESSAGING_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Marvi config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_marvi_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, marvi_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own MARVI_MESSAGING_HOME. When
    ...
    The returned ``marvi_home`` is the path to pass to
    ``set_marvi_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_marvi_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``marvi update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.marvi)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.marvi/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `marvi -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Marvi profile.\n"
    ...
    if not soul_path.exists():
        try:
            from runtime_support.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `marvi update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`marvi profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `marvi update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running Marvi (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `marvi profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `marvi -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches MARVI_MESSAGING_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``marvi profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MARVI_MESSAGING_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_marvi_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Marvi *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Marvi backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``MARVI_MESSAGING_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `marvi -p <canon> profile
    ...
    marvi_markers = ("runtime_support.main", "marvi-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Marvi process: either an entrypoint marker in argv, or
            # a resolved executable named `marvi`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_marvi = (
                any(marker in joined for marker in marvi_markers)
                or exe_name == "marvi"
                or exe_name.startswith("marvi")
            )
            if not is_marvi:
                continue
            ...
            # ...or by MARVI_MESSAGING_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("MARVI_MESSAGING_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.marvi).\n"
            "To remove everything, use: marvi uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_marvi_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``MARVI_MESSAGING_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from runtime_support.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set MARVI_MESSAGING_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("MARVI_MESSAGING_HOME")
    try:
        os.environ["MARVI_MESSAGING_HOME"] = str(profile_dir)
        from runtime_support.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["MARVI_MESSAGING_HOME"] = old_home
        elif "MARVI_MESSAGING_HOME" in os.environ:
            del os.environ["MARVI_MESSAGING_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.marvi/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: marvi profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from MARVI_MESSAGING_HOME.

    Returns ``"default"`` if MARVI_MESSAGING_HOME is not set or points to ``~/.marvi``.
    Returns the profile name if MARVI_MESSAGING_HOME points into ``~/.marvi/profiles/<name>``.
    Returns ``"custom"`` if MARVI_MESSAGING_HOME is set to an unrecognized path.
    """
    from marvi_constants import get_marvi_home
    marvi_home = get_marvi_home()
    resolved = marvi_home.resolve()

    default_resolved = _get_default_marvi_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      MARVI_MESSAGING_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Marvi file the
      user may have lying alongside MARVI_MESSAGING_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``marvi sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``MARVI_MESSAGING_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.marvi itself — its parent is ~/ and its
        # directory name is ".marvi", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: marvi profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.marvi itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.marvi). "
            "Specify a different name: marvi profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="marvi_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"marvi_{old_name}"
    legacy_old_host = f"marvi.{old_name}"
    new_host = f"marvi_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_marvi_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("marvi_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a MARVI_MESSAGING_HOME path string.

    Called early in the CLI entry point, before any marvi modules
    are imported, to set the MARVI_MESSAGING_HOME environment variable.
    """
