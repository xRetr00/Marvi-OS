...
``CommandDef.execute`` (runtime_support/commands.py) names a key in
...
for a fixed context (enforced by tests/runtime_support/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``runtime_support.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_marvi_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: marvi bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from runtime_support.commands import gateway_help_lines
