"""Safe Marvi Console command engine.

This module backs ``marvi console`` and is intentionally narrower than the
full Marvi CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["MarviConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'marvi doctor'")
        and last.startswith("Run 'marvi setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from runtime_support._subprocess_compat import split_command_line
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="marvi", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `marvi "):
        return ""
    return summary
...
# dashboard opens a fresh MarviConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("runtime_support.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["MarviConsoleEngine", list[str]], str]:
    def handler(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "MarviConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["MarviConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `marvi {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `marvi {usage}`?",
        )


class MarviConsoleEngine:
    """Curated line-command executor for Marvi Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Marvi Console does not run shell syntax. Use one supported "
                    "Marvi command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Marvi component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Marvi logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Marvi configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Marvi Console."""

        extracted = {
            "version": (
                "runtime_support.subcommands.version",
            ...
            "dump": (
                "runtime_support.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "debug": (
                "runtime_support.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "runtime_support.subcommands.prompt_size",
            ...
            "insights": (
                "runtime_support.subcommands.insights",
            ...
            "security": (
                "runtime_support.subcommands.security",
            ...
            "backup": (
                "runtime_support.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "runtime_support.subcommands.import_cmd",
            ...
            "config": (
                "runtime_support.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "runtime_support.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "runtime_support.subcommands.plugins",
            ...
            "skills": (
                "runtime_support.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "mcp": (
                "runtime_support.subcommands.mcp",
                "build_mcp_parser",
                "cmd_mcp",
                [
                    ("list",),
            ...
            "memory": (
                "runtime_support.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "auth": (
                "runtime_support.subcommands.auth",
                "build_auth_parser",
                "cmd_auth",
                [
                    ("list",),
            ...
            "pairing": (
                "runtime_support.subcommands.pairing",
            ...
            "webhook": (
                "runtime_support.subcommands.webhook",
            ...
            "hooks": (
                "runtime_support.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "runtime_support.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "runtime_support.subcommands.profile",
            ...
            "cron": (
                "runtime_support.subcommands.cron",
                "build_cron_parser",
                "cmd_cron",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Marvi configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "runtime_support.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("runtime_support.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "runtime_support.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("runtime_support.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("runtime_support.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Marvi Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Marvi Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`marvi {first}` is not available in Marvi Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Marvi Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Marvi Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Marvi Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Marvi Console.",
            ("skills", "publish"): "`skills publish` is not available in Marvi Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Marvi Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Marvi Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Marvi Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Marvi Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Marvi Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Marvi Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Marvi Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Marvi Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Marvi Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Marvi Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Marvi Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Marvi Console.")
...
def _status(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: MarviConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Marvi Console.")
    ...
    from runtime_support.logs import list_logs, tail_log
...
def _sessions_list(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: MarviConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from runtime_support.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _config_set(_engine: MarviConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from runtime_support.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: MarviConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: MarviConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from marvi_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: MarviConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="runtime_support.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: MarviConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from runtime_support.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: MarviConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from runtime_support.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: MarviConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from marvi console")
...
def _cron_resume(_engine: MarviConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: MarviConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``marvi console`` REPL."""
    ...
    engine = MarviConsoleEngine()
    if interactive:
        print("Marvi Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("marvi> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
