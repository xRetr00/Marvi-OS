...
def _root() -> Path:
    ...
    # helper's token, including custom MARVI_MESSAGING_HOME layouts.
    return get_default_marvi_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, marvi_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(marvi_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the marvi exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs runtime_support.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "runtime_support.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "runtime_support.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, marvi_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, marvi_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves runtime_support's own location and prepends its parent, because the
    ...
    # the runtime_support package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the runtime_support package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["marviPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("Marvi path must be absolute")
    marvi_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(marvi_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Marvi Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('runtime_support.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_marvi(marvi_path: str) -> dict[str, Any]:
    path = os.path.abspath(marvi_path)
    if not os.path.isabs(marvi_path) or not os.path.isfile(path):
        raise ValueError("Marvi path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "marviHome": str(get_default_marvi_root()), "python": sys.executable}
