...
the tool registry or launching Marvi (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``marvi-cli`` to the non-existent ``marvi``. ``resolve_toolset('marvi')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``marvi-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``marvi`` instead of ``marvi-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `marvi tools` to reconfigure."
        )
    return warnings
