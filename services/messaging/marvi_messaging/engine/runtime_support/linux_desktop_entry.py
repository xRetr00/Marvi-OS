"""Install and remove the Linux desktop entry (``marvi.desktop``).

``marvi desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "marvi.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``marvi.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``marvi desktop``.

    Prefer the real ``marvi`` executable (argv[0] or PATH). When Marvi
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from runtime_support.relaunch import resolve_marvi_bin

    bin_path = resolve_marvi_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "runtime_support.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Marvi\n"
        "GenericName=Marvi Desktop\n"
        "Comment=Launch Marvi Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Marvi\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Marvi desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "marvi"
