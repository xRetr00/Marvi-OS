"""``marvi verify`` — detect a project's run recipe and smoke-test it.
...
``.marvi/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``marvi verify`` satisfies
