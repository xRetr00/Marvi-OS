...
Used by ``marvi kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``runtime_support/goals.py`` — same aux
...
MARVI_MESSAGING_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("MARVI_MESSAGING_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Marvi Messaging board.
...
def _profile_author() -> str:
    """Mirror of ``runtime_support.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("MARVI_MESSAGING_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=MARVI_MESSAGING_KANBAN_SPECIFY_MAX_TOKENS,
