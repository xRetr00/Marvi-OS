"""Dependency-light venv recovery that runs BEFORE runtime_support.main's imports.

The ``marvi`` console entry point is ``runtime_support.main:main``.  Importing
``runtime_support.main`` pulls in third-party packages at module level (``dotenv``
via ``runtime_support.env_loader``, ``yaml`` via ``runtime_support.config``, ...).  In
...
a corrupted venv.  ``runtime_support.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``runtime_support.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``marvi acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``runtime_support.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``runtime_support.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``marvi update`` exists AND an import probe
