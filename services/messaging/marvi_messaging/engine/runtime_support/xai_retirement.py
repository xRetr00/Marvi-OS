...
Pure logic: walks a Marvi config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `marvi doctor` and a future `marvi migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Marvi config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Marvi config and return retirement issues.
...
def apply_migration(
    ...
    from runtime_support.config import require_readable_config_before_write
    ...
    # None``, so the next `marvi migrate xai` reports nothing to migrate
