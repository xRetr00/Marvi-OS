"""Default configuration data for Marvi Messaging.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from runtime_support/config.py. Must not import from runtime_support.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["marvi-cli"],
    # SQLite journal mode used by every Marvi database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Marvi server processes.
    ...
    "agent": {
        ...
        # an interactive `marvi gateway restart` must never block for hours
        ...
        # the Marvi-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Marvi
        ...
        # identity slot (SOUL.md). Empty by default. The MARVI_MESSAGING_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Marvi adds a coding operating brief
        ...
        # `marvi status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``MARVI_MESSAGING_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             MARVI_MESSAGING_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force MARVI_MESSAGING_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Marvi auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Marvi sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Marvi
        ...
        #  "/home/user/.marvi/cache/documents:/output"]
        ...
        # bundled Marvi image, which drops to the `marvi` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Marvi sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Marvi should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``marvi chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.marvi/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: marvi sweeps the checkpoint base at startup
        ...
        # ``marvi checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .marvi.md, or .cursorrules before Marvi applies
    ...
    # Single-query (``marvi -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Marvi truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Marvi' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); marvi =
                                      # Marvi' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Marvi' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Marvi does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``marvi kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``marvi kanban decompose`` and the kanban
        ...
        # ``marvi profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `marvi model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `marvi` (and `marvi chat`) launches by default:
        ...
        # REPL and `--tui` (or MARVI_MESSAGING_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `marvi --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `marvi -c` muscle memory.  Default off so existing
        # users aren't surprised.  MARVI_MESSAGING_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `marvi --tui` drops a one-time hint
        ...
        # model — see runtime_support/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to MARVI_MESSAGING_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/marvi`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `marvi pets`. Disabled until
        ...
        "pet": {
            ...
            # get_marvi_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``MARVI_MESSAGING_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``MARVI_MESSAGING_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``MARVI_MESSAGING_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``MARVI_MESSAGING_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``MARVI_MESSAGING_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/marvi``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Marvi appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.marvi/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.marvi/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.marvi/cache/delegation/
    ...
    # response. If not, Marvi feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Marvi auto-pauses the goal and
    ...
    "moa": {
        ...
        # <marvi_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.marvi/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${MARVI_MESSAGING_SKILL_DIR} and ${MARVI_MESSAGING_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `marvi curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `marvi curator run --consolidate` overrides
        ...
        # normally restored on every `marvi update`, so pruning them only
        ...
        # skipped), snapshot ~/.marvi/skills/ into
        # ~/.marvi/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `marvi curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for marvi-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Marvi Messaging*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (MARVI_MESSAGING_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Marvi' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.marvi/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or MARVI_MESSAGING_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .marvi config) always require human
        ...
        # longer trigger the startup banner. Add via `marvi doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``runtime_support/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Marvi to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $MARVI_MESSAGING_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via MARVI_MESSAGING_CRON_MAX_PARALLEL env var.
        ...
        # MARVI_MESSAGING_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # MARVI_MESSAGING_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `marvi -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `marvi` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `marvi kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     marvi-messaging's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Marvi tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.marvi/logs/.
    ...
    # update model picker lists without shipping a marvi-messaging release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/xRetr00/Marvi/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `marvi model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Marvi provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal MARVI_MESSAGING_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.marvi/sessions/sessions.json entirely.
        ...
        # the MARVI_MESSAGING_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.marvi.gateway` that defenses 1-2 don't
        ...
        # `marvi update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``MARVI_MESSAGING_GATEWAY_MAX_STARTS`` / ``MARVI_MESSAGING_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.marvi/.env,
        ...
        # behavior: files must live under the Marvi cache, under
        ...
        # user. Bridged to MARVI_MESSAGING_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Marvi
        # cache (~/.marvi/cache/{documents,images,audio,video,screenshots})
        ...
        # to MARVI_MESSAGING_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # MARVI_MESSAGING_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to MARVI_MESSAGING_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.marvi/state.db.
    ...
    "sessions": {
        ...
        # the default of ``marvi sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.marvi/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `marvi sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `marvi update` prints a one-line notice with
        ...
        # ~/.marvi/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to MARVI_MESSAGING_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # MARVI_MESSAGING_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `marvi sessions export`. Checked
    ...
    # ``marvi doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `marvi doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``marvi update`` behaviour.
    "updates": {
        ...
        #     DBs) into <MARVI_MESSAGING_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``marvi backup``-style zip
        #     of MARVI_MESSAGING_HOME into <MARVI_MESSAGING_HOME>/backups/, restorable with
        #     ``marvi import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `marvi update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<MARVI_MESSAGING_HOME>/lsp/bin/`` on first use.
    ...
    # `marvi tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.marvi/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.marvi/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.marvi/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Marvi may use this encrypted
            ...
            # ~/.marvi/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.marvi/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Marvi disables it for our users unless they
        # explicitly opt in here. When false (default), Marvi sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Marvi also calls
    ...
    #
    # Configure with `marvi egress setup`.  Disabled by default — the rest of
    # Marvi works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.marvi/bin/ on
    ...
    # Marvi Desktop (Electron app) launch options. These only affect
    # `marvi desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the MARVI_MESSAGING_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the MARVI_MESSAGING_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Marvi mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'marvi model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Marvi auth store)",
    ...
    "MARVI_MESSAGING_LANGFUSE_PUBLIC_KEY": {
    ...
    "MARVI_MESSAGING_LANGFUSE_SECRET_KEY": {
    ...
    "MARVI_MESSAGING_LANGFUSE_BASE_URL": {
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Marvi, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @marvi:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. MARVI_MESSAGING_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #marvi)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: marvi-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'marvi-messaging' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Marvi API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Marvi API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Marvi API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # MARVI_MESSAGING_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to MARVI_MESSAGING_TOOL_PROGRESS_MODE for backward
    ...
    # MARVI_MESSAGING_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "MARVI_MESSAGING_PREFILL_MESSAGES_FILE": {
    ...
    "MARVI_MESSAGING_EPHEMERAL_SYSTEM_PROMPT": {
