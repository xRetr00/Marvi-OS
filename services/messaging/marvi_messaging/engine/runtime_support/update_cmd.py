"""Marvi update pipeline — extracted from ``runtime_support/main.py``.
...
change is that references to helpers/constants that STAY in ``runtime_support.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``runtime_support.main`` reference — so existing call sites and test monkeypatches
that target ``runtime_support.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``runtime_support.main``.
...
Imports are one-way: ``runtime_support.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``runtime_support.main`` reference.

    Lets callers keep patching ``runtime_support.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``runtime_support.main`` -> ``runtime_support.update_cmd`` stays one-way
    at import time.
    """
    from runtime_support import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``marvi update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``marvi update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``runtime_support.config_defaults``,
    ``runtime_support.config``, and ``runtime_support.config_migrations`` from disk
    so subsequent imports read the UPDATED code.
    """
    import importlib

    importlib.invalidate_caches()
    for mod_name in ("runtime_support.config_defaults", "runtime_support.config", "runtime_support.config_migrations"):
...
def _run_config_check_fresh() -> tuple:
    ...
    from runtime_support.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from runtime_support.config import migrate_config

    return migrate_config(interactive=interactive, quiet=quiet)


# Critical files that Marvi must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "runtime_support/main.py",
    "runtime_support/config.py",
    "runtime_support/__init__.py",
    "runtime_support/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``marvi`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="marvi-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``marvi update`` can be driven by a
    ...
    """
    from marvi_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from marvi_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('marvi_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``marvi update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `marvi update`.
    ...
    print("  Preview now:  marvi curator run --dry-run")
    print("  Pause it:     marvi curator pause")
    print(
        "  Docs:         https://github.com/xRetr00/Marvi-OS/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `marvi update`.
    ...
    try:
        ...
        return
    db_path = get_marvi_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `marvi sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    marvi sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  marvi sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``marvi update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``marvi update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: marvi curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    marvi dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``runtime_support.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.marvi-update-staging"
    backup = f"{dst}.marvi-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `marvi update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``marvi_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("MARVI_MESSAGING_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== marvi-update completed {action_id} ===")


def _update_via_zip(args):
    """Update Marvi Messaging by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `marvi update "
            f"--branch {branch}`, or update against main with `marvi update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/xRetr00/Marvi-OS/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="marvi-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"marvi-messaging-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for marvi-messaging itself
        ...
        # GitHub ZIPs extract to marvi-messaging-<branch>/
        extracted = os.path.join(tmp_dir, f"marvi-messaging-{branch}")
        ...
        print(
            "  Re-run `marvi update` to retry; if the agent won't start, "
            "reinstall from https://github.com/xRetr00/Marvi-OS"
        )
        _m().sys.exit(1)
    finally:
    ...
    from runtime_support.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `marvi update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: marvi skills list-modified  "
    ...
    try:
        from runtime_support.model_catalog import seed_cache_from_checkout
    ...
    try:
        from runtime_support.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_marvi_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_marvi_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "marvi-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Marvi behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes marvi completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Marvi couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Marvi couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Marvi behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Marvi couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Marvi couldn't drop "
...
OFFICIAL_REPO_URLS = {
    ...
    "git@github.com:xRetr00/Marvi-OS",
}

OFFICIAL_REPO_URL = "https://github.com/xRetr00/Marvi-OS.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_marvi_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_marvi_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Marvi repository.")
        print("  This means you may miss updates from xRetr00/Marvi-OS.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/xRetr00/Marvi-OS.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/xRetr00/Marvi-OS.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``marvi update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from marvi_constants import get_default_marvi_root

    default_home = get_default_marvi_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "marvi.exe"
    lines = ["✗ Another marvi.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Marvi Desktop, exit any open `marvi` REPLs, and")
    lines.append("  stop the gateway (`marvi gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `marvi update --force` if you've already")
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``marvi update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `marvi update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `marvi update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Marvi' editable-install extras or
    ...
    try:
        from runtime_support.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from runtime_support.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$MARVI_MESSAGING_HOME/bin/uv``, but on Termux the official
    ...
    """
    from runtime_support.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `marvi update` is exactly the step
...
def _npm_lockfile_changed(marvi_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `marvi
    ...
    try:
        ...
        cache_file = marvi_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(marvi_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    marvi update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `marvi update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from marvi_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `marvi update`.")
            failed = []
            if any(
    ...
    # Marvi profile using this checkout. Keep one per-checkout cache under the
    # shared Marvi root rather than rerunning npm once per named profile.
    shared_marvi_root = get_default_marvi_root()
    ...
    # `marvi update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_marvi_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `marvi update`.")
        return list(labels)
    ...
    nixos_env = with_marvi_node_path(_m()._nixos_build_env())
    ...
    # `marvi update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_marvi_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.marvi/logs/update.log`` only, never the terminal.

    During ``marvi update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``marvi update --check``: fetch and report without installing.
    ...
    """
    from runtime_support.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``marvi update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from runtime_support.config import format_docker_update_message
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from runtime_support.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``marvi update`` without requiring a reinstall.  The
    ...
    ``marvi`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/marvi.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v marvi'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/marvi, code at /usr/local/lib/marvi-messaging).
    fhs_link = Path("/usr/local/bin/marvi")
    ...
    path_comment = (
        "# Marvi Messaging — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``marvi-acp`` launcher next to the ``marvi`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``marvi update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``marvi-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Marvi as not installed even when it is.

    The shim simply delegates to the sibling ``marvi`` launcher with the
    ...
    No-op on Windows (install.ps1 copies ``marvi.exe`` + ``marvi-acp.exe``
    ...
    so ``marvi-acp.exe`` already resolves) and wherever a ``marvi-acp`` is
    already present next to the ``marvi`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        marvi_cmd = bin_dir / "marvi"
        acp_cmd = bin_dir / "marvi-acp"
        try:
            if not (marvi_cmd.is_file() or marvi_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of MARVI_MESSAGING_HOME under
      ``backups/`` (restorable via ``marvi import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from runtime_support.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from marvi_constants import
        # get_marvi_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from runtime_support.config import get_marvi_home as _get_home
    ...
    try:
        from runtime_support.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from runtime_support.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_marvi_home so the user sees ~/.marvi/...
    try:
        from marvi_constants import get_marvi_home, display_marvi_home

        home = get_marvi_home()
        try:
            display_path = f"{display_marvi_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  marvi import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``marvi update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``marvi update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run marvi from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.marvi-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".marvi-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The marvi.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m runtime_support.main
    ...
    calling process and its ancestors are always excluded (a CLI ``marvi
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m runtime_support.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "runtime_support.main" in cmdline_low:
        ...
        # the `-m runtime_support.main gateway run` argv, so autostarted gateways
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Marvi processes are running from this install's venv:",
    ]
    ...
    lines.append(
        "  Close the Marvi desktop app / other Marvi terminals, then re-run:"
    )
    lines.append("    marvi update")
    lines.append("  (or use `marvi update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``marvi update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from runtime_support._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning marvi-setup, and the update-in-progress marker
    ...
    the update with "Marvi is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Marvi backend (``runtime_support.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.marvi-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "runtime_support.main" in argv_low and (
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.marvi-runtime`` interpreter child alive and holding the install open
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    marvi.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        from gateway.status import terminate_pid
        from runtime_support.gateway import (
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m runtime_support.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``marvi gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from runtime_support import gateway_windows
        from runtime_support.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``marvi-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("marvi-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``marvi update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``marvi-setup.exe``) executes
    ``$MARVI_MESSAGING_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_marvi_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``marvi update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``marvi update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Marvi Messaging...")
    print()

    # On Windows, abort early if another marvi.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_marvi_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `marvi serve` backend) will keep .pyd
    ...
    # the marvi.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .marvi-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Marvi is still
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://github.com/xRetr00/Marvi-OS/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Marvi windows/gateways and re-run: marvi update")
            else:
        ...
        # orphan merge-conflict markers in runtime_support/config.py bricked
        # every user who ran ``marvi update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``marvi
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``marvi update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_marvi_home added to marvi_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``marvi`` launch finishes the install
        ...
        # symbols in marvi_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `marvi` again "
        ...
        if not import_ok:
            ...
            print("    Run `marvi update` again — if it persists, reinstall:")
            print("    https://github.com/xRetr00/Marvi-OS")

        node_failures = _update_node_dependencies()
        _m()._build_web_ui(_m().PROJECT_ROOT / "web")

        # Rebuild the desktop app if the source tree changed since the last
        # build.  ``marvi desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``marvi desktop`` shouldn't be forced into a full
        # Electron build by ``marvi update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `marvi desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "runtime_support.main", "desktop", "--build-only"]
                ...
                # debuggable.
                #
                # Start the build subprocess with the Marvi-managed Node on PATH:
                # when `marvi update` runs inside the desktop updater chain
                # (Desktop → marvi-setup → marvi update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from marvi_constants import with_marvi_node_path

                _build_env = with_marvi_node_path()
                build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `marvi desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_marvi_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.marvi/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit MARVI_MESSAGING_HOME so
        # it is not affected by sync_skills()'s module-level MARVI_MESSAGING_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's MARVI_MESSAGING_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from runtime_support.profiles import backfill_profile_envs
        ...
        # ``marvi update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``runtime_support.config`` and
        # ``runtime_support.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from runtime_support.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'marvi config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'marvi config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: marvi config migrate")
            else:
                print()
                print("Skipped. Run 'marvi config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from runtime_support.backup import restore_cron_jobs_if_emptied
        ...
        # check `marvi curator status`. Self-stamps after printing so it
        ...
        # Self-heal the marvi-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Marvi on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("marvi-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``marvi update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `marvi update`
                # must stay fast; `marvi computer-use install --upgrade`
        ...
        # When running as ``marvi update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_marvi_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... marvi-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... marvi-gateway*)
            ...
            try:
                from runtime_support.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all marvi-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: marvi gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `marvi update` restart it automatically, allow\n"
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: marvi gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: marvi -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Marvi gateway unit files are still installed.
        # When both marvi.service (from a pre-rename install) and the
        # current marvi-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `marvi update` surfaces the issue until the user migrates.
        try:
            from runtime_support.gateway import (
                has_legacy_marvi_units,
                _find_legacy_marvi_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_marvi_units():
                print()
                print("⚠ Legacy Marvi gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_marvi_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (marvi.service) fight the current")
                print("  marvi-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    marvi gateway migrate-legacy")
                print()
        ...
        print("  marvi model              # Select provider and model")
