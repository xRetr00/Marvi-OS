"""Slash command definitions and autocomplete for the Marvi CLI.
...
from marvi_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``runtime_support.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``runtime_support.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Marvi config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Marvi works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Marvi is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.marvi/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Marvi Messaging to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Marvi Messaging version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`runtime_support.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/marvi`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Marvi ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Marvi
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``marvi skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``marvi skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``marvi skills list`` and
...
# unrelated command landed. These claim their slots right after /marvi,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /marvi <command>).
...
# native slot, the alias spelling stays reachable via /marvi reset).
...
# ``/marvi <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/marvi" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /marvi topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /marvi moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /marvi debug on Slack.
#   - egress: Docker-only proxy status; reachable as /marvi egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /marvi init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /marvi version on
...
#   - diff: git working-tree diff; reached via /marvi diff on Slack so it
...
#     /marvi update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /marvi heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /marvi refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /marvi pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /marvi whoami on Slack.
...
_SLACK_VIA_MARVI_MESSAGING_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "whoami"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/marvi <verb>`` subcommand.
    ...
    can still reach them via ``/marvi <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/marvi`` is always reserved as the first entry so the
    legacy ``/marvi <subcommand>`` form keeps working for anything that
    ...
    # Reserve /marvi as the catch-all top-level command.
    entries.append(("marvi", "Talk to Marvi or run a subcommand", "[subcommand] [args]"))
    seen.add("marvi")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_MARVI_MESSAGING_ONLY:
            # Intentionally Slack-via-/marvi only (see _SLACK_VIA_MARVI_MESSAGING_ONLY).
            return
    ...
    # everything except /marvi, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://marvi-messaging.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /marvi handler.

    Maps both canonical names and aliases so /marvi bg do stuff works
    the same as /marvi background do stuff.

    Plugin-registered slash commands are included so ``/marvi <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from runtime_support.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via runtime_support.personality."""
        try:
