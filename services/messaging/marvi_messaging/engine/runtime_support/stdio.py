...
on Japanese locales, etc.).  Marvi's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``runtime_support/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Marvi is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``MARVI_MESSAGING_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("MARVI_MESSAGING_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Marvi-managed Git install directories so
    ...
    # already-running shells don't see the change — which means marvi
    ...
    # install without Marvi-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Marvi's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Marvi's
    ``marvi config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Marvi (User env var in Windows Settings, or
    export in a PowerShell profile) and Marvi picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Marvi-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\marvi\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``marvi`` from right after install) retain their old PATH.

    Any subprocess Marvi spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Marvi-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "marvi", "git", "cmd"),
        os.path.join(local_appdata, "marvi", "git", "bin"),
        os.path.join(local_appdata, "marvi", "git", "usr", "bin"),
        # Marvi venv Scripts directory — host of the marvi.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes marvi, but harmless to include.
        os.path.join(local_appdata, "marvi", "marvi-messaging", "venv", "Scripts"),
