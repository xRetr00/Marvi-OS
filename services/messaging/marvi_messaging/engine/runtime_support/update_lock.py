"""Cross-process mutual exclusion for in-flight Marvi updates.

Three different surfaces can start an update of the same install tree:

* ``marvi update`` from a terminal,
* the dashboard's Update button (``POST /api/marvi/update`` →
  ``_spawn_marvi_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``marvi-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``marvi update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <MARVI_MESSAGING_HOME>/.marvi-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``marvi update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Marvi is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``marvi-setup`` binary under ``~/.marvi`` is only refreshed by a full
  installer run (``copy_self_to_marvi_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Marvi is still running") on every
  GUI update forever, with no Marvi process actually running.
"""
...
MARKER_NAME = ".marvi-update-in-progress"

# Set by an orchestrating updater (the Tauri `marvi-setup --update` flow) to
# its own pid before spawning `marvi update` as a child stage. The parent
...
HANDOFF_PID_ENV = "MARVI_MESSAGING_UPDATE_HANDOFF_PID"
...
# to show "Marvi is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Marvi home (never the context-local profile override):
    the Rust updater resolves ``$MARVI_MESSAGING_HOME`` or the platform default, and the
    ...
    """
    from marvi_constants import get_process_marvi_home

    return get_process_marvi_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``marvi update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``marvi-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Marvi update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `marvi update` as a stage): we run under ITS claim
