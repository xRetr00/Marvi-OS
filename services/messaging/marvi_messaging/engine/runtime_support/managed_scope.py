...
A system-level directory (default ``/etc/marvi``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.marvi/config.yaml`` and ``~/.marvi/.env`` on a per-leaf-key basis.

This is DISTINCT from ``runtime_support.config.is_managed()`` / ``MARVI_MESSAGING_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/marvi")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/marvi`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``MARVI_MESSAGING_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$MARVI_MESSAGING_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/marvi`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("MARVI_MESSAGING_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through runtime_support.config.load_config) to honor managed
    scope. Mirrors runtime_support.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from runtime_support.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
