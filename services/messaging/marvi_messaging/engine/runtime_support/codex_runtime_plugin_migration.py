"""Migrate Marvi' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Marvi' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Marvi mcp_servers.<n>.command/args/env  → codex stdio transport
  Marvi mcp_servers.<n>.url/headers       → codex streamable_http transport
  Marvi mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Marvi mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Marvi-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Marvi' own
...
MIGRATION_MARKER = (
    "# managed by marvi-messaging — `marvi codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end marvi-messaging managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Marvi config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Marvi' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, marvi_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Marvi MCP server config to the codex inline-table dict
    ...
    if not isinstance(marvi_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(marvi_cfg.get("command"))
    has_url = bool(marvi_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (MARVI_MESSAGING_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Marvi)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Marvi' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Marvi' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Marvi
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="marvi-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a MARVI_MESSAGING_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed marvi-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.marvi`` symlink that happens to live under ``/private/var/folders``
...
def _build_marvi_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Marvi' own
    ...
    so a marvi installed under /opt/, /usr/local/, or a venv all work.
    MARVI_MESSAGING_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # MARVI_MESSAGING_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_marvi_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever MARVI_MESSAGING_HOME its launcher sets
    ...
    # — that would override the launcher's MARVI_MESSAGING_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("MARVI_MESSAGING_HOME", tmp_path) would
    ...
    marvi_home = os.environ.get("MARVI_MESSAGING_HOME") or ""
    if marvi_home and _looks_like_test_tempdir(marvi_home):
        marvi_home = ""
    if marvi_home:
        env["MARVI_MESSAGING_HOME"] = marvi_home
    # PYTHONPATH passes through so a worktree-launched marvi finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["MARVI_MESSAGING_QUIET"] = "1"
    env["MARVI_MESSAGING_REDACT_SECRETS"] = env.get("MARVI_MESSAGING_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.marvi_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_marvi_tools: bool = True,
) -> MigrationReport:
    """Translate Marvi mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        marvi_config: full ~/.marvi/config.yaml dict
        ...
        expose_marvi_tools: when True (default), register Marvi' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Marvi for tools
    ...
    marvi_servers = (marvi_config or {}).get("mcp_servers") or {}
    if not isinstance(marvi_servers, dict):
        report.errors.append(
            "mcp_servers in Marvi config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in marvi_servers.items():
    ...
    # Inject Marvi' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Marvi for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/marvi_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_marvi_tools:
        translated["marvi-tools"] = _build_marvi_tools_mcp_entry()
        if "marvi-tools" not in report.migrated:
            report.migrated.append("marvi-tools")
