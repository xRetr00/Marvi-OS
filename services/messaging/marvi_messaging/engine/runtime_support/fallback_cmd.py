"""
marvi fallback — manage the fallback provider chain.
...
Subcommands:
  marvi fallback [list]   Show the current fallback chain (default when no subcommand)
  marvi fallback add      Pick provider + model via the same picker as `marvi model`,
                           then append the selection to the chain
  marvi fallback remove   Pick an entry to delete from the chain
  marvi fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.marvi/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from runtime_support.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `marvi model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  marvi fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `marvi model`, then append the selection to the chain."""
    from runtime_support.main import _require_tty, select_provider_and_model
    ...
    print("  `marvi model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `marvi fallback list` to view, or `marvi fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``marvi fallback [subcommand]``."""
