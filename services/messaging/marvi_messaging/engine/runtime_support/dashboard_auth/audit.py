...
Profile-aware location: ``$MARVI_MESSAGING_HOME/logs/dashboard-auth.log``.
...
from ``marvi_constants`` or other runtime_support modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$MARVI_MESSAGING_HOME/logs/dashboard-auth.log``.

    Uses ``marvi_constants.get_marvi_home()`` (a leaf module — no import
    ...
    return get_marvi_home() / "logs" / "dashboard-auth.log"
