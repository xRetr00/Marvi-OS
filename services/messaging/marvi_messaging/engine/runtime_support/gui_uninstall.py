"""
Marvi Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``marvi desktop`` / ``marvi gui``)
     Built inside the agent checkout under ``$MARVI_MESSAGING_HOME/marvi-messaging/``:
       ...
       - ``$MARVI_MESSAGING_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Marvi.app`` or ``~/Applications/Marvi.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Marvi`` (NSIS per-user)
...
the app name ("Marvi"), separate from ``$MARVI_MESSAGING_HOME``:
  - macOS:   ``~/Library/Application Support/Marvi``
  - Windows: ``%APPDATA%\\Marvi``
  - Linux:   ``$XDG_CONFIG_HOME/Marvi`` (default ``~/.config/Marvi``)
...
``marvi uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(marvi_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return marvi_home / "marvi-messaging"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Marvi"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Marvi"
...
def source_built_gui_artifacts(marvi_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``marvi desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``marvi-messaging/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(marvi_home)
    ...
    return [
        ...
        marvi_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Marvi" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Marvi.app"),
            home / "Applications" / "Marvi.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Marvi).
            local_base / "Programs" / "Marvi",
            # Older / alternate layout some builds used.
            local_base / "marvi-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Marvi")
    else:
        ...
        # ``uninstall_gui``.
        from runtime_support.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `marvi desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Marvi.desktop",
        ]
    return paths


def agent_is_installed(marvi_home: Path) -> bool:
    """Return True when a usable Python agent install exists under MARVI_MESSAGING_HOME.
    ...
    if (agent_root / "runtime_support").is_dir():
        return True
...
def gui_is_installed(marvi_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(marvi_home):
        if p.exists():
            return True
...
def gui_install_summary(marvi_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = marvi_home if marvi_home is not None else get_marvi_home()
...
def uninstall_gui(marvi_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``marvi-messaging/runtime_support`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$MARVI_MESSAGING_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Marvi disappears from the launcher.
        try:
            from runtime_support.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove marvi' or "
            "'sudo dnf remove marvi'). AppImage builds are a single file you "
