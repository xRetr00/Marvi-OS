...
- Keep data: Remove code but keep ~/.marvi/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Marvi PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing legacy agent markers or marvi PATH entries.
            new_lines = []
            skip_next = False
            legacy_agent_marker = "# " + "Marvi" + " Agent"
            legacy_repo_marker = "# " + "marvi" + "-agent"
            ...
            for line in content.split('\n'):
                ...
                if legacy_agent_marker in line or legacy_repo_marker in line:
                    skip_next = True
                    continue
                if skip_next and ('marvi' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False

                # Remove any PATH line containing marvi
                if 'marvi' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Marvi-owned file, and
...
def remove_wrapper_script():
    """Remove the marvi wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "marvi",
        Path.home() / ".local" / "bin" / "marvi-acp",
        Path.home() / ".local" / "bin" / "marvi-messaging",
        Path("/usr/local/bin/marvi"),
        Path("/usr/local/bin/marvi-acp"),
        Path("/usr/local/bin/marvi-messaging"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains runtime_support reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'runtime_support' in content or 'marvi-messaging' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(marvi_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``marvi`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Marvi home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (marvi_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``marvi gateway run`` processes
    ...
    try:
        from runtime_support.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from runtime_support.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``MARVI_MESSAGING_HOME`` and ``MARVI_MESSAGING_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\marvi\git\cmd``, ``%LOCALAPPDATA%\marvi\git\bin``,
#      ``%LOCALAPPDATA%\marvi\git\usr\bin``, ``%LOCALAPPDATA%\marvi\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\marvi\git\`` and Node to
#      ``%LOCALAPPDATA%\marvi\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``marvi dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\marvi\gateway-service\`` and sometimes
...
def _marvi_path_markers(marvi_home: Path) -> list[str]:
    """Path-entry substrings that identify Marvi-owned User-PATH entries."""
    root = str(marvi_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare marvi-messaging install dir.
    markers = [root + "\\marvi-messaging", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if MARVI_MESSAGING_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "marvi".  We don't want to catch
    # unrelated entries like "cmarvi-foo" or "ephermeral", so we look for
    # backslash-marvi as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(marvi_home: Path) -> list[str]:
    """Strip Marvi-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _marvi_path_markers(marvi_home)
            kept: list[str] = []
...
def remove_marvi_env_vars_windows() -> list[str]:
    """Delete MARVI_MESSAGING_HOME and MARVI_MESSAGING_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("MARVI_MESSAGING_HOME", "MARVI_MESSAGING_GIT_BASH_PATH"):
                try:
...
def _is_default_marvi_home(marvi_home: Path) -> bool:
    """Return True when ``marvi_home`` points at the default (non-profile) root."""
    try:
        from marvi_constants import get_default_marvi_root
        return marvi_home.resolve() == get_default_marvi_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its MARVI_MESSAGING_HOME directory.

    We shell out to ``marvi -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current MARVI_MESSAGING_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m runtime_support.main` so we don't depend on a `marvi`
    #    wrapper that may be half-removed mid-uninstall.
    marvi_invocation = [_sys.executable, "-m", "runtime_support.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                marvi_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's MARVI_MESSAGING_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``marvi uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$MARVI_MESSAGING_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from runtime_support.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(marvi_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Marvi Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Marvi Chat GUI installation was found.")
        print(f"  Checked: {marvi_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Marvi Messaging stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(marvi_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Marvi Messaging at {marvi_home / 'marvi-messaging'}")
        print(f"  • Your config, sessions, and secrets under {marvi_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Marvi Messaging is still installed. Run 'marvi' to use the CLI,")
    print("or 'marvi uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.marvi/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.marvi/ for future reinstall
    """
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            ...
            marvi_home=marvi_home,
            full_uninstall=bool(getattr(args, "full", False)),
        )
        return

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie MARVI_MESSAGING_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_marvi_home(marvi_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.marvi data); otherwise keep-data. Named profiles
    ...
    print(f"  Config:  {marvi_home / 'config.yaml'}")
    print(f"  Secrets: {marvi_home / '.env'}")
    print(f"  Data:    {marvi_home / 'cron/'}, {marvi_home / 'sessions/'}, {marvi_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their MARVI_MESSAGING_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Marvi data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Marvi code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, marvi_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Marvi PATH entries from shell configs / Windows User PATH")
    print("  • Marvi wrapper scripts and Marvi-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Marvi config/data: {marvi_home}")
        if _is_default_marvi_home(marvi_home):
        ...
        print(f"  • Keep Marvi config/data: {marvi_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``marvi`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$MARVI_MESSAGING_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in marvi_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\marvi\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(marvi_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Marvi-owned PATH entries in User environment")

        log_info("Removing MARVI_MESSAGING_HOME / MARVI_MESSAGING_GIT_BASH_PATH User env vars...")
        removed_env = remove_marvi_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Marvi-set User env vars to remove")

    # 3. Remove wrapper script
    log_info("Removing marvi command...")
    ...
    #     (only when they still point into this Marvi home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Marvi-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(marvi_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Marvi-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(marvi_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE MARVI_MESSAGING_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from runtime_support.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(marvi_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.marvi/, just remove the marvi-messaging subdir
            if marvi_home in project_root.parents or project_root.parent == marvi_home:
    ...
    #     under MARVI_MESSAGING_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(marvi_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(marvi_home)
    ...
    # 5. Optionally remove ~/.marvi/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile MARVI_MESSAGING_HOME dirs live under
        ...
        try:
            if marvi_home.exists():
                shutil.rmtree(marvi_home)
                log_success(f"Removed {marvi_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {marvi_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {marvi_home}")
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m runtime_support.uninstall --mode <gui|lite|full>``.
    ...
    so ``import runtime_support`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``marvi_constants`` + ``runtime_support.colors``
    (and lazily ``runtime_support.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m runtime_support.uninstall")
    parser.add_argument(
        "--mode",
