"""
Doctor command for marvi CLI.

Diagnoses issues with Marvi Messaging setup.
"""
...
from marvi_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
MARVI_MESSAGING_HOME = get_marvi_home()
_DHH = display_marvi_home()  # user-facing display path (e.g. ~/.marvi or ~/.marvi/profiles/coder)

# Load environment variables from ~/.marvi/.env so API key checks work
_env_path = get_env_path()
load_marvi_dotenv(marvi_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from runtime_support.colors import Colors, color
from runtime_support.models import _MARVI_MESSAGING_USER_AGENT
from runtime_support.vercel_auth import describe_vercel_auth
from marvi_constants import OPENROUTER_MODELS_URL
...
from marvi_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Marvi containers"
    elif method in {"nix", "nixos"}:
        action = recommended_update_command_for_method(method)
    else:
        action = "run `marvi update`"
    return (
...
def _marvi_database_paths(marvi_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Marvi-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from runtime_support.backup import _QUICK_STATE_FILES

    entries = [
        (name, marvi_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((marvi_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(marvi_home)), board_db))
    return entries
...
def _format_db_size(db_path: Path) -> str:
    ...
    from runtime_support.backup import _format_size
...
def _report_database_journal_modes(
    marvi_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from marvi_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = marvi_home if marvi_home is not None else MARVI_MESSAGING_HOME
    try:
        databases = _marvi_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Marvi databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.marvi/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("MARVI_MESSAGING_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from runtime_support.auth import get_minimax_oauth_auth_status
...
from runtime_support.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'marvi sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. MARVI_MESSAGING_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("MARVI_MESSAGING_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("MARVI_MESSAGING_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches runtime_support.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``marvi --version`` reporting a stale version while
    ...
    """
    try:
        from runtime_support import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != runtime_support/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'marvi update', or set "
            "runtime_support/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-marvi and dashboard static services are up
    ...
    for static in ("main-marvi", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `marvi profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `marvi doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from runtime_support.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the MARVI_MESSAGING_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("MARVI_MESSAGING_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via MARVI_MESSAGING_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `marvi`.
    os.environ.setdefault("MARVI_MESSAGING_INTERACTIVE", "1")

    # Handle `marvi doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.marvi/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Marvi Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`marvi doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from marvi_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Marvi already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and runtime_support/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.marvi/.env (primary location for user config)
    env_path = MARVI_MESSAGING_HOME / '.env'
    if env_path.exists():
        ...
        # matches runtime_support.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'marvi setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'marvi setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'marvi setup' to create one")
                issues.append("Run 'marvi setup' to create .env")

    # Check ~/.marvi/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = MARVI_MESSAGING_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from runtime_support.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from runtime_support.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'marvi config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from runtime_support.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.marvi/.env or run 'marvi setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'marvi setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'marvi config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from runtime_support.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from runtime_support.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'marvi setup' to migrate config")
                else:
                    issues.append("Run 'marvi doctor --fix' or 'marvi setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from runtime_support.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'marvi doctor --fix'")
        except Exception:
            pass

        # Detect stale MARVI_MESSAGING_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives MARVI_MESSAGING_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from runtime_support.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("MARVI_MESSAGING_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"MARVI_MESSAGING_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `marvi setup` run)",
                )
                if should_fix:
                    if remove_env_value("MARVI_MESSAGING_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale MARVI_MESSAGING_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove MARVI_MESSAGING_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the MARVI_MESSAGING_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale MARVI_MESSAGING_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'marvi doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from runtime_support.config import load_env as _load_env_depr
            from runtime_support.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Marvi' own device-code flow — the Codex CLI is
    ...
    marvi_home = MARVI_MESSAGING_HOME
    if marvi_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        marvi_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = marvi_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Marvi a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Marvi Messaging Persona\n\n"
    ...
    memories_dir = marvi_home / "memories"
    ...
    state_db_path = marvi_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from marvi_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'marvi doctor --fix' "
                        "(or 'marvi sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from marvi_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'marvi doctor --fix' "
                        "(or 'marvi sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Marvi
        ...
        try:
            from marvi_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'marvi sessions optimize-storage' "
    ...
    wal_path = marvi_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'marvi doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "marvi"
        ...
        _cmd_link = _cmd_link_dir / "marvi"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(marvi not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/marvi → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/marvi points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/marvi → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/marvi — run 'marvi doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/marvi exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/marvi not found",
                    "(marvi command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/marvi → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/marvi symlink — run 'marvi doctor --fix'")
    ...
    try:
        from marvi_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'marvi-messaging[vercel]')",
                "Install the Vercel optional dependency: pip install 'marvi-messaging[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/Marvi-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `marvi update` does, so a session's first browser call
            ...
            # `marvi update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `marvi doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable MARVI_MESSAGING_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Marvi one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'marvi config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _MARVI_MESSAGING_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``marvi doctor``.
    ...
    # never the legitimate source for `marvi doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'marvi setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")

    _section("Skills Hub")
    hub_dir = MARVI_MESSAGING_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: marvi skills list)")
    ...
    try:
        from runtime_support.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = MARVI_MESSAGING_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: marvi memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: marvi memory setup",
                    "No Honcho API key — run 'marvi memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run marvi memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: marvi memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: marvi memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from runtime_support.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "marvi -p" in content:
                            _m = _re.search(r"marvi -p (\S+)", content)
    ...
    # `marvi doctor --live` (real network calls; bounded + read-only).
    try:
        from runtime_support.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'marvi doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
