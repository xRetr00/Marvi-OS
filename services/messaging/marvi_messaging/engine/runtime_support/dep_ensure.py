...
from marvi_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import marvi_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $MARVI_MESSAGING_HOME/node is
    ...
    "browser": lambda: (
        ...
        or _has_marvi_messaging_browser()
...
def _has_marvi_messaging_browser() -> bool:
    ...
    # install.sh installs globally into $MARVI_MESSAGING_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-MarviHome", str(get_marvi_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = marvi_subprocess_env(inherit_credentials=False)
