...
Why this exists
---------------
``runtime_support/web_server.py`` owns all dashboard runtime state: the ephemeral
...
``APIRouter`` modules under ``runtime_support/web_routers/`` need those helpers, but
...
thin proxy that resolves ``runtime_support.web_server.<name>`` *at call time*.  This
...
def _server():
    """Return the live ``runtime_support.web_server`` module (imported on demand)."""
    mod = sys.modules.get("runtime_support.web_server")
    if mod is None:  # pragma: no cover - routers are only mounted by web_server
        import runtime_support.web_server as mod  # type: ignore[no-redef]
    return mod
