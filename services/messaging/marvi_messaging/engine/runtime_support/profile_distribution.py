"""Profile distributions — shareable, packaged Marvi profiles via git.

A distribution is a Marvi profile published as a git repository (or
...
* ``marvi profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``marvi skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``marvi profile``, not a parallel tree):

    marvi profile install <source> [--name N] [--alias] [--force] [--yes]
    marvi profile update  <name>  [--force-config] [--yes]
    marvi profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "marvi-messaging", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            marvi_requires=str(data.get("marvi_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.marvi_requires:
            out["marvi_requires"] = self.marvi_requires
        if self.author:
...
def check_marvi_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Marvi {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Marvi distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``marvi profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Marvi profile distribution."
            )
...
def plan_install(
    ...
    from runtime_support import __version__ as marvi_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Marvi distribution."
        )

    # Version check up-front so we fail fast
    check_marvi_requires(manifest.marvi_requires, marvi_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.marvi).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="marvi_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `marvi profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`marvi profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="marvi_dist_update_") as tmp:
        plan = plan_install(
