...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``marvi model`` CLI picker. Adds network calls
    ...
    """
    from runtime_support.model_switch import list_authenticated_providers
    ...
    try:
        from runtime_support.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``runtime_support/moa_cmd.py``.
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from runtime_support.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `marvi model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Marvi.
    """
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `marvi model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from runtime_support.models import CANONICAL_PROVIDERS
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`runtime_support.model_switch.list_picker_providers`) so the
