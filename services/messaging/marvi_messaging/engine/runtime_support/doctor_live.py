"""``marvi doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from runtime_support.doctor import (
    _section,
    check_fail,
    check_info,
    check_ok,
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from runtime_support.doctor import MARVI_MESSAGING_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (MARVI_MESSAGING_HOME / "node" / "bin",
                          MARVI_MESSAGING_HOME / "node",
                          MARVI_MESSAGING_HOME / "node_modules" / ".bin"):
    ...
    # runtime_support.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``marvi mcp test``.
    """
