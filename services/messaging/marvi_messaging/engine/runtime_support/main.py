...
Usage:
    marvi                     # Interactive chat (default)
    marvi chat                # Interactive chat
    marvi gateway             # Run gateway in foreground
    marvi gateway start       # Start gateway as service
    marvi gateway stop        # Stop gateway service
    marvi gateway status      # Show gateway status
    marvi gateway install     # Install gateway service
    marvi gateway uninstall   # Uninstall gateway service
    marvi setup               # Interactive setup wizard
    marvi logout              # Clear stored authentication
    marvi status              # Show status of all components
    marvi cron                # Manage cron jobs
    marvi cron list           # List cron jobs
    marvi cron status         # Check if cron scheduler is running
    marvi doctor              # Check configuration and dependencies
    marvi honcho setup                    # Configure Honcho AI memory integration
    marvi honcho status                   # Show Honcho config and connection status
    marvi honcho sessions                 # List directory → session name mappings
    marvi honcho map <name>               # Map current directory to a session name
    marvi honcho peer                     # Show peer names and dialectic settings
    marvi honcho peer --user NAME         # Set user peer name
    marvi honcho peer --ai NAME           # Set AI peer name
    marvi honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    marvi honcho mode                     # Show current memory mode
    marvi honcho mode [hybrid|honcho|local]  # Set memory mode
    marvi honcho tokens                   # Show token budget settings
    marvi honcho tokens --context N       # Set session.context() token cap
    marvi honcho tokens --dialectic N     # Set dialectic result char cap
    marvi honcho identity                 # Show AI peer identity representation
    marvi honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    marvi honcho migrate                  # Step-by-step migration guide: OpenClaw native → Marvi + Honcho
    marvi version             Show version
    marvi update              Update to latest version
    marvi uninstall           Uninstall Marvi Messaging
    marvi acp                 Run as an ACP server for editor integration
    marvi sessions browse     Interactive session picker with search

    marvi claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: marvi_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``marvi_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``marvi update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``marvi_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, marvi crashes on import and the user can't run
# ``marvi update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import marvi_bootstrap  # noqa: F401
...
from runtime_support._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python runtime_support/main.py`` (script
# mode — sys.path[0] is runtime_support/, not the repo root) can import the
# canonical helpers; everything else lives in runtime_support._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from runtime_support import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``marvi update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from runtime_support.env_loader import ...`` / ``from runtime_support.config import
...
# package directory, so if IT can't import, nothing else in runtime_support can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from runtime_support import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from runtime_support.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'marvi' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``marvi tools`` or
...
# that run *before* runtime_support.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("MARVI_MESSAGING_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".marvi", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``MARVI_MESSAGING_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``marvi … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("MARVI_MESSAGING_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``MARVI_MESSAGING_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("MARVI_MESSAGING_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`marvi --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(marvi_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(marvi_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``marvi --version`` before config/logging imports."""
...
from runtime_support.sessions_cmd import cmd_sessions  # noqa: F401
from runtime_support.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from runtime_support.subcommands.cron import build_cron_parser
from runtime_support.subcommands.sync import build_sync_parser
from runtime_support.subcommands.gateway import build_gateway_parser
from runtime_support.subcommands.profile import build_profile_parser
from runtime_support.subcommands.model import build_model_parser
from runtime_support.subcommands.setup import build_setup_parser

from runtime_support.subcommands.whatsapp import build_whatsapp_parser
from runtime_support.subcommands.slack import build_slack_parser
from runtime_support.subcommands.login import build_login_parser
from runtime_support.subcommands.logout import build_logout_parser
from runtime_support.subcommands.auth import build_auth_parser
from runtime_support.subcommands.status import build_status_parser
from runtime_support.subcommands.pause import build_pause_parser
from runtime_support.subcommands.webhook import build_webhook_parser
from runtime_support.subcommands.hooks import build_hooks_parser
from runtime_support.subcommands.doctor import build_doctor_parser
from runtime_support.subcommands.verify import build_verify_parser
from runtime_support.subcommands.security import build_security_parser
from runtime_support.subcommands.approvals import build_approvals_parser
from runtime_support.subcommands.dump import build_dump_parser
from runtime_support.subcommands.debug import build_debug_parser
from runtime_support.subcommands.backup import build_backup_parser
from runtime_support.subcommands.import_cmd import build_import_cmd_parser
from runtime_support.subcommands.import_agent import build_import_agent_parser
from runtime_support.subcommands.config import build_config_parser
from runtime_support.subcommands.skin import build_skin_parser
from runtime_support.subcommands.console import build_console_parser
from runtime_support.subcommands.version import build_version_parser
from runtime_support.subcommands.update import build_update_parser
from runtime_support.subcommands.uninstall import build_uninstall_parser
from runtime_support.subcommands.dashboard import build_dashboard_parser
from runtime_support.subcommands.gui import build_gui_parser
from runtime_support.subcommands.logs import build_logs_parser
from runtime_support.subcommands.prompt_size import build_prompt_size_parser
from runtime_support.subcommands.memory import build_memory_parser
from runtime_support.subcommands.acp import build_acp_parser
from runtime_support.subcommands.tools import build_tools_parser
from runtime_support.subcommands.insights import build_insights_parser
from runtime_support.subcommands.monitoring import build_monitoring_parser
from runtime_support.subcommands.skills import build_skills_parser
from runtime_support.subcommands.pairing import build_pairing_parser
from runtime_support.subcommands.plugins import build_plugins_parser
from runtime_support.subcommands.mcp import build_mcp_parser
from runtime_support.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (marvi tools, marvi setup, marvi model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'marvi {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any marvi module import.
#
# Many modules cache MARVI_MESSAGING_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("MARVI_MESSAGING_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.marvi/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set MARVI_MESSAGING_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `marvi mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Marvi' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo marvi -p <name>` against the invoking user's home.
        ...
        candidate = home / ".marvi" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`marvi chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors runtime_support.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If MARVI_MESSAGING_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.marvi/profiles/coder or
    # /opt/data/profiles/coder).  If MARVI_MESSAGING_HOME points to the marvi root
    # instead (e.g. systemd hardcodes MARVI_MESSAGING_HOME=/root/.marvi), we must
    # still read active_profile — the user may have switched profiles via
    # `marvi profile use` and the gateway should honour that choice.
    # See issue #22502.
    marvi_home_env = os.environ.get("MARVI_MESSAGING_HOME", "")
    if profile_name is None and marvi_home_env:
        if Path(marvi_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the marvi root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as MARVI_MESSAGING_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``marvi gateway run`` to mean
    # "the root MARVI_MESSAGING_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("MARVI_MESSAGING_S6_SUPERVISED_CHILD"):
        try:
            from marvi_constants import get_default_marvi_root

            active_path = get_default_marvi_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set MARVI_MESSAGING_HOME
    if profile_name is not None:
        try:
            from runtime_support.profiles import resolve_profile_env

            marvi_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            marvi_home = _resolve_sudo_user_profile_env(profile_name)
            if not marvi_home:
            ...
            # A bug in profiles.py must NEVER prevent marvi from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["MARVI_MESSAGING_HOME"] = marvi_home
...
# Load .env from ~/.marvi/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from runtime_support.config import get_marvi_home
from runtime_support.env_loader import load_marvi_dotenv

load_marvi_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → MARVI_MESSAGING_REDACT_SECRETS env
# var BEFORE marvi_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves marvi_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from runtime_support.config import read_raw_config as _read_raw_early

    _cfg_path = get_marvi_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "MARVI_MESSAGING_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["MARVI_MESSAGING_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `marvi` subcommands
...
try:
    from marvi_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from marvi_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if marvi_constants not importable yet
...
from runtime_support import __version__, __release_date__

# Provider model-selection wizard flows extracted to runtime_support/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (runtime_support.main._model_flow_*) keep resolving unchanged.
from runtime_support.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_marvi_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("MARVI_MESSAGING_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("MARVI_MESSAGING_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`runtime_support.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from runtime_support.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from runtime_support.config import get_env_path, get_marvi_home, load_config
    from runtime_support.auth import get_auth_status

    # Determine whether Marvi itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from runtime_support.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_marvi_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from runtime_support.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_marvi_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Marvi has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Marvi to use their tokens.
    if _has_marvi_config:
        try:
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from runtime_support.model_selection_guards import combined_selection_warning
    except Exception as exc:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``marvi -c`` from repo A continues repo A's last session rather than the
    ...
    db = None
    try:
        from marvi_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, marvi_bin
        cli_args: the original CLI arguments (everything after 'marvi')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    marvi_bin = container_info["marvi_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo marvi {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo marvi {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  marvi --tui --resume {target}")
    if title:
        print(f'  marvi --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @marvi/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@marvi" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/marvi-ink/package.json",
    "packages/marvi-ink/index.js",
    "packages/marvi-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``MARVI_MESSAGING_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("MARVI_MESSAGING_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``MARVI_MESSAGING_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("MARVI_MESSAGING_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "MARVI_MESSAGING_HOME": marvi_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(marvi_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(runtime_support_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if runtime_support_dir is None:
        runtime_support_dir = Path(__file__).parent
    bundled = runtime_support_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``marvi update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("MARVI_MESSAGING_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Marvi checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `marvi update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Marvi checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `marvi --tui`\n"
        "If the checkout is still inconsistent, run `marvi update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (MARVI_MESSAGING_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("MARVI_MESSAGING_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $MARVI_MESSAGING_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Marvi installed,
        # and would pick a system Node over the managed one when both exist.
        from marvi_constants import find_node_executable

        path = find_node_executable(bin)
        if not path and bin == "node":
            try:
                from runtime_support.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("MARVI_MESSAGING_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with MARVI_MESSAGING_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset MARVI_MESSAGING_TUI_DIR (e.g. `unset MARVI_MESSAGING_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # runtime_support/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from marvi_constants import with_marvi_node_path
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env={**with_marvi_node_path(), "CI": "1"},
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Marvi-managed
            ...
            from runtime_support.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @marvi/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @marvi/ink resolves through
        # packages/marvi-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "marvi-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from runtime_support.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("MARVI_MESSAGING_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["MARVI_MESSAGING_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("MARVI_MESSAGING_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["MARVI_MESSAGING_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("MARVI_MESSAGING_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["MARVI_MESSAGING_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["MARVI_MESSAGING_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a marvi process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from runtime_support.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="marvi-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["MARVI_MESSAGING_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["MARVI_MESSAGING_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["MARVI_MESSAGING_MODEL"] = model
        env["MARVI_MESSAGING_INFERENCE_MODEL"] = model
    if provider:
        env["MARVI_MESSAGING_TUI_PROVIDER"] = provider
        env["MARVI_MESSAGING_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["MARVI_MESSAGING_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["MARVI_MESSAGING_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["MARVI_MESSAGING_TUI_SKILLS"] = value
    if query:
        env["MARVI_MESSAGING_TUI_QUERY"] = query
    if image:
        env["MARVI_MESSAGING_TUI_IMAGE"] = image
    if checkpoints:
        env["MARVI_MESSAGING_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["MARVI_MESSAGING_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["MARVI_MESSAGING_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["MARVI_MESSAGING_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["MARVI_MESSAGING_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["MARVI_MESSAGING_ACCEPT_HOOKS"] = "1"
    ...
    # MARVI_MESSAGING_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `marvi --tui` try to
    ...
    # still set MARVI_MESSAGING_TUI_RESUME themselves.
    env.pop("MARVI_MESSAGING_TUI_RESUME", None)
    if resume_session_id:
        env["MARVI_MESSAGING_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `marvi update` so
    ...
    if code == 42:
        from runtime_support.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``MARVI_MESSAGING_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``marvi kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``marvi kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("MARVI_MESSAGING_KANBAN_BOARD"):
        return
    try:
        from runtime_support.kanban_db import get_current_board

        os.environ["MARVI_MESSAGING_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.marvi/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Marvi — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Marvi still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      4. ``MARVI_MESSAGING_TUI=1`` env   → TUI
    ...
    workers, cron jobs, and pipelines run ``marvi … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("MARVI_MESSAGING_TUI") == "1":
        return True
    try:
        from runtime_support.config import load_config
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'marvi sessions list' to see available sessions.")
            sys.exit(1)
    ...
    try:
        from runtime_support.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from runtime_support.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'marvi doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Marvi isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  marvi setup")
        print()

        from runtime_support.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'marvi setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from runtime_support.banner import prefetch_banner_data, prefetch_update_check
    ...
    # rare launch right after `marvi update` changed a bundled skill.
    ...
    if getattr(args, "yolo", False):
        os.environ["MARVI_MESSAGING_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.marvi/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["MARVI_MESSAGING_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["MARVI_MESSAGING_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["MARVI_MESSAGING_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from runtime_support.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from runtime_support.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from runtime_support.config import get_env_value, save_env_value
    from marvi_constants import find_node_executable, with_marvi_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Marvi?")
        print()
    ...
    # has no creds.json.  Every subsequent `marvi gateway` then paid a 30s
    ...
    session_dir = get_marvi_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: marvi gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `marvi gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  marvi gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with 'Marvi Messaging'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  marvi gateway")
        ...
        print()
        print("  Or install as a service: marvi gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'marvi whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``marvi whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``runtime_support/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from runtime_support.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from runtime_support.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from runtime_support.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``marvi model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from runtime_support.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from runtime_support.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("MARVI_MESSAGING_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from runtime_support.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'marvi model' for "
                "available providers, or run 'marvi doctor' to diagnose config "
                "issues."
            )
    ...
    from runtime_support.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        _PROVIDER_ALIASES,
        group_providers,
    ...
    # PROVIDER_GROUPS in runtime_support/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``marvi model``
    ...
    # matching against marvi_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    # a leftover OPENAI_BASE_URL in ~/.marvi/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.marvi/.env if the active provider is not 'custom'.
    ...
    """
    from runtime_support.config import get_env_value, save_env_value, load_config
...
#
# Marvi uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `marvi model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `marvi model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`runtime_support.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from runtime_support.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Marvi only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``marvi model`` flow, then route aux tasks
    to them here.
    """
    from runtime_support.config import load_config
    from runtime_support.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from runtime_support.auth import _prompt_model_selection
    from runtime_support.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from runtime_support.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from runtime_support.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from runtime_support.runtime_provider import _detect_api_mode_for_url
...
def _remove_custom_provider(config):
    ...
    try:
        from runtime_support.curses_ui import curses_radiolist
...
# downstream call sites read `runtime_support.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `marvi` invocation — including
# fast paths like `marvi --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from runtime_support.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``marvi setup`` / ``marvi model``.
    ...
    recover from a malformed paste without editing ``~/.marvi/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from runtime_support.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from runtime_support.config import save_env_value
    from runtime_support.secret_prompt import masked_secret_prompt
    ...
    from runtime_support.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `marvi setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from marvi_constants import display_marvi_home as _dhh_fn

            print(
                f"    Marvi will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    ...
    from runtime_support.auth import login_command
...
def cmd_logout(args):
    ...
    from runtime_support.auth import logout_command
...
def cmd_auth(args):
    ...
    from runtime_support.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from runtime_support.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from runtime_support.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.marvi/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `marvi sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set MARVI_MESSAGING_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "MARVI_MESSAGING_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    if not ssc.resolve_sync_base_url():
        print(
            ...
            "or MARVI_MESSAGING_SYNC_BASE_URL).",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="marvi sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="marvi sync now")
...
def cmd_webhook(args):
    ...
    from runtime_support.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``marvi slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: marvi slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `marvi slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from runtime_support.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from runtime_support.kanban import kanban_command
...
def cmd_project(args):
    ...
    from runtime_support.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from runtime_support.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from runtime_support.doctor import run_doctor
...
def cmd_verify(args):
    ...
    from runtime_support.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `marvi security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from runtime_support.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `marvi approvals <subcmd>`."""
    from runtime_support.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from runtime_support.dump import run_dump
...
def cmd_debug(args):
    ...
    from runtime_support.debug import run_debug
...
def cmd_config(args):
    ...
    from runtime_support.config import config_command
...
def cmd_skin(args):
    ...
    from runtime_support.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Marvi home directory to a zip file."""
    if getattr(args, "quick", False):
        from runtime_support.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from runtime_support.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Marvi backup from a zip file."""
    from runtime_support.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from runtime_support.config import detect_install_method
    from runtime_support.slash_exec import CommandContext, execute_command
    ...
    try:
        from runtime_support.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
        from runtime_support.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Marvi Messaging (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from runtime_support.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from runtime_support.uninstall import run_gui_uninstall
    ...
    from runtime_support.uninstall import run_uninstall
...
# Update pipeline extracted to runtime_support/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (runtime_support.main._cmd_update_impl,
# runtime_support.main._run_pre_update_backup, ...) keep resolving unchanged.
from runtime_support.update_cmd import (  # noqa: F401
...
# validated against. Lives next to the checkout (NOT in MARVI_MESSAGING_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``marvi update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``marvi update``
    ...
    ``marvi`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``marvi update``
    ...
    ``runtime_support/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "runtime_support" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $MARVI_MESSAGING_HOME."""
    from marvi_constants import get_marvi_home
    return get_marvi_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``marvi`` launcher present but ``runtime_support`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the marvi venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — marvi venv python3, no nix-shell overhead
    ...
    # a marvi venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``marvi update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Marvi-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_marvi_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "runtime_support" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``marvi web``).
    ...
    # build path usable on a stock `py -m runtime_support.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_marvi_node_path()
    ...
    # as `marvi update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (marvi web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``marvi update`` can unconditionally call ``marvi desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``marvi desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $MARVI_MESSAGING_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $MARVI_MESSAGING_HOME."""
    from marvi_constants import get_marvi_home
    return get_marvi_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Marvi.app/Contents/MacOS/Marvi → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``marvi desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``marvi update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = [
            path
            for product_name in ("Marvi", "Marvi")
        ...
        candidates = [
            ...
            for executable_name in ("Marvi.exe", "Marvi.exe")
        ]
    else:
        candidates = [
            release_dir / unpacked_dir / executable_name
            for unpacked_dir in ("linux-unpacked", "linux-arm64-unpacked")
            for executable_name in ("Marvi", "marvi", "Marvi", "marvi")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Marvi.exe
...
#
# The desktop self-update chain (Desktop → marvi-setup --update →
# `marvi update` → `marvi desktop --build-only` → relaunch) rebuilds
# Marvi.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Marvi.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Marvi.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 marvi-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Marvi.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Marvi.exe from backup.")
        print("    Your existing version was kept and still works. Run `marvi desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `marvi desktop --force-build` to rebuild, or re-run the Marvi")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Marvi'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_marvi_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Marvi.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Marvi.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Marvi.exe: Access is
    ...
    an unrelated "Marvi" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Marvi.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Marvi is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Marvi.app/Contents/MacOS/Marvi  ->  app bundle = .../Marvi.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``marvi desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Marvi Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Marvi Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `marvi desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the MARVI_MESSAGING_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Marvi Desktop (Linux only, best-effort).
    ...
    shell. ``marvi uninstall --gui`` removes it.
    """
    try:
        from runtime_support.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from marvi_logging import setup_logging as _setup_logging_gui
    ...
    # with_marvi_node_path() copies os.environ when called with no arg.
    env = with_marvi_node_path()
    if getattr(args, "fake_boot", False):
        env["MARVI_MESSAGING_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["MARVI_MESSAGING_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "marvi_root", None):
        env["MARVI_MESSAGING_DESKTOP_MARVI_MESSAGING_ROOT"] = str(Path(args.marvi_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["MARVI_MESSAGING_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["MARVI_MESSAGING_DESKTOP_CWD"] = os.getcwd()
    ...
    # `MARVI_MESSAGING_DESKTOP_DISABLE_GPU=... marvi desktop` keeps working.
    config_electron_flags, config_disable_gpu, config_password_store = _desktop_launch_options()
    if config_disable_gpu != "auto" and "MARVI_MESSAGING_DESKTOP_DISABLE_GPU" not in os.environ:
        env["MARVI_MESSAGING_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `MARVI_MESSAGING_DESKTOP_PASSWORD_STORE=... marvi desktop` keeps working.
    if sys.platform == "linux" and "MARVI_MESSAGING_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["MARVI_MESSAGING_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  marvi gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Marvi-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → marvi-setup →
            # marvi update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `marvi update` path).
            nixos_env = with_marvi_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Marvi.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Marvi.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    ...
                    print("  running Marvi desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> marvi desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Marvi is
                ...
                # success on a Marvi.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Marvi shows up
    ...
    if source_mode:
        print("→ Launching Marvi Desktop from source build...")
    ...
    print(f"→ Launching packaged Marvi Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers extracted to runtime_support/dashboard_procs.py
# (main.py decomposition, mechanical move). Re-exported so callers and test
# monkeypatches on runtime_support.main.<name> keep resolving unchanged.
from runtime_support.dashboard_procs import (  # noqa: F401
    _detect_concurrent_marvi_instances,
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "marvi dashboard",
            "runtime_support.main dashboard",
            "runtime_support/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "marvi serve",
            "runtime_support.main serve",
            "runtime_support/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "marvi-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Marvi installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``marvi-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/marvi-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but marvi launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``marvi update``.
    ...
    log_path = get_marvi_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `marvi update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $MARVI_MESSAGING_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``marvi update``.
    ...
    protocol stream (``marvi acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `marvi update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``marvi.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_marvi_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from marvi.exe; applying package-only first aid, "
    ...
    try:
        from runtime_support.managed_uv import ensure_uv
        ...
        if self_locked:
            print(
                "  Marvi is still running from the launcher that needs "
                "replacing. Close other Marvi windows, restart from a "
...
def _windows_running_marvi_launcher_locked() -> bool:
    """True when a venv ``marvi*.exe`` shim is this process or an ancestor.
    ...
    if scripts_dir is None:
        return False
    shims = _marvi_exe_shims(scripts_dir)
    if not shims:
        return False
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``marvi update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from marvi_constants import venv_bin_dir

    scripts = venv_bin_dir(venv_dir, windows=_is_windows())
    return scripts if scripts.is_dir() else None


def _marvi_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_marvi_exe(
    ...
    when ``marvi update`` runs, ``marvi.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``marvi.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next marvi invocation by ``_cleanup_quarantined_exes``.
    ...
    3. Print a clear warning naming the most likely culprit (running
       Marvi Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _marvi_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Marvi Desktop, exit other `marvi` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `marvi update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_marvi_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``marvi.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``marvi.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``marvi`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_marvi_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``marvi.exe.old.*`` left by prior updates.

    Called early on every marvi invocation. The .old files are unlocked once
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``marvi.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``marvi.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `marvi update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``marvi.exe`` / ``marvi-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_marvi_exe`` for the rationale.
    """
    ...
    # hours later inside ``marvi update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``marvi.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``marvi.exe`` shim is locked during ``marvi update``, or when uv/distlib
    skips a launcher write. The symptom is ``marvi-messaging.exe`` and
    ``marvi-acp.exe`` present but ``marvi.exe`` missing, so ``marvi`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `marvi update --force` after "
            "closing other marvi processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m runtime_support.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``marvi update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``marvi.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``marvi`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `marvi update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `marvi update --force` after closing other marvi processes."
        )
        return
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from marvi_constants import venv_python_path
...
class _UpdateOutputStream:
    """Stream wrapper used during ``marvi update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.marvi/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``marvi update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``marvi update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.marvi/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``marvi update --gateway``) the update is already
    ...
    try:
        ...
        # runtime_support.config.get_marvi_home to simulate setup failure.
        from runtime_support.config import get_marvi_home as _get_marvi_home

        logs_dir = _get_marvi_home() / "logs"
...
def cmd_update(args):
    """Update Marvi Messaging to the latest version.
    ...
    if is_managed():
        managed_error("update Marvi Messaging")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `marvi update --branch=<x>` would actually pull.
    ...
    from runtime_support.update_lock import (
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``marvi -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from runtime_support.profiles import (
        list_profiles,
        create_profile,
        delete_profile,
        seed_profile_skills,
    ...
    from marvi_constants import display_marvi_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `marvi profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_marvi_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → marvi -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.marvi)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  marvi profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     marvi -p {name} chat")
                else:
        ...
        from runtime_support import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from marvi_constants import get_marvi_home as _hh
        ...
        from runtime_support import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `marvi profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → marvi -p {name}  ({wrapper})")
        print()
        ...
        from runtime_support.profiles import profile_exists, validate_alias_name
        ...
        from runtime_support.profiles import rename_profile
        ...
        from runtime_support.profiles import export_profile
        ...
        from runtime_support.profiles import import_profile
        ...
        import tempfile
        from runtime_support.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="marvi_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  marvi -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      marvi -p {plan.manifest.name} chat")
        ...
        from runtime_support.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `marvi profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"marvi -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from runtime_support.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("marvi_requires"):
            print(f"Requires:     Marvi {data['marvi_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from runtime_support.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.marvi_requires:
        print(f"  Requires: Marvi {mf.marvi_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No marvi dashboard processes running.")
        return 0

    print(f"{len(live)} marvi dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``marvi dashboard register`` for OAuth.
    ...
    try:
        from runtime_support.web_server import should_require_auth
    ...
    try:
        from runtime_support.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `marvi dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    marvi dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "MARVI_MESSAGING_DASHBOARD_OAUTH_CLIENT_ID into ~/.marvi/.env for you.\n"
    ...
    try:
        from runtime_support.config import load_config, save_config
        from runtime_support.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from runtime_support.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.marvi/desktop-ssh: a
    # literal "~/.marvi/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of MARVI_MESSAGING_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_marvi_home(): a non-default sticky profile (or any MARVI_MESSAGING_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_marvi_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".marvi" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``MARVI_MESSAGING_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``marvi dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No marvi dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `marvi update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with marvi serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with MARVI_MESSAGING_DESKTOP=1 plus
    # MARVI_MESSAGING_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # MARVI_MESSAGING_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `marvi dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited MARVI_MESSAGING_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # MARVI_MESSAGING_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (MARVI_MESSAGING_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets MARVI_MESSAGING_SERVE_HEADLESS below.
    if os.environ.get("MARVI_MESSAGING_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("MARVI_MESSAGING_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("MARVI_MESSAGING_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("MARVI_MESSAGING_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → MARVI_MESSAGING_HOME points into
    ...
    try:
        from runtime_support.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("MARVI_MESSAGING_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "runtime_support.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: MARVI_MESSAGING_HOME is explicitly pinned to the
        ...
        # MARVI_MESSAGING_HOME.  We must resolve the root explicitly instead of just
        # dropping MARVI_MESSAGING_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV MARVI_MESSAGING_HOME=/opt/data`), so an unset
        # MARVI_MESSAGING_HOME falls back to $HOME/.marvi = /opt/data/.marvi — an
        ...
        # update-button guard also misfires).  get_default_marvi_root()
        # returns the root for both layouts: ~/.marvi for a standard install
        ...
        try:
            from marvi_constants import get_default_marvi_root
            env["MARVI_MESSAGING_HOME"] = str(get_default_marvi_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop MARVI_MESSAGING_HOME) rather than block the reroute.
            env.pop("MARVI_MESSAGING_HOME", None)
    ...
    from runtime_support.resource_limits import apply_nofile_soft_limit
    ...
    # the same logs directory as every other Marvi surface.
    try:
        from marvi_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["MARVI_MESSAGING_SERVE_HEADLESS"] = "1"
    elif "MARVI_MESSAGING_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["MARVI_MESSAGING_WEB_DIST"])
            if "MARVI_MESSAGING_WEB_DIST" in os.environ
            else PROJECT_ROOT / "runtime_support" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom MARVI_MESSAGING_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "MARVI_MESSAGING_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # MARVI_MESSAGING_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["MARVI_MESSAGING_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ MARVI_MESSAGING_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset MARVI_MESSAGING_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads MARVI_MESSAGING_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["MARVI_MESSAGING_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from MARVI_MESSAGING_WEB_DIST: {_dist_root}")
    ...
    try:
        from runtime_support.mcp_startup import start_background_mcp_discovery
    ...
    from runtime_support.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from runtime_support.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from runtime_support.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from runtime_support.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from runtime_support.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Marvi log files."""
    from runtime_support.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Marvi command console."""
    from runtime_support.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from runtime_support.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``marvi -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``runtime_support/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``marvi -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``marvi`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``marvi <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``marvi photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("MARVI_MESSAGING_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that MARVI_MESSAGING_YOLO_MODE is set before ANY
    ...
    try:
        from runtime_support.plugins import start_background_plugin_discovery
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["MARVI_MESSAGING_SAFE_MODE"] = "1"
    os.environ["MARVI_MESSAGING_IGNORE_USER_CONFIG"] = "1"
    os.environ["MARVI_MESSAGING_IGNORE_RULES"] = "1"
...
def _try_fast_chat_launch() -> bool:
    ...
    ``marvi`` / ``marvi -w -s foo --yolo`` / ``marvi chat`` don't need the
    ...
    """
    if os.environ.get("MARVI_MESSAGING_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from runtime_support.config import get_container_exec_info
    ...
    from runtime_support._parser import build_top_level_parser
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("MARVI_MESSAGING_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["MARVI_MESSAGING_DEFER_AGENT_STARTUP"] = "1"
            os.environ["MARVI_MESSAGING_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["MARVI_MESSAGING_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `marvi --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from marvi_constants import get_marvi_home, display_marvi_home

        mem_dir = get_marvi_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_marvi_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_marvi_home()}/memories/\n")
    else:
        from runtime_support.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Marvi Messaging as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from runtime_support.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from runtime_support.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from runtime_support.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: marvi-messaging[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from runtime_support.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from runtime_support.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from runtime_support.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from runtime_support.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from runtime_support.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from runtime_support.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    (``marvi-messaging`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``MARVI_MESSAGING_AGENT`` is the Marvi-specific marker. setdefault: never
    clobber an outer harness (e.g. Marvi running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "marvi-messaging")
    os.environ.setdefault("MARVI_MESSAGING_AGENT", "true")


def main():
    """Main entry point for marvi CLI."""
    # Cosmetic: make the process show up as 'marvi' instead of 'python3.11'
    ...
    try:
        from runtime_support.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``marvi.exe.old.*`` quarantine files left by previous
    # ``marvi update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_marvi_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (marvi update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``marvi update``
    ...
    # ``marvi skills install update``) merely defers recovery one launch;
    # under-matching (missing ``marvi -p work update``) would race a recovery
    ...
    chat_parser.set_defaults(func=cmd_chat)
    from runtime_support.presence_cmd import add_parser as add_presence_parser
    add_presence_parser(subparsers)

    # =========================================================================
    # model command  (parser built in runtime_support/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from runtime_support.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from runtime_support.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `marvi model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.marvi/.env.  Supports Bitwarden "
    ...
    from runtime_support import secrets_cli as _secrets_cli
    from runtime_support import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `marvi proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    from runtime_support import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``marvi proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from runtime_support.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in runtime_support/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in runtime_support/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in runtime_support/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `marvi whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in runtime_support/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from runtime_support.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in runtime_support/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in runtime_support/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in runtime_support/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in runtime_support/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in runtime_support/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in runtime_support/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in runtime_support/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from runtime_support.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from runtime_support.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from runtime_support.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in runtime_support/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in runtime_support/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in runtime_support/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in runtime_support/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in runtime_support/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in runtime_support/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in runtime_support/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in runtime_support/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.marvi/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo marvi uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from runtime_support.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in runtime_support/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: runtime_support/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from runtime_support.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in runtime_support/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in runtime_support/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in runtime_support/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in runtime_support/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in runtime_support/subcommands/skills.py)
    # =========================================================================
    ...
    from runtime_support.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in runtime_support/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``marvi --help``, ``marvi version``, ``marvi logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from runtime_support.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from runtime_support.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Marvi shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from runtime_support.pets import register_cli as _register_pets_cli
    ...
    try:
        from runtime_support.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in runtime_support/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in runtime_support/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `marvi computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `marvi tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `marvi computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Marvi app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from runtime_support.tools_config import install_cua_driver
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from runtime_support.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: marvi computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: marvi computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: marvi computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: marvi computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: marvi computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: marvi computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in runtime_support/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <marvi home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    # cmd_sessions lives in runtime_support/sessions_cmd.py (main.py decomposition).
    ...
    # =========================================================================
    # insights command  (parser built in runtime_support/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in runtime_support/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in runtime_support/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in runtime_support/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in runtime_support/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in runtime_support/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in runtime_support/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in runtime_support/subcommands/dashboard.py)
    # =========================================================================
    ...
    # run `marvi desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in runtime_support/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in runtime_support/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in runtime_support/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``marvi -c Pokemon Agent Dev`` → ``marvi -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'marvi -c model' where 'model' is consumed as the
    ...
    # --yolo: set MARVI_MESSAGING_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (marvi hooks list, cron
    ...
    # ``marvi egress start`` refusing when credential_source=bitwarden
