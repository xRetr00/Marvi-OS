"""`marvi checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.marvi/checkpoints/``.  Actions:

    marvi checkpoints               # same as `status`
    marvi checkpoints status        # total size, project count, breakdown
    marvi checkpoints list          # per-project checkpoint counts + workdir
    marvi checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    marvi checkpoints clear [-f]    # nuke the entire base (asks first)
    marvi checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    marvi checkpoints
    marvi checkpoints prune --retention-days 3 --max-size-mb 200
    marvi checkpoints clear -f
...
from runtime_support.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: marvi checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``marvi checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `marvi checkpoints` → status
