"""On-demand supply-chain audit for Marvi Messaging installs.

Scans three surfaces a Marvi user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Marvi venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.marvi/plugins``
...
def _discover_plugins(marvi_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.marvi/plugins``.
    ...
    """
    plugins_dir = marvi_home / "plugins"
...
def _discover_components(
    *,
    ...
    home = marvi_home or Path(get_marvi_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `marvi security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, marvi_home=home
    )
