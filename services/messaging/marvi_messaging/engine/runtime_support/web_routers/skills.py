...
the late-binding seam in :mod:`runtime_support.web_deps` so tests that
``monkeypatch.setattr(web_server, "_spawn_marvi_action", ...)`` keep
working.
"""
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from runtime_support.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
