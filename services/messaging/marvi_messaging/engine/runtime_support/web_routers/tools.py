...
late-binding seam in :mod:`runtime_support.web_deps` (``late`` for callables,
...
from runtime_support.web_deps import late, LateState
from runtime_support.web_models import (
...
_log = logging.getLogger("runtime_support.web_server")
...
_spawn_marvi_action = late("_spawn_marvi_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from runtime_support.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `marvi tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `marvi computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``marvi tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `marvi tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``marvi tools``
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``marvi computer-use permissions grant`` as a background action.
