...
Pure display functions with no MarviCLI state dependency.
"""
import json
import logging
import os
...
from marvi_constants import PRODUCT_NAME, get_marvi_home
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `marvi ... > file`) raises
...
from runtime_support import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built marvi — no local git history to count against).
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/nousresearch/marvi-messaging/"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "marvi-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `marvi update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Marvi update is available.

    Two paths: if ``MARVI_MESSAGING_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = marvi_home / ".update_check"
    embedded_rev = os.environ.get("MARVI_MESSAGING_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # MARVI_MESSAGING_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/marvi/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from runtime_support.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $MARVI_MESSAGING_HOME/marvi-messaging/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = marvi_home / "marvi-messaging"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Marvi git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$MARVI_MESSAGING_HOME/marvi-messaging/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``runtime_support/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from runtime_support.build_info import get_build_sha
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Marvi checkout. Cached per-process. Release URL always points at the
...
def _format_update_notice(behind: int) -> str:
    ...
    from runtime_support.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built marvi; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_marvi_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_marvi_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else MARVI_MESSAGING_CADUCEUS
    except Exception:
        _bskin = None
        _hero = MARVI_MESSAGING_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or marvi setup[/]"
            )
        else:
    ...
    if os.getenv("MARVI_MESSAGING_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from runtime_support.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from runtime_support.codex_runtime_switch import get_current_runtime
        from runtime_support.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else MARVI_MESSAGING_AGENT_LOGO
        console.print(_logo)
        console.print()
