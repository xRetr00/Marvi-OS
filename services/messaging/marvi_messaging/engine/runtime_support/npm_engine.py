...
Scope of the repair is deliberately narrow. Marvi only upgrades an npm that
lives inside its **own** managed Node tree (``$MARVI_MESSAGING_HOME/node``), installing
...
and their other projects; Marvi never modifies those. When the failing npm is
one of those foreign installs, Marvi instead provisions its own managed Node
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Marvi-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$MARVI_MESSAGING_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$MARVI_MESSAGING_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Marvi-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    try:
        # A temp cwd keeps the checkout's .npmrc (engine-strict, min-release-age)
        # from applying to the upgrade itself.
        with tempfile.TemporaryDirectory(prefix="marvi-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Marvi could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Marvi-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$MARVI_MESSAGING_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning a Marvi-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_marvi_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Marvi-managed install, or
    ...
    if prefix is not None:
        # Marvi owns this npm — upgrade it in place. Only an npm-range
