"""``marvi logs`` — view and filter Marvi log files.
...
Usage examples::
    ...
    marvi logs                    # last 50 lines of agent.log
    marvi logs -f                 # follow agent.log in real time
    marvi logs errors             # last 50 lines of errors.log
    marvi logs learning -f        # follow local learning activity
    marvi logs memory -f          # follow memory activity
    marvi logs gateway -n 100    # last 100 lines of gateway.log
    marvi logs gui -f            # follow gui.log (dashboard/pty/ws)
    marvi logs desktop -f        # follow desktop.log (Electron app boot/backend)
    marvi logs --level WARNING    # only WARNING+ lines
    marvi logs --session abc123   # filter by session ID substring
    marvi logs --component tools  # only tool-related lines
    marvi logs --since 1h         # lines from the last hour
    marvi logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_marvi_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Marvi runs — try 'marvi chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_marvi_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_marvi_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_marvi_home()}/logs/")
        return

    print(f"Log files in {display_marvi_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'marvi chat' to generate logs)")
