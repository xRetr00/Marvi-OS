"""CLI handlers for the ``marvi proxy`` subcommand."""
...
from runtime_support.proxy.adapters import ADAPTERS, get_adapter
from runtime_support.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "marvi proxy requires aiohttp. Run `marvi setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"marvi auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Marvi proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Marvi proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: marvi proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``marvi proxy <subcommand>``."""
    ...
    print(
        "marvi proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  marvi proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  marvi proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  marvi proxy providers\n"
