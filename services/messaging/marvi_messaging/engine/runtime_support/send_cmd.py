"""CLI subcommand: ``marvi send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"marvi send: {file_path} is not a text file. --file reads the "
                ...
                f'  marvi send --to telegram "MEDIA:{file_path}"\n'
                f'  marvi send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  marvi send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"marvi send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"marvi send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"marvi send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"marvi send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `marvi send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"marvi send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `marvi gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.marvi/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_marvi_env() -> None:
    """Populate ``os.environ`` from ``~/.marvi/.env`` AND bridge top-level
    ...
    startup that ``marvi send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.marvi/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.marvi/config.yaml`` into
       ...
       via ``marvi config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``marvi send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.marvi/.env and ~/.marvi/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "marvi send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  marvi send --to telegram \"hello\"\n"
            "  marvi send --to discord:#ops --file report.md\n"
            "  marvi send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "marvi send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `marvi send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Marvi "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.marvi/.env + ~/.marvi/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  marvi send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | marvi send --to telegram:-1001234567890\n"
            "  marvi send --to discord:#ops --file /tmp/report.md\n"
            "  marvi send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  marvi send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  marvi send --list                  # all platforms\n"
            "  marvi send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `marvi send --list telegram`.",
    )
