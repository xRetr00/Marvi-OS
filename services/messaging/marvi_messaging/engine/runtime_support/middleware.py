"""Marvi middleware contract helpers.
...
OBSERVER_SCHEMA_VERSION = "marvi.observer.v1"
MIDDLEWARE_SCHEMA_VERSION = "marvi.middleware.v1"
...
def apply_llm_request_middleware(
    ...
    **context: Any,
    ...
    provider kwargs before Marvi sends them.
    """
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from runtime_support.plugins import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from runtime_support.plugins import has_middleware
