...
- ``runtime_support.providers``        -- canonical provider identity + overlays
- ``runtime_support.model_normalize``  -- per-provider name formatting
...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``marvi model`` wizard — not a
    ...
    while ``marvi model`` still live-probed the full ``/v1/models`` list.
...
_MARVI_MESSAGING_MODEL_WARNING = (
    "Marvi 3 & 4 chat-family models are NOT agentic and are not designed "
    "for use with Marvi Messaging. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Marvi 3 / Marvi 4 chat families.
# The previous substring check (`"marvi" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``marvi-brain:qwen3-14b-ctx16k`` that just
# happen to carry "marvi" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Marvi-3-Llama-3.1-70B, marvi-4-405b, openrouter/marvi3:70b
# Negative examples it must NOT match:
#   marvi-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_MARVI_MESSAGING_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])marvi[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_marvi_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Marvi 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_MARVI_MESSAGING_NON_AGENTIC_RE.search(model_name))


def _check_marvi_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Marvi 3/4 chat model."""
    if is_nous_marvi_non_agentic(model_name):
        return _MARVI_MESSAGING_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``marvi config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from runtime_support.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``runtime_support.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'marvi model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'marvi doctor' — config issues detected:"
        ...
        from runtime_support.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from runtime_support.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from runtime_support.config import load_config
            from runtime_support.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from runtime_support.providers import resolve_user_provider as _ruser
    ...
    if target_provider in {"nous", "nous-portal", "nousresearch"}:
        from runtime_support.providers import nous_api_mode
    ...
    # runtime_support.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from runtime_support.models import normalize_opencode_base_url
    ...
    marvi_warn = _check_marvi_model_warning(new_model)
    if marvi_warn:
        warnings.append(marvi_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from runtime_support.config import normalize_extra_headers
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from runtime_support/models.py (OPENROUTER_MODELS,
    ...
    # Compared against marvi_id / mdev_id (section 1), pid / marvi_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from runtime_support.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by marvi provider ID
    ...
    # requiring a Marvi release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from runtime_support.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from runtime_support.models import fetch_lmstudio_models
    ...
    # --- 1. Check Marvi-mapped providers ---
    from runtime_support.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from runtime_support.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for marvi_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(marvi_id)
        if (
            _alias_target
            and _alias_target != marvi_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        # Resolve the canonical provider profile name.  Skip marvi_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose marvi_id matches the canonical profile name so
        ...
        # same marvi_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if marvi_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(marvi_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (MARVI_MESSAGING_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Marvi may not route yet.
        ...
        from runtime_support.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(marvi_id):
            continue
        ...
        if not has_creds:
            try:
                from runtime_support.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(marvi_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        marvi_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `marvi model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = _cached_model_ids(marvi_id)
        if not model_ids:
            model_ids = curated.get(marvi_id, [])
            if marvi_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(marvi_id, model_ids)
    ...
    # --- 2. Check Marvi-only providers (nous, openai-codex, copilot, opencode-go) ---
    from runtime_support.providers import MARVI_MESSAGING_OVERLAYS
    from runtime_support.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Marvi provider ID.
    # MARVI_MESSAGING_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Marvi IDs ("copilot").
    _mdev_to_marvi = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in MARVI_MESSAGING_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Marvi slug — e.g. "github-copilot" → "copilot"
        marvi_slug = _mdev_to_marvi.get(pid, pid)
        if marvi_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or marvi_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(marvi_slug)
        ...
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, marvi_slug):
                pcfg = _auth_registry.get(_key)
                if pcfg and pcfg.api_key_env_vars:
                    if any(os.environ.get(ev) for ev in pcfg.api_key_env_vars):
                        has_creds = True
                        break
        if not has_creds and marvi_slug == "llamacpp":
            has_creds = bool(
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or marvi_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(marvi_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(marvi_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", marvi_slug, exc)
        ...
        # configured.
        if not has_creds and marvi_slug == "anthropic":
            try:
                ...
                if (marvi_creds and marvi_creds.get("accessToken")) or \
        ...
        if marvi_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = _cached_model_ids(marvi_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = _cached_model_ids(marvi_slug)
                model_ids = _ids if _ids else (curated.get(marvi_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(marvi_slug, []) or curated.get(pid, [])
        elif marvi_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `marvi model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from runtime_support.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(marvi_slug, []) or curated.get(pid, [])
                if marvi_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(marvi_slug, model_ids)
        total = len(model_ids)
        if marvi_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": marvi_slug,
            "name": get_label(marvi_slug),
            "is_current": marvi_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "marvi",
        ...
        seen_slugs.add(marvi_slug.lower())
        _record_builtin_endpoint(marvi_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or MARVI_MESSAGING_OVERLAYS (keeps /model in sync
    # with `marvi model`).
    try:
        from runtime_support.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Marvi writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Marvi's own writer uses (mirrors section-4 grouping).
            ...
            # ``marvi model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``marvi model``. Pin with
            ...
            if _discovery_allowed:
                try:
                    from runtime_support.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Marvi's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, runtime_support/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``marvi model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from runtime_support.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`runtime_support.models.fetch_openrouter_models`, which filters the
    ...
    """
    from runtime_support.models import fetch_openrouter_models
