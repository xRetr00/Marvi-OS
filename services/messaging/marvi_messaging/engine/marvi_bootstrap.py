"""Windows UTF-8 bootstrap for Marvi entry points.
...
should be imported at the very top of every Marvi entry point
(``marvi``, ``marvi-messaging``, ``marvi-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``runtime_support._subprocess_compat.suppress_platform_ver_console``
    ...
    ``marvi_bootstrap``, never ``runtime_support.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Marvi modules.

    Marvi ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Marvi source root
    ...
    """
    root = src_root or os.environ.get("MARVI_MESSAGING_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``MARVI_MESSAGING_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("MARVI_MESSAGING_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import marvi_bootstrap``
# (or ``from marvi_bootstrap import apply_windows_utf8_bootstrap``) at
