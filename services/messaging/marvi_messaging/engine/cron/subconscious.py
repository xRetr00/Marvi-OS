...
``marvi subconscious enable|disable|status`` (``runtime_support/subconscious.py``)
...
def _subconscious_dir() -> Path:
    return get_marvi_home() / "subconscious"
...
def recent_activity_summary(hours: int = 24, limit: int = 30) -> str:
    from marvi_time import format_timestamp
...
def build_runtime_context(job_name: str) -> str:
    ...
    from marvi_time import format_timestamp, now as local_now
...
def _write_snapshot_shim() -> Path:
    """Materialize the Contract-1 pre-run script under MARVI_MESSAGING_HOME/scripts/.

    Cron pre-run scripts are sandboxed to ``MARVI_MESSAGING_HOME/scripts/`` (see
    ...
    shim_path.write_text(
        (
            '"""Auto-generated shim -- regenerated by `marvi subconscious enable`.\n\n'
...
def _mark_pending_trigger_reason(reason: str) -> None:
    ...
    try:
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            json.dumps({"reason": reason, "at": _marvi_now().isoformat()}),
            encoding="utf-8",
        )
    except Exception:
