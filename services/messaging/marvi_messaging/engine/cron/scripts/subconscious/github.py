...
def fetch_delta(store: SurfaceStore) -> Optional[str]:
    ...
    if store.is_first_run():
        store.set_cursor({"since": _marvi_now().isoformat(), "etag": None})
        return None
    ...
    if isinstance(raw_result, dict) and raw_result.get("not_modified"):
        store.update_cursor(since=_marvi_now().isoformat())
        return None
    ...
    store.update_cursor(since=_marvi_now().isoformat(), etag=new_etag or etag)
