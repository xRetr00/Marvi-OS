...
(used by passive status output like ``marvi composio list``) --
:func:`ensure_sdk_installed` is the function that actually triggers an
install attempt, and only ``marvi composio connect`` and the internal
...
def install_hint() -> str:
    ...
    internally (`ComposioUnavailable`) and by the CLI (`marvi composio
...
def _install_hint() -> str:
    ...
    return (
        ...
        "run `marvi composio connect <app>`, which now installs it "
...
def is_sdk_installed() -> bool:
    ...
    -- used by passive CLI status output (``marvi composio list``). Callers
...
def ensure_sdk_installed(*, prompt: bool = False) -> bool:
    ...
    interactive CLI call sites (``marvi composio connect``, which already
    has a terminal to ask "install now? [Y/n]" on), False for unattended
    call sites (the subconscious cron fetchers, ``marvi composio list``'s
    ...
    def __init__(self, api_key: str):
        if not api_key:
            raise ComposioAuthError(
                ...
                "the Marvi secret store (or run `marvi composio connect`)."
            )
...
def get_client(api_key: Optional[str] = None) -> ComposioClient:
    ...
    if not key:
        raise ComposioAuthError(
            "No Composio API key configured. Run `marvi composio connect <app>` "
