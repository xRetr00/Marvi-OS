...
``~/.marvi/subconscious/snapshots/<surface>.json`` holding:
...
def snapshots_dir() -> Path:
    """Return ``~/.marvi/subconscious/snapshots`` (created if missing)."""
    d = get_marvi_home() / "subconscious" / "snapshots"
...
class SurfaceStore:
    ...
    def record_failure(self, error: str) -> None:
        ...
        self._snapshot.next_retry_at = (_marvi_now() + timedelta(seconds=delay)).isoformat()
        self._dirty = True
    ...
    def status_dict(self) -> Dict[str, Any]:
        """Read-only summary for CLI status display (`marvi composio list`)."""
        return {
            "surface": self.surface,
