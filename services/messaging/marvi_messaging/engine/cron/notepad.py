...
Write path is the CLI (``marvi cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_marvi_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`marvi cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `marvi cron notepad {job_id} delete "
