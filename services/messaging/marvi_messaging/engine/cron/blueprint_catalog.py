...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``marvi://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``marvi://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"marvi://blueprint/{quote(blueprint.key)}{qs}"
