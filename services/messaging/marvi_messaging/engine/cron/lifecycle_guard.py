...
``marvi gateway restart`` (or ``launchctl kickstart ai.marvi.gateway``
or ``systemctl restart marvi-gateway``).  When the cron fires, the
...
``marvi cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``marvi gateway``, ``launchctl ... marvi-gateway``,
``systemctl ... marvi-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_MARVI_MESSAGING_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``marvi gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `marvi gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:marvi\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a marvi-gateway label. macOS launchd
    # labels look like `ai.marvi.gateway` / `marvi-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated marvi services (e.g.
    # `launchctl unload ai.marvi.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.marvi.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a marvi-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bmarvi[.\-]?gateway)"
    # Branch D: pkill / kill targeting the marvi gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.marvi.gateway-... \` / `  -- ...`,
...
# for `systemctl restart marvi-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart marvi-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.marvi.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart marvi-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        grep -c 'systemctl restart marvi-gateway' /var/log/syslog
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart marvi-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".marvi"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<MARVI_MESSAGING_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.marvi/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_marvi_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_marvi_home() falls back to Path.home(), which raises when
        # neither MARVI_MESSAGING_HOME nor HOME is resolvable (launchd/systemd
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `marvi gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `marvi gateway restart` from a shell outside "
