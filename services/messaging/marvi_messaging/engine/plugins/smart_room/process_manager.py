...
def _root() -> Path:
    return (_supervisor_home or Path(get_marvi_home())) / "smart_room"
...
def _call_runtime(method: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    token = set_marvi_home_override(_root().parent)
    try:
...
def _start(config: Optional[Dict[str, Any]] = None, *, restart_count: int = 0) -> Dict[str, Any]:
    ...
    with _lock:
        ...
        env["MARVI_MESSAGING_HOME"] = str(_root().parent)
...
def start_supervisor(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with _lock:
        ...
        _supervisor_home = Path(get_marvi_home())
