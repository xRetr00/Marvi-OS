...
def state_path() -> Path:
    return Path(get_marvi_home()) / "smart_room" / "state.json"
...
def load_config() -> Dict[str, Any]:
    ...
    try:
        import yaml
        from runtime_support.config import cfg_get, load_config as load_marvi_config

        return cfg_get(load_marvi_config(), "smart_room", default={}) or {}
    except Exception:
        return {}


def events_path() -> Path:
    return Path(get_marvi_home()) / "smart_room" / "events.jsonl"


def locations_path() -> Path:
    return Path(get_marvi_home()) / "smart_room" / "locations.jsonl"
