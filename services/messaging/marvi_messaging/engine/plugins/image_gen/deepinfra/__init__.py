...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=marvi``
where each entry's ``metadata.tags`` declares its surface (``image-gen``
here). ``list_models()`` filters that catalog via
:func:`runtime_support.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``marvi tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from marvi the next time the catalog is fetched, no patch required.
...
def _live_models() -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        from runtime_support.models import _fetch_deepinfra_models_by_tag
    except Exception as exc:
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `marvi tools` → Image "
                    "Generation → DeepInfra to configure, or `marvi setup` "
                    "to add the key."
                ),
        ...
        from runtime_support.models import deepinfra_base_url
