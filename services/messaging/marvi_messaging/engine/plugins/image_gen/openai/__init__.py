...
three virtual model IDs so the ``marvi tools`` model picker and the
...
``$MARVI_MESSAGING_HOME/cache/images/``.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `marvi tools` → Image "
                    "Generation → OpenAI to configure, or `marvi setup` "
                    "to add the key."
                ),
