...
```bash
marvi plugins enable google_meet
marvi meet install                 # pip deps + Chromium (transcribe only)
marvi meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
marvi meet auth                    # optional; skips guest-lobby wait
marvi meet setup                   # preflight checks
```

`marvi meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or MARVI_MESSAGING_MEET_REALTIME_KEY in ~/.marvi/.env
```
...
pip install playwright websockets && python -m playwright install chromium
marvi plugins enable google_meet
marvi meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
marvi meet node approve my-mac ws://<mac-ip>:18789 <token>
marvi meet node ping my-mac                   # confirm reachable
```

Run `marvi meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Marvi Messaging bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `marvi meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `MARVI_MESSAGING_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `marvi_meet_src`). |
...
Local:
```
$MARVI_MESSAGING_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$MARVI_MESSAGING_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `marvi meet node approve`.
