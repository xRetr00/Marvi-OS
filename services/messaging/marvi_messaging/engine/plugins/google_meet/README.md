...
Let the Marvi Messaging join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where marvi runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `marvi meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `marvi meet` CLI |
...
| `cli.py` | `marvi meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$MARVI_MESSAGING_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `marvi meet node {run,list,approve,remove,status,ping}` |
...
marvi meet install                                      # pip + Chromium
marvi meet setup                                        # preflight
marvi meet auth                                         # optional
marvi meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
marvi meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.marvi/.env
marvi meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
marvi meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
marvi meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.marvi/.env
marvi meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, marvi will **not** switch your system audio input automatically — the
...
marvi plugins enable google_meet
marvi meet node run --display-name my-mac --host 0.0.0.0 --port 18789
