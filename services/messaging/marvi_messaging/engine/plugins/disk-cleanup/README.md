...
Auto-tracks and cleans up ephemeral files created during Marvi Messaging
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$MARVI_MESSAGING_HOME` and `/tmp/marvi-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MARVI_MESSAGING_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under MARVI_MESSAGING_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `MARVI_MESSAGING_HOME` or `/tmp/marvi-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$MARVI_MESSAGING_HOME/disk-cleanup/` is itself excluded
- `$MARVI_MESSAGING_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
