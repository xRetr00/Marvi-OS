"""disk_cleanup — ephemeral file cleanup for Marvi Messaging.
...
Rules:
  ...
  - empty dirs    → always delete (under MARVI_MESSAGING_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly MARVI_MESSAGING_HOME and /tmp/marvi-*
Never touches: ~/.marvi/logs/ or any system directory.
"""
...
try:
    ...
    def get_marvi_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("MARVI_MESSAGING_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".marvi").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$MARVI_MESSAGING_HOME/logs/``."""
    return get_marvi_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$MARVI_MESSAGING_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under MARVI_MESSAGING_HOME or ``/tmp/marvi-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    marvi_home = get_marvi_home()
    try:
        path.resolve().relative_to(marvi_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/marvi-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("marvi-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "marvi-messaging", "backups", "profiles", ".worktrees",
...
# Paths under $MARVI_MESSAGING_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so MARVI_MESSAGING_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        marvi_home = get_marvi_home()
        for parent in ("cron", "cronjobs"):
            base = marvi_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside MARVI_MESSAGING_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under MARVI_MESSAGING_HOME, but never recurse into known
    # durable state trees.  Some installs place the Marvi checkout, venv,
    # and desktop build under MARVI_MESSAGING_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    marvi_home = get_marvi_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in marvi_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    marvi_home = get_marvi_home()
    try:
        rel = path.resolve().relative_to(marvi_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "marvi-messaging",
        ...
        # Path isn't under MARVI_MESSAGING_HOME (e.g. /tmp/marvi-*) — fall through.
        pass
