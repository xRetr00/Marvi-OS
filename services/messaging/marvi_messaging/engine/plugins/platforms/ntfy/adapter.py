"""ntfy platform adapter (Marvi plugin).
...
``plugins/platforms/ntfy/``. The Marvi plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "marvi-in"              # subscribe topic (incoming)
          publish_topic: "marvi-out"     # optional — defaults to topic
...
_ECHO_TAG = "marvi-messaging"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``marvi cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Marvi dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `marvi gateway status` without
