...
- Per-user tokens (keyed by sender email):
    ``${MARVI_MESSAGING_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${MARVI_MESSAGING_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${MARVI_MESSAGING_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${MARVI_MESSAGING_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${MARVI_MESSAGING_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's MARVI_MESSAGING_HOME helper so the token follows the user's
# profile (e.g. tests can override via MARVI_MESSAGING_HOME=/tmp/...).
try:
    from marvi_constants import display_marvi_home, get_marvi_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where marvi_constants isn't importable
...
def _marvi_home() -> Path:
    """Resolve MARVI_MESSAGING_HOME at call time (NOT module import).

    Tests and ``MARVI_MESSAGING_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.marvi/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _marvi_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _marvi_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _marvi_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _marvi_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _marvi_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `marvi setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `marvi setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into MARVI_MESSAGING_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_marvi_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_marvi_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Marvi (native attachment delivery)"
    )
