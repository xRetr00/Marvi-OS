...
# ``model_tools`` import time, so ``marvi status``, ``marvi chat``, etc.
...
# ``marvi_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Marvi is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a marvi MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "marvi-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${MARVI_MESSAGING_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _marvi_home = _get_marvi_home()
        except (ModuleNotFoundError, ImportError):
            _marvi_home = _Path.home() / ".marvi"
        self._thread_count_store = _ThreadCountStore(
            _marvi_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Marvi is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("MARVI_MESSAGING_HOME", str(_Path.home() / ".marvi"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${MARVI_MESSAGING_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a marvi MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Marvi is thinking…' marker message.
        ...
        "Marvi is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Marvi is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Marvi is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Marvi is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``marvi setup``.

    The setup wizard at ``runtime_support/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.marvi/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. marvi-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.marvi/.env")
    print_info("Restart the gateway: marvi gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``marvi cron`` running as a
    separate process from ``marvi gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Marvi plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `marvi setup` to install Google Chat support.",
        ...
        # the "Marvi is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Marvi is thinking…' marker message "
