...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Marvi command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Marvi accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Marvi ``/command`` text."""
...
# Uses get_marvi_home() so each profile gets its own Matrix store.
from marvi_constants import get_marvi_dir as _get_marvi_dir

_STORE_DIR = _get_marvi_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Marvi
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("MARVI_MESSAGING_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("MARVI_MESSAGING_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "marvi"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Marvi Messaging" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in runtime_support/{setup,gateway}.py, and the
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        txn_id = f"marvi_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces runtime_support/setup.py::_setup_matrix
...
def _is_connected(config) -> bool:
    ...
    configured. Read via runtime_support.gateway.get_env_value so setup-status
