...
# identify MarviAgent traffic — matching other Marvi outbound surfaces
# that already set ``MarviAgent/<version>`` for platform-partner attribution.
try:
    from runtime_support import __version__ as _MARVI_MESSAGING_VERSION
except Exception:
    _MARVI_MESSAGING_VERSION = "unknown"
_MARVI_MESSAGING_SLACK_USER_AGENT_PREFIX = f"MarviAgent/{_MARVI_MESSAGING_VERSION}"
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from runtime_support.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `marvi slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `marvi gateway setup` "
                "or in the active profile's ~/.marvi/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `marvi gateway setup` "
                "or add it to your active profile's ~/.marvi/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `marvi gateway setup` "
                "or in the active profile's ~/.marvi/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `marvi gateway setup` "
                "or add it to your active profile's ~/.marvi/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_marvi_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_MARVI_MESSAGING_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Marvi has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Marvi can "
                    "be removed from the Slack app manifest via "
                    "`marvi slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /marvi prefix). A single regex
            ...
            # manifest (see `marvi slack manifest`). In Socket Mode, Slack
            ...
            from runtime_support.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/marvi$")

            @self._app.command(_slash_pattern)
            async def handle_marvi_command(ack, command):
            ...
            for _action_id in (
                "marvi_approve_once",
                "marvi_approve_session",
                "marvi_approve_always",
                "marvi_deny",
            ):
            ...
            for _action_id in (
                "marvi_confirm_once",
                "marvi_confirm_always",
                "marvi_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("marvi_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^marvi_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("marvi_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'marvi slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Marvi handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Marvi session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Marvi owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_marvi_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_marvi_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_marvi_no_thread_response"):
        ...
        force_process = bool(event.get("_marvi_force_process"))
        ...
        # the shape produced by peer Marvi Messagings in Socket Mode on some
        ...
        try:
            ...
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "marvi_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "marvi_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "marvi_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "marvi_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "marvi_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``marvi_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``marvi_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"marvi_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "marvi_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "marvi_confirm_once": "once",
            "marvi_confirm_always": "always",
            "marvi_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "marvi_approve_once": "once",
            "marvi_approve_session": "session",
            "marvi_approve_always": "always",
            "marvi_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "marvi_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/marvi <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/marvi
        ...
        if slash_name in {"marvi", ""}:
            # Legacy /marvi <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from runtime_support.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/marvi   ``).
        ...
        # "Running /cmd…" ack shown by handle_marvi_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/marvi <question>" must produce public replies so
...
# ``runtime_support/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_marvi_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``runtime_support/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under MARVI_MESSAGING_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from runtime_support.slack_cli import _build_full_manifest
            from marvi_constants import get_marvi_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Marvi",
                bot_description="Your Marvi Messaging on Slack",
            )
            target = Path(get_marvi_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `marvi slack manifest --write` anytime to refresh after "
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `marvi update`)",
                True,
            ):
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``runtime_support.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `marvi setup` to install Slack support.",
        # Interactive setup wizard — replaces runtime_support/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in runtime_support/gateway.py.
