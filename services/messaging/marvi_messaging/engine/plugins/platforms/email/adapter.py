"""
Email platform adapter for the Marvi gateway.

Allows users to interact with Marvi by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            from runtime_support import __version__ as _marvi_version
        except Exception:  # noqa: BLE001 — keep ID best-effort if import fails
            _marvi_version = "0"
        imap.xatom(
            "ID",
            f'("name" "marvi-messaging" "version" "{_marvi_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `marvi config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `marvi gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Marvi Messaging")
        ...
        msg_id = f"<marvi-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# runtime_support/gateway.py, and the _send_email dispatch in
