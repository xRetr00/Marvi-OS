"""
LINE Messaging API platform adapter for Marvi Messaging.
...
to Marvi Messaging. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``MARVI_MESSAGING_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            marvi_home = Path(get_marvi_home()).resolve()
        except Exception:
            marvi_home = Path.home().joinpath(".marvi").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``marvi status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``marvi status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``marvi setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.marvi/.env`` via ``runtime_support.config``.
    """
    print()
    ...
    try:
        from runtime_support.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("runtime_support.config not available; set LINE_* vars manually in ~/.marvi/.env")
        return
