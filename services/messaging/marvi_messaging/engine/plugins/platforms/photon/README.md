...
This plugin connects Marvi Messaging to iMessage (and other Spectrum
...
TypeScript-only, Marvi runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Marvi channel,
...
Run `marvi photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.marvi/.env` (the same place every other
...
Management metadata lives in `~/.marvi/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `marvi photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Marvi
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Marvi coalesces
...
- **Native polls are supported.** Marvi posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `marvi send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<marvi-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `marvi photon status`, a DM and a group roundtrip,
