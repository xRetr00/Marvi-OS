...
under an immutable ``/opt/marvi`` (read-only for the marvi user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$MARVI_MESSAGING_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_marvi_home() / "photon" / "sidecar"
    try:
