...
Credential storage mirrors every other Marvi channel:

    * runtime SDK creds  -> ``~/.marvi/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.marvi/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a marvi dependency
...
# registers Marvi as its own client_id.
...
# Default name of the project Marvi provisions for the operator.
DEFAULT_PROJECT_NAME = "Marvi Messaging"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of marvi-messaging.

def _auth_json_path() -> Path:
    """Resolve ``~/.marvi/auth.json`` honouring the active Marvi profile."""
    try:
        from marvi_constants import get_marvi_home
        return Path(get_marvi_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.marvi")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # runtime_support/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from runtime_support.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.marvi/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.marvi/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.marvi/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: runtime_support.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `marvi photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `marvi photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `marvi photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `marvi photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `marvi photon setup`)"
