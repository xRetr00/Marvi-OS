"""
``marvi photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Marvi
...
# Writable sidecar runtime dir (mirrors to MARVI_MESSAGING_HOME on immutable
...
# wiring imports this module for `marvi --help`).
...
# when called later (gateway start, marvi status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `marvi photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Marvi Messaging')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``marvi photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Marvi Messaging" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.marvi/.env,
    ...
    try:
        from runtime_support.config import write_platform_config_field
    ...
    print("  Start the gateway:  marvi gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `marvi photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`marvi photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.marvi/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `marvi photon telemetry on` / `marvi photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.marvi/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  marvi gateway restart")
    return 0
...
#
# `marvi gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``marvi photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `marvi gateway setup` wizard."""
