...
Small Node helper that bridges Marvi Messaging to Photon's Spectrum SDK
(`spectrum-ts`).  Marvi is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Marvi-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Marvi can receive inbound messages
...
The Marvi plugin's `marvi photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Marvi is Python, so replies go through this sidecar
