// Marvi Messaging — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Marvi): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Marvi -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Marvi-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `marvi photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Marvi can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "marvi-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Marvi'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Marvi allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Marvi, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-marvi-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Marvi supplied them so a known-good
