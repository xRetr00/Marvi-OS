"""SimpleX Chat platform adapter (Marvi plugin).
...
This adapter ships as a Marvi platform plugin under
``plugins/platforms/simplex/``. The Marvi plugin loader scans the
...
Optional environment variables:
    ...
    MARVI_MESSAGING_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``marvi setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Marvi — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("MARVI_MESSAGING_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``marvi setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.marvi/.env`` via
    ``runtime_support.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "runtime_support.config not available; set SIMPLEX_* vars manually in "
            "~/.marvi/.env"
        )
        return
