    ...
    async def _create_and_stream_card(
        ...
        *,
        ...
        try:
            ...
            out_track_id = f"marvi_{uuid.uuid4().hex[:12]}"
...
# dict in runtime_support/gateway.py, and the _send_dingtalk dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces runtime_support/setup.py-era _setup_dingtalk + the static
    _PLATFORMS["dingtalk"] dict in runtime_support/gateway.py. CLI helpers are
    ...
    if method == 0:
        try:
            from runtime_support.dingtalk_auth import dingtalk_qr_auth
