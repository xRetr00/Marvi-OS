# A2A — Agent-to-Agent protocol for Marvi
...
peer (another Marvi, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
marvi gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Marvi serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.marvi/a2a_audit.jsonl`.
- Conversations persist to `~/.marvi/a2a_conversations/` — they survive context
