...
``{title, url, description, position}`` rows every other Marvi web
provider produces.
...
prefers Marvi-managed xAI Grok OAuth (via ``marvi auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.marvi/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``marvi tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `marvi auth` to sign in with "
