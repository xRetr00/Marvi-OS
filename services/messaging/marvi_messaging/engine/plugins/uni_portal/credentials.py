...
outside of the interactive ``marvi uni login`` capture -> store round-trip
(``runtime_support/subcommands/uni.py``). Storage uses the Windows Credential
...
TARGET_NAME = "marvi:uni_portal:duzce_student_system"
...
def has_credentials() -> bool:
    ...
    surfaces (``marvi uni status``, the plugin's availability check) that
    only need a yes/no."""
    return read_credentials() is not None


def delete_credentials() -> bool:
    """Remove the stored credentials (``marvi uni login --logout``).
