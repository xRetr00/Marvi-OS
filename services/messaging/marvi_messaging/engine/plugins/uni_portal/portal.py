...
enrollment (``marvi uni login``) to confirm against the real portal.
"""
...
def _uni_portal_config() -> Dict[str, Any]:
    ...
    (enrollment is CLI-driven via ``marvi uni login``), mirrors the
...
def login() -> bool:
    ...
    if creds is None:
        logger.info("uni_portal: no stored credentials — run `marvi uni login` first")
        return False
