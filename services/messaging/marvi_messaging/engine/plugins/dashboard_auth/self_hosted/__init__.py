...
A standards-compliant OpenID Connect Relying Party for the ``marvi dashboard``
...
:class:`~runtime_support.dashboard_auth.DashboardAuthProvider` methods and touches
...
``runtime_support/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~runtime_support.dashboard_auth.Session`.
...
same precedence convention as the ``nous`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          issuer: https://auth.example.com/application/o/marvi/   # required
          client_id: marvi-dashboard                              # required
          scopes: "openid profile email"                           # optional
          # client_secret: set ONLY for a confidential client. It is a
          # credential — prefer the env var / ~/.marvi/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    MARVI_MESSAGING_DASHBOARD_OIDC_ISSUER
    MARVI_MESSAGING_DASHBOARD_OIDC_CLIENT_ID
    MARVI_MESSAGING_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    MARVI_MESSAGING_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "marvi_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Marvi does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``MARVI_MESSAGING_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "MARVI_MESSAGING_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "MARVI_MESSAGING_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("MARVI_MESSAGING_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.marvi/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "MARVI_MESSAGING_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(MARVI_MESSAGING_DASHBOARD_OIDC_ISSUER + MARVI_MESSAGING_DASHBOARD_OIDC_CLIENT_ID) "
