...
Optional Marvi observability plugin that configures exporters and maps
Marvi-specific observer hooks to NeMo Relay marks and ATIF state. Marvi core
...
not replace Marvi Messaging's planner, tools, memory, model provider routing, or
CLI UX. Marvi core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Marvi Messaging can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Marvi core.
- Add Marvi session, turn, approval, and subagent mark events.
...
```bash
marvi plugins enable observability/nemo_relay
```

The `MARVI_MESSAGING_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `MARVI_MESSAGING_HOME` that the
agent run will use:

```bash
env MARVI_MESSAGING_HOME=/tmp/marvi-nemo-relay-test \
  marvi plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`MARVI_MESSAGING_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`MARVI_MESSAGING_HOME` is the Marvi profile/config home used by both
`marvi plugins enable ...` and the later `marvi chat ...` run. If unset,
Marvi uses the user's default home, usually `~/.marvi`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export MARVI_MESSAGING_HOME=/tmp/marvi-nemo-relay-test
marvi plugins enable observability/nemo_relay
marvi chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `marvi` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run marvi plugins enable observability/nemo_relay
uv run marvi chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Marvi installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/marvi_messaging-*.whl
marvi plugins enable observability/nemo_relay
```
...
behavior; it does not control Marvi shared client metrics.
...
`marvi.session.segment` / `marvi.session.segment_reason`
...
The plugin can configure exporters directly from `MARVI_MESSAGING_NEMO_RELAY_*`
...
```bash
export MARVI_MESSAGING_NEMO_RELAY_ATOF_ENABLED=1
export MARVI_MESSAGING_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export MARVI_MESSAGING_NEMO_RELAY_ATIF_ENABLED=1
export MARVI_MESSAGING_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `MARVI_MESSAGING_NEMO_RELAY_ATOF_FILENAME`
- `MARVI_MESSAGING_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `MARVI_MESSAGING_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `MARVI_MESSAGING_NEMO_RELAY_ATIF_AGENT_NAME`
- `MARVI_MESSAGING_NEMO_RELAY_ATIF_AGENT_VERSION`
- `MARVI_MESSAGING_NEMO_RELAY_ATIF_MODEL_NAME`
- `MARVI_MESSAGING_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Marvi at it:

```bash
export MARVI_MESSAGING_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Marvi Messaging"
agent_version = "local"
```

When `MARVI_MESSAGING_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`MARVI_MESSAGING_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`MARVI_MESSAGING_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Marvi does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Marvi keeps the direct env-var fallbacks active for that run.

Marvi core routes provider and tool execution through NeMo Relay managed APIs
...
Marvi uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Marvi-owned
...
with this Marvi-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Marvi rejects
...
Marvi activates these plugins before registering its managed LLM and tool
...
For the full generic Marvi middleware contract, see
...
with Marvi and a local Ollama model served through the OpenAI-compatible API.

```bash
export MARVI_MESSAGING_HOME=/tmp/marvi-nemo-relay-docs/marvi-home
mkdir -p "$MARVI_MESSAGING_HOME"

cat > "$MARVI_MESSAGING_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Marvi session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export MARVI_MESSAGING_NEMO_RELAY_ATOF_ENABLED=1
export MARVI_MESSAGING_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/marvi-nemo-relay-docs/subagent/atof
export MARVI_MESSAGING_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export MARVI_MESSAGING_NEMO_RELAY_ATOF_MODE=overwrite
export MARVI_MESSAGING_NEMO_RELAY_ATIF_ENABLED=1
export MARVI_MESSAGING_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/marvi-nemo-relay-docs/subagent/atif
export MARVI_MESSAGING_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export MARVI_MESSAGING_NEMO_RELAY_ATIF_AGENT_NAME='Marvi Messaging E2E'
export MARVI_MESSAGING_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export MARVI_MESSAGING_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"marvi.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Marvi Messaging E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Marvi dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/marvi-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/marvi-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/marvi-nemo-relay-docs/workdir/beta.txt
cd /tmp/marvi-nemo-relay-docs/workdir

export MARVI_MESSAGING_NEMO_RELAY_ATOF_ENABLED=1
export MARVI_MESSAGING_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/marvi-nemo-relay-docs/parallel/atof
export MARVI_MESSAGING_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export MARVI_MESSAGING_NEMO_RELAY_ATOF_MODE=overwrite
export MARVI_MESSAGING_NEMO_RELAY_ATIF_ENABLED=1
export MARVI_MESSAGING_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/marvi-nemo-relay-docs/parallel/atif
export MARVI_MESSAGING_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export MARVI_MESSAGING_NEMO_RELAY_ATIF_AGENT_NAME='Marvi Messaging E2E'
export MARVI_MESSAGING_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Marvi sessions map to `agent` scopes.
- Marvi core managed provider calls map to `llm` scope start/end events.
- Marvi core managed tool calls map to `tool` scope start/end events.
...
Marvi core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Marvi-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Marvi calls the
...
```bash
export MARVI_MESSAGING_NEMO_RELAY_PLUGINS_TOML=/tmp/marvi-middleware-test/plugins.toml
```
...
Marvi provider call
  -> nemo_relay.llm.execute(...)
    -> Marvi provider adapter callback(...)

Marvi tool call
  -> nemo_relay.tools.execute(...)
    -> Marvi authorization and dispatch callback(...)
```
...
middleware for a local Marvi run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export MARVI_MESSAGING_HOME=/tmp/marvi-middleware-test/marvi-home
mkdir -p "$MARVI_MESSAGING_HOME" /tmp/marvi-middleware-test/nemo-relay
...
cat > /tmp/marvi-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/marvi-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/marvi-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Marvi Middleware E2E"
agent_version = "local"
...
export MARVI_MESSAGING_NEMO_RELAY_PLUGINS_TOML=/tmp/marvi-middleware-test/nemo-relay/plugins.toml
