/**
 * marvi Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.marvi/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__marvi_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__marvi_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "marvi-kanban-dot-triage",
    todo: "marvi-kanban-dot-todo",
    ready: "marvi-kanban-dot-ready",
    running: "marvi-kanban-dot-running",
    blocked: "marvi-kanban-dot-blocked",
    review: "marvi-kanban-dot-review",
    done: "marvi-kanban-dot-done",
    archived: "marvi-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-marvi-task";
  ...
  const LS_BOARD_KEY = "marvi.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="marvi-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])marvi-kanban-md-code\1/i.test(attrs)) {
      return ' class="marvi-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "marvi-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "marvi-kanban-md",
  ...
  // standard `drop` event and our `marvi-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("marvi-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("marvi-kanban-column--drop");
          if (target) target.classList.add("marvi-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("marvi-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("marvi-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("marvi-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__marvi_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "marvi-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "marvi-kanban-attention",
        "marvi-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "marvi-kanban-attention-bar" },
        h("span", { className: "marvi-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "marvi-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "marvi-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "marvi-kanban-attention-row",
                  "marvi-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "marvi-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "marvi-kanban-attention-row-id" }, task.id),
                h("span", { className: "marvi-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "marvi-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "marvi-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "marvi-kanban-diag-action-btn",
      action.suggested ? "marvi-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " marvi-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".marvi-kanban-drawer-comment-row input, .marvi-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "marvi-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("marvi-kanban-diag", sevClass) },
      h("div", { className: "marvi-kanban-diag-header" },
        h("span", { className: "marvi-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "marvi-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "marvi-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "marvi-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "marvi-kanban-diag-data-row" },
                  h("span", { className: "marvi-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "marvi-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "marvi-kanban-diag-data-row" },
                h("span", { className: "marvi-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "marvi-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "marvi-kanban-diag-reassign-row" },
            h("span", { className: "marvi-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "marvi-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "marvi-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "marvi-kanban-diag-msg",
              msg.ok ? "marvi-kanban-diag-msg--ok" : "marvi-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "marvi-kanban-section" },
      h("div", { className: "marvi-kanban-section-head-row" },
        h("span", { className: "marvi-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "marvi-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "marvi-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "marvi-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "marvi-kanban-docs-link",
      title: "Open marvi Kanban docs in a new tab",
      "aria-label": "marvi Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `marvi kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "marvi-kanban-msg-ok" : "marvi-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "marvi-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "marvi-kanban-boardswitcher" },
      h("div", { className: "marvi-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "marvi-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "marvi-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "marvi-kanban-dialog-title" },
        ...
        h("div", { className: "marvi-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned marvi profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "marvi-kanban-bulk" },
      h("span", { className: "marvi-kanban-bulk-count" },
      ...
      h("div", { className: "marvi-kanban-bulk-priority",
      ...
      h("div", { className: "marvi-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different marvi profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "marvi-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("marvi-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("marvi-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "marvi-kanban-trash",
        dragOver ? "marvi-kanban-trash--drop" : "",
        props.draggingTaskId ? "marvi-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "marvi-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "marvi-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".marvi-kanban-card")) return true;
      if (target.closest && target.closest(".marvi-kanban-column-add")) return true;
      if (target.closest && target.closest(".marvi-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("marvi-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("marvi-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".marvi-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "marvi-kanban-columns",
        isScrollable ? "marvi-kanban-columns--scrollable" : "",
        isPanning ? "marvi-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("marvi-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("marvi-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "marvi-kanban-column",
        dragOver ? "marvi-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "marvi-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "marvi-kanban-col-check",
        ...
        h("span", { className: cn("marvi-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "marvi-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "marvi-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "marvi-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "marvi-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "marvi-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "marvi-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "marvi-kanban-lane" },
                  h("div", { className: "marvi-kanban-lane-head" },
                    h("span", { className: "marvi-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "marvi-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "marvi-kanban-card--stale-red";
    if (age >= tier.amber) return "marvi-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".marvi-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "marvi-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "marvi-kanban-card",
        props.selected ? "marvi-kanban-card--selected" : "",
        props.failed ? "marvi-kanban-card--failed" : "",
        props.draggingSource ? "marvi-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "marvi-kanban-card-content" },
          h("div", { className: "marvi-kanban-card-row" },
            h("label", {
              className: "marvi-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "marvi-kanban-card-check",
            ...
            h("span", { className: "marvi-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or marvi kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "marvi-kanban-warning-badge",
                    "marvi-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "marvi-kanban-needs-assignee",
          ...
          h("div", { className: "marvi-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "marvi-kanban-card-row marvi-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "marvi-kanban-assignee",
                            title: `Assigned to marvi profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "marvi-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "marvi-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "marvi-kanban-ago",
  ...
  // centered modal reusing the marvi-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "marvi-kanban-dialog marvi-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "marvi profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "marvi profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "marvi-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "marvi-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "marvi-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "marvi-kanban-drawer-close",
        ...
        data ? h("div", { className: "marvi-kanban-drawer-comment-foot" },
          h("div", {
            className: "marvi-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "marvi-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "marvi-kanban-section" },
      h("div", { className: "marvi-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "marvi-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "marvi-kanban-drawer-body" },
      h("div", { className: "marvi-kanban-drawer-title" },
        h("span", { className: cn("marvi-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "marvi-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "marvi-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "marvi-kanban-section" },
            h("div", { className: "marvi-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "marvi-kanban-section" },
            h("div", { className: "marvi-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "marvi-kanban-done-no-result marvi-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "marvi-kanban-section" },
            h("div", { className: "marvi-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "marvi-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "marvi-kanban-section" },
        h("div", { className: "marvi-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "marvi-kanban-comment" },
            h("div", { className: "marvi-kanban-comment-head" },
              h("span", { className: "marvi-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "marvi-kanban-diag-action-btn",
      ...
      h("div", { className: "marvi-kanban-section" },
        h("div", { className: "marvi-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "marvi-kanban-comment" },
                h("div", { className: "marvi-kanban-comment-head" },
                  h("span", { className: "marvi-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "marvi-kanban-comment-ago" },
      ...
      h("div", { className: "marvi-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "marvi-kanban-event",
              isDiag ? "marvi-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "marvi-kanban-event-header" },
                  h("span", { className: "marvi-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "marvi-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "marvi-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "marvi-kanban-event-header-plain" },
                  h("span", { className: "marvi-kanban-event-kind" }, e.kind),
                  h("span", { className: "marvi-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "marvi-kanban-event-phantom-row" },
                  h("span", { className: "marvi-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "marvi-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "marvi-kanban-event-payload" },
    ...
    return h("div", { className: "marvi-kanban-section" },
      h("div", { className: "marvi-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "marvi-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `marvi-kanban-run--${r.outcome || r.status || "ended"}`
          : "marvi-kanban-run--active";
        return h("div", { key: r.id, className: cn("marvi-kanban-run", outcomeClass) },
          h("div", { className: "marvi-kanban-run-head" },
            h("span", { className: "marvi-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "marvi-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "marvi-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "marvi-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "marvi-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "marvi-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "marvi-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "marvi-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "marvi-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "marvi-kanban-pre marvi-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "marvi-kanban-meta-row" },
      h("span", { className: "marvi-kanban-meta-label" }, props.label),
      h("span", { className: "marvi-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "marvi-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "marvi-kanban-meta-row" },
        h("span", { className: "marvi-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "marvi-kanban-meta-value marvi-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "marvi-kanban-meta-row" },
        h("span", { className: "marvi-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "marvi-kanban-meta-value marvi-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "marvi-kanban-meta-row" },
        h("span", { className: "marvi-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "marvi-kanban-meta-value marvi-kanban-editable",
    ...
    return h("div", { className: "marvi-kanban-meta-row" },
      h("span", { className: "marvi-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "marvi-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "marvi-kanban-section" },
      h("div", { className: "marvi-kanban-section-head-row" },
        h("span", { className: "marvi-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "marvi-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "marvi-kanban-section" },
      h("div", { className: "marvi-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "marvi-kanban-deps-row" },
        h("span", { className: "marvi-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "marvi-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "marvi-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "marvi-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "marvi-kanban-dep-chip-x",
      ...
      h("div", { className: "marvi-kanban-deps-row" },
        h("span", { className: "marvi-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "marvi-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "marvi-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "marvi-kanban-section" },
      ...
      h("div", { className: "marvi-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "marvi-kanban-home-sub marvi-kanban-home-sub--on"
              : "marvi-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__marvi_PLUGINS__ && typeof window.__marvi_PLUGINS__.register === "function") {
    window.__marvi_PLUGINS__.register("kanban", KanbanPage);
  }
})();
