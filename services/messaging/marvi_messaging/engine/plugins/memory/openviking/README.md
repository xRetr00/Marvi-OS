...
- OpenViking server running and reachable from Marvi

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Marvi can identify older servers that expose the legacy status-only health
...
Then configure Marvi:

```bash
marvi memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into Marvi, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:

```bash
marvi config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.marvi/.env`; for a named profile use
`~/.marvi/profiles/<profile>/.env`.
...
# OPENVIKING_AGENT=marvi
```

## Config

OpenViking's server config is separate from Marvi:
...
Marvi-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `marvi` | Marvi peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Marvi lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Marvi sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Marvi built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Marvi action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Marvi
...
file URIs, such as
`viking://user/peers/marvi/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/marvi/memories/preferences/mem_abc123.md`. Files
