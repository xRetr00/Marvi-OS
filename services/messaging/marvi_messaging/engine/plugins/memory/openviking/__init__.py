...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Marvi peer ID in OpenViking (default: marvi)
...
_DEFAULT_AGENT = "marvi"
_AGENT_PROMPT_LABEL = "Marvi peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "MARVI_MESSAGING_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Marvi slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Marvi to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Marvi refused the connection."
        ) from exc
...
def _load_marvi_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("MARVI_MESSAGING_HOME", "")).expanduser() if os.environ.get("MARVI_MESSAGING_HOME") else Path.home() / ".marvi"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Marvi did not start "
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Marvi will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Marvi session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Marvi only", "write values only to Marvi .env"),
    ...
    _save_marvi_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Marvi .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See marvi-messaging#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_marvi_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Marvi peer ID in OpenViking, sent as the actor peer and "
    ...
    def save_config(self, values: Dict[str, Any], marvi_home: str) -> None:
    ...
    def post_setup(self, marvi_home: str, config: dict) -> None:
        ...
        from runtime_support.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        marvi_home_path = Path(marvi_home)
        env_path = marvi_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_marvi_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        marvi_home = str(kwargs.get("marvi_home") or "").strip()
        if not marvi_home:
            try:
                from marvi_constants import get_marvi_home
                marvi_home = str(get_marvi_home())
            except Exception:
                marvi_home = str(Path.home() / ".marvi")
        self._marvi_home = marvi_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Marvi will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.marvi/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts marvi (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Marvi will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._marvi_home:
            return None
        return Path(self._marvi_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._marvi_home:
            return None
        return Path(self._marvi_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_marvi_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Marvi' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Marvi canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See marvi-messaging#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/marvi/.overview.md).
