...
```bash
marvi memory setup    # select "byterover"
```

Or manually:
```bash
marvi config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.marvi/.env
```
...
Working directory: `$MARVI_MESSAGING_HOME/byterover/` (profile-scoped).
