...
the ``marvi`` integration, and applies a chosen manifest to the user's bank
...
def fetch_marvi_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``marvi`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "marvi" in (e.get("integrations") or [])]
