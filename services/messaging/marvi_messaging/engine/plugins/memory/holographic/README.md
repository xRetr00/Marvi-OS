...
```bash
marvi memory setup    # select "holographic"
```

Or manually:
```bash
marvi config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.marvi-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$MARVI_MESSAGING_HOME/memory_store.db` | SQLite database path |
