"""marvi-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $MARVI_MESSAGING_HOME/config.yaml (profile-scoped):
  plugins:
    marvi-memory-store:
      db_path: $MARVI_MESSAGING_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "marvi-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, marvi_home):
        """Write config to config.yaml under plugins.marvi-memory-store."""
        from pathlib import Path
        config_path = Path(marvi_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["marvi-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_marvi_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _marvi_home = str(get_marvi_home())
        _default_db = _marvi_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $MARVI_MESSAGING_HOME in user-supplied paths so config values like
        # "$MARVI_MESSAGING_HOME/memory_store.db" or "~/.marvi/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$MARVI_MESSAGING_HOME", _marvi_home)
            db_path = db_path.replace("${MARVI_MESSAGING_HOME}", _marvi_home)
