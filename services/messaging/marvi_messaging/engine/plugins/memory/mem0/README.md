...
```bash
marvi memory setup    # select "mem0"
```

Or manually:
```bash
marvi config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.marvi/.env
```

## Config

Behavioral settings live in `$MARVI_MESSAGING_HOME/mem0.json` (set them via `marvi memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.marvi/.env`.
...
| `user_id` | `marvi-user` | User identifier on Mem0 |
| `agent_id` | `marvi` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   marvi memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   marvi memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.marvi/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.marvi/.env
   ```
   or in `$MARVI_MESSAGING_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Marvi session and call `mem0_search` — it connects to your server.
...
```bash
marvi memory setup mem0 --mode oss \
...
```bash
marvi memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$MARVI_MESSAGING_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.marvi/mem0_qdrant"}}
  }
}
```
...
```bash
marvi memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
marvi memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$MARVI_MESSAGING_HOME/mem0.json`).
