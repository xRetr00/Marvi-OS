...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/marvi>
...
```bash
marvi memory setup honcho   # configure Honcho directly (works on a fresh install)
marvi memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
marvi config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.marvi/.env
```

> `marvi honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `marvi memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `marvi model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$MARVI_MESSAGING_HOME/honcho.json` | Profile-local (isolated Marvi instances) |
| 2 | `~/.marvi/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Marvi profile: `marvi` (default) or `marvi_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `marvi honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Marvi session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`marvi-messaging`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`marvi`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-marvi-messaging`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening marvi in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Marvi session ID (timestamp + hex). Every `marvi` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Marvi profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`marvi`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Marvi profile: `marvi` (default) or `marvi_<profile>` (e.g. `marvi -p coder` -> host key `marvi_coder`). Older `marvi.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `MARVI_MESSAGING_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `marvi-messaging`) |
...
|---------|-------------|
| `marvi memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `marvi honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `marvi memory setup`) |
| `marvi honcho status` | Show resolved config for active profile |
| `marvi honcho enable` / `disable` | Toggle Honcho for active profile |
| `marvi honcho mode <mode>` | Change recall or observation mode |
| `marvi honcho peer --user <name>` | Update user peer name |
| `marvi honcho peer --ai <name>` | Update AI peer name |
| `marvi honcho tokens --context <N>` | Set context token budget |
| `marvi honcho tokens --dialectic <N>` | Set dialectic max chars |
| `marvi honcho map <name>` | Map current directory to a session name |
| `marvi honcho sync` | Create host blocks for all Marvi profiles |
