...
``marvi://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Marvi OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "marvi-messaging"
...
def begin_authorization(
    ...
    *,
    ...
    (``marvi-desktop`` / ``marvi-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Marvi.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local MARVI_MESSAGING_HOME override can't reach.
