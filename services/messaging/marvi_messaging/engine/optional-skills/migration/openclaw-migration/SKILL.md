...
description: Import an OpenClaw setup (memories, skills) into Marvi.
version: 1.0.0
author: Marvi Messaging (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  marvi:
    tags: [Migration, OpenClaw, Marvi, Memory, Persona, Import]
    related_skills: [marvi-messaging]
---

# OpenClaw -> Marvi Migration

Use this skill when a user wants to move their OpenClaw setup into Marvi Messaging with minimal manual cleanup.
...
```bash
marvi claw migrate              # Full interactive migration
marvi claw migrate --dry-run    # Preview what would be migrated
marvi claw migrate --preset user-data   # Migrate without secrets
marvi claw migrate --overwrite  # Overwrite existing conflicts
marvi claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `marvi setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_marvi.py` to:

- import `SOUL.md` into the Marvi home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Marvi memory entries
- merge OpenClaw command approval patterns into Marvi `command_allowlist`
- migrate Marvi-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Marvi working-directory configuration
- copy OpenClaw skills into `~/.marvi/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Marvi workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.marvi/tts/`
- archive non-secret docs that do not have a direct Marvi destination
...
- `scripts/openclaw_to_marvi.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.marvi/skills/migration/openclaw-migration/scripts/openclaw_to_marvi.py`

Do not guess a shorter path like `~/.marvi/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.marvi/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Marvi-compatible secrets, currently:
...
Marvi CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Marvi. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Marvi workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Marvi skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.marvi/skills/migration/openclaw-migration/scripts/openclaw_to_marvi.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.marvi/skills/migration/openclaw-migration/scripts/openclaw_to_marvi.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.marvi/skills/migration/openclaw-migration/scripts/openclaw_to_marvi.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.marvi/skills/migration/openclaw-migration/scripts/openclaw_to_marvi.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.marvi/skills/migration/openclaw-migration/scripts/openclaw_to_marvi.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.marvi/skills/migration/openclaw-migration/scripts/openclaw_to_marvi.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Marvi unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Marvi targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Marvi destination. Unsupported auth blobs must still be reported as skipped.
...
- Marvi persona state imported
- Marvi memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.marvi/skills/openclaw-imports/`
