#!/usr/bin/env python3
"""OpenClaw -> Marvi migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Marvi Messaging, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Marvi.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Marvi memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Marvi memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Marvi-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Marvi-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Marvi command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.marvi/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.marvi/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Marvi .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Marvi .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Marvi .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Marvi .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Marvi .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Marvi config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Marvi config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Marvi.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Marvi MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Marvi config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'marvi cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Marvi config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Marvi session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Marvi custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Marvi config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Marvi config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Marvi config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `marvi config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`marvi config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.marvi/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Marvi config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Marvi in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Marvi`` (prose), but lowercase matches
# like ``openclaw`` → ``marvi`` (so filesystem paths like ``~/.openclaw``
# become ``~/.marvi`` — the real Marvi home — not the broken ``~/.Marvi``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Marvi'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Marvi'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Marvi'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Marvi`` but maps ``openclaw`` → ``marvi`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.marvi/config.yaml`` (the real Marvi home) instead of the broken
    ``~/.Marvi/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Marvi.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Marvi memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Marvi Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-marvi/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Marvi config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-marvi/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Marvi session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Marvi config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Marvi
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Marvi-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Marvi-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: marvi config set",
                        )
                    continue
        ...
        for oc_key, marvi_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and marvi_key not in secret_additions:
                secret_additions[marvi_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, marvi_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and marvi_key not in secret_additions:
                        secret_additions[marvi_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        marvi_config = load_yaml_file(destination)
        current_model = marvi_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = marvi_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                marvi_config["model"] = {"default": model_str}
            dump_yaml_file(destination, marvi_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Marvi still uses "edge"
        ...
        marvi_config = load_yaml_file(destination)
        existing_tts = marvi_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Marvi destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Marvi-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Marvi-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        marvi_cfg_path = self.target_root / "config.yaml"
        marvi_cfg = load_yaml_file(marvi_cfg_path)
        existing_mcp = marvi_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Marvi config")
                continue

            marvi_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                marvi_srv["command"] = srv["command"]
                if srv.get("args"):
                    marvi_srv["args"] = srv["args"]
                if srv.get("env"):
                    marvi_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    marvi_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                marvi_srv["url"] = srv["url"]
                if srv.get("headers"):
                    marvi_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    marvi_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                marvi_srv["enabled"] = False
            if srv.get("timeout"):
                marvi_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                marvi_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                marvi_srv["tools"] = {}
                if tools_cfg.get("include"):
                    marvi_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    marvi_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(marvi_cfg_path)
            marvi_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(marvi_cfg_path, marvi_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'marvi cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = marvi_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Marvi reasoning_effort
        ...
        if compaction:
            compression = marvi_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'marvi gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("MARVI_MESSAGING_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        marvi_cfg_path = self.target_root / "config.yaml"
        marvi_cfg = load_yaml_file(marvi_cfg_path)
        sr = marvi_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        marvi_cfg_path = self.target_root / "config.yaml"
        marvi_cfg = load_yaml_file(marvi_cfg_path)
        custom_providers = marvi_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(marvi_cfg_path)
            marvi_cfg["custom_providers"] = custom_providers
            dump_yaml_file(marvi_cfg_path, marvi_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Marvi config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            marvi_cfg_path = self.target_root / "config.yaml"
            marvi_cfg = load_yaml_file(marvi_cfg_path)
            discord_marvi = marvi_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_marvi["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_marvi["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                marvi_cfg["discord"] = discord_marvi
                dump_yaml_file(marvi_cfg_path, marvi_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        marvi_cfg_path = self.target_root / "config.yaml"
        marvi_cfg = load_yaml_file(marvi_cfg_path)
        browser_marvi = marvi_cfg.get("browser") or {}
        changed = False

        # Map fields that have Marvi equivalents
        if browser.get("cdpUrl"):
            browser_marvi["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_marvi["headless"] = browser["headless"]
            changed = True

        if changed:
            marvi_cfg["browser"] = browser_marvi
            if self.execute:
                self.maybe_backup(marvi_cfg_path)
                dump_yaml_file(marvi_cfg_path, marvi_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            marvi_mode = mode_map.get(mode, "manual")
            marvi_cfg.setdefault("approvals", {})["mode"] = marvi_mode
            if self.execute:
                self.maybe_backup(marvi_cfg_path)
                dump_yaml_file(marvi_cfg_path, marvi_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{marvi_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Marvi Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Marvi. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Marvi Config Not Overwritten)",
                "",
                "These items already existed in your Marvi config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Marvi Messaging discovers these",
            "directories, it may read/write to them instead of the Marvi state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `marvi claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `marvi claw cleanup` to fix it.",
            "",
            "## Marvi-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `marvi claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `marvi setup` to configure any remaining settings",
            "- Run `marvi mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `marvi cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `marvi cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `marvi gateway install` if you need the gateway service",
            "- Review `~/.marvi/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Marvi Messaging.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("MARVI_MESSAGING_HOME") or str(Path.home() / ".marvi"), help="Marvi home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Marvi targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Marvi-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Marvi Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.marvi/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.marvi/config.yaml")
        print("    2. Run: marvi mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: marvi cron")
