...
Usage (via cron with --no-agent):
    ...
    marvi cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$MARVI_MESSAGING_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Marvi-Watcher/1.0"})
