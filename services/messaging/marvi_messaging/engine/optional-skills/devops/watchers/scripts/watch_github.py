...
Usage (via cron with --no-agent):
    ...
    marvi cron create marvi-issues \\

      --schedule "*/5 * * * *" --no-agent \\

      --script "$MARVI_MESSAGING_HOME/skills/devops/watchers/scripts/watch_github.py" \\

      --script-args "--name marvi-issues --repo xRetr00/Marvi --scope issues"



Set GITHUB_TOKEN (or GH_TOKEN) in the Marvi .env file

(``${MARVI_MESSAGING_HOME:-~/.marvi}/.env``) to avoid the 60 req/hr
