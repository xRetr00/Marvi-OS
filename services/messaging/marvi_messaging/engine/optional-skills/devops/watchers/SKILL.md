...
All three live in `$MARVI_MESSAGING_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$MARVI_MESSAGING_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
python $MARVI_MESSAGING_HOME/skills/devops/watchers/scripts/watch_rss.py \
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `${MARVI_MESSAGING_HOME:-~/.marvi}/.env` to avoid the 60 req/hr anonymous rate limit):



```bash

python $MARVI_MESSAGING_HOME/skills/devops/watchers/scripts/watch_github.py \

  --name marvi-issues --repo xRetr00/Marvi --scope issues
...
python $MARVI_MESSAGING_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$MARVI_MESSAGING_HOME/watcher-state/<name>.json`. Inspect:



```bash

cat $MARVI_MESSAGING_HOME/watcher-state/hn.json
...
rm $MARVI_MESSAGING_HOME/watcher-state/hn.json
...
4. **Putting the state dir where the agent's sandbox can't write.** `$MARVI_MESSAGING_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
