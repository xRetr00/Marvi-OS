...
version: 0.1.0
author: Hugo Sequier (Hugo-SEQUIER), Marvi Messaging
license: MIT
...
The script reads `${MARVI_MESSAGING_HOME:-~/.marvi}/.env` for two optional defaults:
...
Helper script: `~/.marvi/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.marvi/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${MARVI_MESSAGING_HOME:-~/.marvi}/.env`.
...
```bash
python ~/.marvi/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.marvi/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
