...
version: 0.2.0
author: Tony Simons (asimons81), Marvi Messaging
license: MIT
platforms: [linux, macos, windows]
metadata:
  marvi:
    tags: [Coding-Agent, Antigravity, CLI, Auth, Plugins, Sandbox]
    related_skills: [grok, codex, claude-code, marvi-messaging]
---

# Antigravity CLI (`agy`)

Operator guide for the Antigravity CLI, invoked as `agy`. Run all `agy`
commands through the Marvi `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure — it does not wrap a network
API, so there is nothing to authenticate from Marvi itself.
