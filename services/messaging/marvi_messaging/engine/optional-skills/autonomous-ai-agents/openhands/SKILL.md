...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Marvi Messaging
license: MIT
platforms: [linux, macos]
metadata:
  marvi:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, marvi-messaging]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Marvi.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Marvi-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `marvi-messaging` (Marvi subagents via `delegate_task`).
