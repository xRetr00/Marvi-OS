---
name: honcho
description: Configure and troubleshoot Honcho memory for Marvi.
version: 2.0.0
...
Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Marvi profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/marvi#running-honcho-locally-with-marvi

### Verify

```bash
marvi honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Marvi profile's AI peer
...
Honcho models conversations as interactions between **peers**. Marvi creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Marvi instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Marvi run |
| `global` | Single session across all directories |

Manual override: `marvi honcho map my-project-name`
...
Each Marvi profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
marvi profile create coder --clone
# creates host block marvi.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `marvi.coder` host block in `honcho.json`
...
```bash
marvi honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Marvi when Honcho memory is active.
...
Config file: `$MARVI_MESSAGING_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `marvi honcho setup`. Ensure `memory.provider: honcho` is in `~/.marvi/config.yaml`.

### Memory not persisting across sessions
Check `marvi honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `marvi profile create <name> --clone`. For existing profiles: `marvi honcho sync`.
...
| Command | Description |
|---------|-------------|
| `marvi honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `marvi honcho status` | Show resolved config, connection test, peer info for active profile |
| `marvi honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `marvi honcho disable` | Disable Honcho for the active profile |
| `marvi honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `marvi honcho peers` | Show peer identities across all profiles |
| `marvi honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `marvi honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `marvi honcho sessions` | List known directory-to-session-name mappings |
| `marvi honcho map <name>` | Map current working directory to a Honcho session name |
| `marvi honcho identity` | Seed AI peer identity or show both peer representations |
| `marvi honcho sync` | Create host blocks for all Marvi profiles that don't have one yet |
| `marvi honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Marvi + Honcho |
| `marvi memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `marvi memory status` | Show active memory provider and config |
| `marvi memory off` | Disable external memory provider |
