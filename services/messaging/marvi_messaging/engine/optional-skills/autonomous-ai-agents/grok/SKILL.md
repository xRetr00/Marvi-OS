...
1. Prepare stable input files first with Marvi tools (`read_file`,
...
2. **Don't conflate Marvi' xAI auth with the `grok` CLI's auth.** Marvi'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Marvi' xAI auth carries
   over.
