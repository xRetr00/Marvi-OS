...
The design is **Marvi-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
marvi skills install official/security/unbroker
```

Then start a new Marvi session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Marvi home directory (`0600`, optionally
