...
All per-subject data lives under PDD_DATA_DIR (default: $MARVI_MESSAGING_HOME/unbroker),
which is the same trust boundary Marvi uses for .env and OAuth tokens.
"""
...
def marvi_home() -> Path:
    return Path(os.environ.get("MARVI_MESSAGING_HOME") or (Path.home() / ".marvi"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else marvi_home() / "unbroker"
