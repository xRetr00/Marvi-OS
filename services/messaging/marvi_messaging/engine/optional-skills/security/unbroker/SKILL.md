...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  one it detects, reading credentials from the shell env **and from `$MARVI_MESSAGING_HOME/.env`** so keys
  Marvi already loads for its own tools are picked up without re-exporting - each one converts a
  ...
  - Email automation, two credential-free-or-not options:
    - **Browser mode (no password): `setup --email-mode browser`.** The agent sends opt-out/CCPA
      ...
      `browser_*` tools. Nothing is stored. This requires Marvi to be pointed at the operator's own
      ...
      `chrome --remote-debugging-port=9222 --user-data-dir="$HOME/.marvi/chrome-debug"` (a dedicated
...
The engine stores data under `$PDD_DATA_DIR` (default `$MARVI_MESSAGING_HOME/unbroker`), written
...
- **Dossiers are plaintext by default** (JSON, `0600` under `MARVI_MESSAGING_HOME`). For at-rest encryption run
  ...
  a full-`MARVI_MESSAGING_HOME` read; set `PDD_AGE_IDENTITY` to a separate volume for real key separation.
