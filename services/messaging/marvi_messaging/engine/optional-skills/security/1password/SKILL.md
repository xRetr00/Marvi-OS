...
version: 1.0.0
author: arceus77-7, enhanced by Marvi Messaging
license: MIT
...
- `tmux` available for stable authenticated sessions during Marvi terminal calls (desktop app flow only)
...
### Service Account (recommended for Marvi)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${MARVI_MESSAGING_HOME:-~/.marvi}/.env` (the skill will prompt for this on first load).
...
Marvi terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/marvi-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/marvi-op.sock"
