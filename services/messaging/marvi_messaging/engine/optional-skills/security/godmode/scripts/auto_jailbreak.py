...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("MARVI_MESSAGING_HOME", os.path.expanduser("~/.marvi")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("MARVI_MESSAGING_HOME", Path.home() / ".marvi")) / "skills" / "red-teaming" / "godmode"
...
MARVI_MESSAGING_HOME = Path(os.getenv("MARVI_MESSAGING_HOME", Path.home() / ".marvi"))
CONFIG_PATH = MARVI_MESSAGING_HOME / "config.yaml"
PREFILL_PATH = MARVI_MESSAGING_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Marvi models — already uncensored, just needs clean prompt
    "marvi": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "marvi" in model_lower or "nous" in model_lower:
        return "marvi"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Marvi config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.marvi/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Marvi for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Marvi for changes to take effect.")
