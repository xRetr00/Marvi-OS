...
## 5. GODMODE FAST — Marvi 4 405B (Zero Refusal)

**Model:** `nousresearch/marvi-4-405b`
**Strategy:** Marvi models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Marvi Messaging

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.marvi/config.yaml`:
