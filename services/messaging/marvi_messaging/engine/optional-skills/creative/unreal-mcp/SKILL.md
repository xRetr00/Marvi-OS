...
Companion skill for the `unreal-engine` entry in the Marvi MCP catalog. The
...
Two halves, in this order: the editor side must be up before Marvi connects.
...
### One-time, Marvi side
    ...
    marvi mcp install unreal-engine
...
`~/.marvi/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Marvi — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Marvi connects
...
2. Start the Marvi session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Marvi
they appear as:
...
then happens at `marvi mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Marvi runs batched calls
...
- **Start order matters.** Editor + server up first, then the Marvi
...
- **Timeouts on long operations.** Marvi' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.marvi/config.yaml` for
