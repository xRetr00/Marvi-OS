...
### 1. Start order: editor first, Marvi second

Marvi probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Marvi session.
...
restart the editor, restart the Marvi session.
...
server fails to bind (Output Log) or Marvi' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.marvi/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Marvi

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Marvi' connection
lives in `~/.marvi/config.yaml` via `marvi mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Marvi — just don't
mistake it for the Marvi setup step.
...
explicitly warns against overlapping calls. Marvi executes same-turn tool
...
### 9. Timeouts: Marvi gives up before Unreal does

Marvi' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Marvi runs elsewhere (SSH backend,
