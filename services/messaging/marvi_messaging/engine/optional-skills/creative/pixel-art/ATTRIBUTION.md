...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Marvi uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Marvi has `image_generate`
...
- License: MIT (inherits from marvi-messaging repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Marvi Messaging contributors
