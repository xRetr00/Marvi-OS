#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `marvi kanban list` and
...
def marvi_available() -> bool:
    return shutil.which("marvi") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed marvi CLI."""
    try:
        out = subprocess.run(
            ["marvi", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `marvi kanban list`
    out = subprocess.run(
        ["marvi", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["marvi", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not marvi_available():
        print("ERROR: 'marvi' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
