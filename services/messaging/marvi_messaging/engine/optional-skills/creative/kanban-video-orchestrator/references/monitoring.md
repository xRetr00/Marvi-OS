...
marvi kanban watch --tenant <project-slug>

# Snapshot of the board
marvi kanban list --tenant <project-slug>
marvi kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
marvi kanban stats --tenant <project-slug>

# Visual dashboard (browser)
marvi dashboard

# Inspect a specific task (includes comments + events)
marvi kanban show <task-id>

# Follow a single task's event stream
marvi kanban tail <task-id>
```

Verify available subcommands with `marvi kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `marvi kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `marvi kanban block <id>` then `marvi kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `marvi kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `marvi kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `marvi profile list` (profile exists?), `marvi status` (gateway/dispatcher up?), and verify tenant. |
...
marvi kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
marvi kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(marvi kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
marvi kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
marvi kanban block <task-id>
marvi kanban archive <task-id>

# Diagnose what happened
marvi kanban show <task-id>      # task body, comments, recent events
marvi kanban tail <task-id>      # follow the live event stream
marvi kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `marvi kanban create` for the director
...
while true; do
    clear
    marvi kanban list --tenant <slug>
    echo "---"
    marvi kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `marvi kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `marvi profile list` and start
