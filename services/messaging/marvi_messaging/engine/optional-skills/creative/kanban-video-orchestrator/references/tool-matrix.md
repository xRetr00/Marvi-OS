...
Maps each role archetype to the Marvi skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public marvi-messaging
...
### Visual / rendering skills (`marvi-messaging/skills/creative/`)
...
### Design / pre-production skills (`marvi-messaging/skills/creative/`)
...
### Audio / media skills (`marvi-messaging/skills/creative/` + `skills/media/`)
...
These are **not** Marvi skills but external CLIs / APIs that profiles invoke.
...
These are native Marvi tools — not invoked via terminal but through their own
...
key is present in `${MARVI_MESSAGING_HOME:-~/.marvi}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Marvi profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${MARVI_MESSAGING_HOME:-~/.marvi}/.env` → environment variable.
...
When a new Marvi-public video skill ships:
