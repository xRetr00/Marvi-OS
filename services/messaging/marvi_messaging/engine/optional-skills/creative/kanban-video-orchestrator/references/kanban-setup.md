...
the actual `setup.sh` that creates the project workspace, configures Marvi
...
2. **Create profiles** — `marvi profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.marvi/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `marvi kanban create` assigned to the director
...
```bash
marvi profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.marvi/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.marvi/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Marvi
...
Each profile gets a `SOUL.md` at `~/.marvi/profiles/<name>/SOUL.md` that
...
```bash
marvi kanban create "Direct production of <video title>" \
    --assignee director \
...
the Marvi `.env` (`${MARVI_MESSAGING_HOME:-$HOME/.marvi}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _marvi_env="${MARVI_MESSAGING_HOME:-$HOME/.marvi}/.env"
    if grep -q "^${var}=" "$_marvi_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_marvi_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_marvi_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY marvi ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY marvi OPENROUTER_API_KEY || exit 1
# ...
```
