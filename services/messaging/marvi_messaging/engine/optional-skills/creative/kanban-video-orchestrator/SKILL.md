...
short to a music video to an ASCII loop — in a Marvi Kanban pipeline that
...
3. **Generates** a setup script that creates Marvi profiles, project workspace, and the initial kanban task
...
For mapping role → which Marvi skills + toolsets it loads, see
...
3. Creates each Marvi profile via `marvi profile create --clone`
...
7. Fires the initial `marvi kanban create` task assigned to the director
...
```bash
marvi kanban watch --tenant <project-tenant>     # live events
marvi kanban list  --tenant <project-tenant>     # board snapshot
marvi dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `marvi kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${MARVI_MESSAGING_HOME:-~/.marvi}/.env` or the user's secret store.
