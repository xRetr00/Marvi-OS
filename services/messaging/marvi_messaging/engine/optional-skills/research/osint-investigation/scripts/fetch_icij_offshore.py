...
$MARVI_MESSAGING_OSINT_CACHE/icij/ (default: ~/.cache/marvi-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("MARVI_MESSAGING_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "marvi-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "marvi-messaging osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $MARVI_MESSAGING_OSINT_CACHE/icij or ~/.cache/marvi-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
