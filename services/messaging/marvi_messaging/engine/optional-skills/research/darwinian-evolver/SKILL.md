...
version: 0.1.0
author: Bihruze (Asahi0x), Marvi Messaging
license: MIT
...
import upstream classes into Marvi itself.
...
```bash
mkdir -p ~/.marvi/cache/darwinian-evolver && cd ~/.marvi/cache/darwinian-evolver
...
```bash
cd ~/.marvi/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.marvi/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.marvi/skills/research/darwinian-evolver
DE_DIR=~/.marvi/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Marvi core.
   Custom driver scripts under `~/.marvi/skills/...` are user-side and fine.
