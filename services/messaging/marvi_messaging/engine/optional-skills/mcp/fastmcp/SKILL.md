...
metadata:
  marvi:
    ...
    related_skills: [marvi-messaging, mcporter]
prerequisites:
  commands: [python]
---
...
- smoke-test a server with the FastMCP CLI before wiring it into Marvi or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Marvi. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.marvi/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.marvi/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Marvi integration, either:

- configure the server in `~/.marvi/config.yaml` using the `native-mcp` skill, or
...
The server-building part may be correct while the Marvi config is not. Load the `native-mcp` skill and configure the server in `~/.marvi/config.yaml`, then restart Marvi.
