...
Usage:
  ...
  <server_name> matches the files in $MARVI_MESSAGING_HOME/mcp-tokens/<server>.json etc.
...
def _marvi_home():
    # Prefer Marvi' own resolver (profile-safe); fall back to env then ~/.marvi.
    try:
        from marvi_constants import get_marvi_home
        return str(get_marvi_home())
    except Exception:
        return os.environ.get("MARVI_MESSAGING_HOME") or os.path.expanduser("~/.marvi")


def _tokens_dir():
    return os.path.join(_marvi_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "marvi-diag", "version": "1.0"}},
        },
        headers={
