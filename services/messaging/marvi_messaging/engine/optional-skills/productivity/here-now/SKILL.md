...
- Skill helper paths:
  - `${MARVI_MESSAGING_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${MARVI_MESSAGING_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${MARVI_MESSAGING_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client marvi
```
...
```bash
PUBLISH="${MARVI_MESSAGING_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client marvi
```
...
```bash
DRIVE="${MARVI_MESSAGING_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `marvi`)    |
