#!/usr/bin/env python3
"""Canvas LMS API CLI for Marvi Messaging.
...
def _check_config():
    ...
    if missing:
        marvi_env = os.path.join(
            os.environ.get("MARVI_MESSAGING_HOME", os.path.expanduser("~/.marvi")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {marvi_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Marvi Messaging"
    )
