...
This optional skill gives Marvi practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${MARVI_MESSAGING_HOME:-~/.marvi}/.env`
...
It does **not** turn Marvi into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Marvi memory
...
- pairs well with Marvi `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${MARVI_MESSAGING_HOME:-~/.marvi}/.env`
...
```bash
SCRIPT="$(find ~/.marvi/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
marvi skills search telephony
marvi skills install official/productivity/telephony
```
...
Then save credentials into Marvi:
...
3. Buy it and save it into `${MARVI_MESSAGING_HOME:-~/.marvi}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Marvi calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Marvi's existing `text_to_speech` support.

Use this when:
- you want the call to use Marvi's configured TTS voice rather than Twilio `<Say>`
...
Recommended Marvi TTS -> Twilio Play workflow:

1. Generate the audio with Marvi `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Marvi to create the message audio with `text_to_speech`
...
Important note:
- Marvi TTS is great for prerecorded outbound messages
...
- Marvi STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Marvi memory.
...
- Do not store arbitrary third-party phone numbers in Marvi memory.
...
3. persist that number to `${MARVI_MESSAGING_HOME:-~/.marvi}/.env`
