...
class SessionSearchMixin:
    ...
    def search_messages(
        ...
        Threshold: MARVI_MESSAGING_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("MARVI_MESSAGING_SEARCH_SLOW_MS", "1000"))
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Marvi surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``MARVI_MESSAGING_DISABLE_FTS_TRIGRAM`` or not yet created), so
