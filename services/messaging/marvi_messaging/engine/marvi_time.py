"""
Timezone-aware clock for Marvi.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``MARVI_MESSAGING_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.marvi/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Marvi never
...
from marvi_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Marvi requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("MARVI_MESSAGING_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``MARVI_MESSAGING_TIMEZONE`` update) to force ``get_timezone()`` /
