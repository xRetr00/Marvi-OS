#!/usr/bin/env python3
"""
SQLite State Store for Marvi Messaging.
...
from runtime_support.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Tuple, TypeVar

from marvi_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from marvi_state_portability import SessionPortabilityMixin
from marvi_state_schema import SessionSchemaMixin
from marvi_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``marvi -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_marvi_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.marvi`` even when a test fixture later
    redirects ``MARVI_MESSAGING_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_marvi_home()`` fresh so a runtime
       ``MARVI_MESSAGING_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_marvi_home() / "state.db"
...
# ~/.marvi/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects MARVI_MESSAGING_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without MARVI_MESSAGING_HOME, a stale worktree
# without the re-pin, or a developer shell that exports MARVI_MESSAGING_HOME to the
...
#: ``~/.marvi``).  The test conftest injects the pre-sandbox production
#: root here so custom-``MARVI_MESSAGING_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Marvi root for the guard.

    Deliberately avoids ``Path.home()`` / ``marvi_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``marvi_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "marvi"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "marvi"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".marvi"
        return root.resolve()
    except Exception:
        return None
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Marvi home *root*.
    ...
    under ``~/.marvi/marvi-messaging/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``MARVI_MESSAGING_HOME``) paths.
    """
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Marvi root {root}). "
                "Tests must run against a temporary MARVI_MESSAGING_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "MARVI_MESSAGING_HOME. If this test genuinely needs the live "
...
# runtime_support/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``marvi sessions optimize`` (FTS merge + VACUUM)
    ...
    ``runtime_support/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``runtime_support.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``marvi update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Marvi-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Marvi"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Marvi-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `marvi doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see runtime_support/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with runtime_support.config.
...
# threads inside ONE interpreter, yet a normal Marvi host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``marvi serve`` backend, interactive CLI sessions,
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``runtime_support.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Marvi home tree (``get_marvi_home()``) — the safe repair scope:
      Marvi owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``runtime_support.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_marvi_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Marvi needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (marvi_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_marvi_state.py:600-632)
        ...
        probe_session_id = f"_marvi_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.marvi/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``marvi sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("MARVI_MESSAGING_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_marvi_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("MARVI_MESSAGING_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship marvi_state without runtime_support). A
    ...
    try:
        ...
        logger.debug(
            "runtime_support.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch marvi_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``runtime_support.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from runtime_support.backup import is_zeroed_sqlite_file
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `marvi snapshot list` / "
                "`marvi snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (marvi doctor, dashboards) ──────────
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple marvi processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Marvi
    ...
    # `marvi update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `marvi snapshot list` / "
                    f"`marvi snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`marvi db optimize`),
            ...
            # ``marvi_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `marvi update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `marvi sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Marvi process held the "
    ...
    #
    # `optimize_fts_storage()` (the `marvi sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`marvi sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``marvi sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``marvi --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: marvi_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``marvi -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``marvi sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``MARVI_MESSAGING_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Marvi, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Marvi session.

        A Marvi session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Marvi session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `marvi sessions vacuum` runs in a transient
