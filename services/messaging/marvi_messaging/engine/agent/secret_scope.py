...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the MARVI_MESSAGING_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Marvi runtime / deployment
    "MARVI_MESSAGING_HOME", "MARVI_MESSAGING_PROFILE", "MARVI_MESSAGING_GATEWAY_LOCK_DIR",
    "MARVI_MESSAGING_MAX_ITERATIONS", "MARVI_MESSAGING_MAX_TOKENS", "MARVI_MESSAGING_API_TIMEOUT",
    "MARVI_MESSAGING_REDACT_SECRETS", "MARVI_MESSAGING_NOUS_TIMEOUT_SECONDS",
    "_MARVI_MESSAGING_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "MARVI_MESSAGING_KANBAN_DB", "MARVI_MESSAGING_KANBAN_WORKSPACES_ROOT", "MARVI_MESSAGING_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "MARVI_MESSAGING_KANBAN_",
    "MARVI_MESSAGING_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Marvi writes
    ...
    ``runtime_support.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Marvi parser: save_env_value
    ...
    from runtime_support.config import _parse_env_value
...
def build_profile_secret_scope(marvi_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(marvi_home)
    secrets = load_env_file(home / ".env")

    try:
        from runtime_support.env_loader import get_secret_source_values
