...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        from toolsets import _MARVI_MESSAGING_CORE_TOOLS

        _core_tool_names = set(_MARVI_MESSAGING_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Marvi expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile MARVI_MESSAGING_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped MARVI_MESSAGING_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``marvi_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_marvi_home()`` themselves.
        """
        if "marvi_home" not in kwargs:
            from marvi_constants import get_marvi_home
            kwargs["marvi_home"] = str(get_marvi_home())
        for provider in self._providers:
            try:
