...
``MARVI_MESSAGING_HOME/memory/episodic.db`` (a sibling of ``MARVI_MESSAGING_HOME/brain/``).
...
# from a profile switch (MARVI_MESSAGING_HOME override) never lingers.
_lock = threading.RLock()


def _db_path() -> Path:
    return get_marvi_home() / "memory" / _DB_FILENAME
...
# hook (runtime_support/plugins.py, VALID_HOOKS) instead of editing gateway/run.py
...
def _cheap_session_summary(session_id: str) -> tuple[str, str]:
    ...
    (see ``marvi_state.SessionDB``). Returns ``("", "")`` when the session
    can't be found or has no title — callers treat an empty title as
    "nothing worth recording".
    """
    try:
        from marvi_state import SessionDB, DEFAULT_DB_PATH
