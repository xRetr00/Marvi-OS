...
upstream prefix cache warm.  See ``marvi-messaging-dev``'s
...
from agent.prompt_builder import (
    ...
    MARVI_MESSAGING_AGENT_HELP_GUIDANCE,
    KANBAN_GUIDANCE,
    MEMORY_GUIDANCE,
...
from marvi_constants import get_default_marvi_root, get_marvi_home
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- marvi-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``MARVI_MESSAGING_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("MARVI_MESSAGING_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the MARVI_MESSAGING_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from runtime_support.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from runtime_support.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound MARVI_MESSAGING_HOME ContextVar override wins. Surfaces that multiplex
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Marvi never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want MARVI_MESSAGING_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the MARVI_MESSAGING_HOME ContextVar
    ...
    # Pointer to the marvi-messaging skill + docs for user questions about Marvi itself.
    stable_parts.append(MARVI_MESSAGING_AGENT_HELP_GUIDANCE)
    ...
    # MARVI_MESSAGING_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Marvi, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Marvi profile the agent is running
    # under so it doesn't conflate ~/.marvi/skills/ (default profile) with
    # ~/.marvi/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient MARVI_MESSAGING_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_marvi_root(): get_marvi_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_marvi_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_marvi_root())
    else:
        _home_str = _root_str = str(get_marvi_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Marvi profile: default. Other profiles (if any) live "
        ...
        # data sits at the ROOT (get_default_marvi_root()), which in ambient
        # profile mode is NOT get_marvi_home().
        profile_home = _home_str
        default_root = get_default_marvi_root()
        post_workspace_parts.append(
            f"Active Marvi profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Marvi). Every other surface (desktop chat panel,
