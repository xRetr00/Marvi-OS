...
``auth.json``.  They intentionally have no dependency on ``runtime_support.auth`` so
...
# Sources Marvi owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Marvi-owned OAuth/device-code state)
