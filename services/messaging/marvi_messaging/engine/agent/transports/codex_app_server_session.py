...
Owns one Codex thread per Marvi session. Drives `turn/start`, consumes
...
# Marvi' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_MARVI_MESSAGING_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Marvi/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Marvi session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _MARVI_MESSAGING_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("MARVI_MESSAGING_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="marvi",
            client_title="Marvi Messaging",
            client_version=_get_marvi_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Marvi' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Marvi' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own marvi-tools callback we
            # auto-accept — the user already approved Marvi' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Marvi
        approval-mode/timeout logic. The Marvi-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Marvi approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Marvi approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Marvi returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Marvi approval mode/timeout resolution
    ...
    # "prompt expired"; the Marvi-side messaging already distinguishes them.
    return "decline"
...
def _get_marvi_version() -> str:
    """Best-effort Marvi version string for codex's userAgent line."""
    try:
