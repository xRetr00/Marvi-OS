...
projection into Marvi' display, approval bridging, transcript projection into
...
"codex_app_server"`. Marvi' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Marvi secret — gateway
        ...
        spawn_env = marvi_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("MARVI_MESSAGING_KANBAN_TASK"):
            kanban_db = spawn_env.get("MARVI_MESSAGING_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "MARVI_MESSAGING_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("MARVI_MESSAGING_HOME", os.path.expanduser("~/.marvi")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "marvi",
        client_title: str = "Marvi Messaging",
