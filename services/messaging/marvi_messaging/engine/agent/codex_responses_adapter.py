...
def _clamp_responses_call_id(call_id: str) -> str:
    ...
    component the built-in ``marvi-tools`` server already overflows 64 chars,
...
# output items.  Marvi injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while Marvi-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Marvi threading encrypted reasoning back across
    ...
    ``convert_messages``). Dropping the checkpoint costs nothing: Marvi'
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Marvi-side metadata key and not part
...
def _normalize_codex_response(
    ...
    *,
    ...
    if tool_calls:
        ...
        # See https://github.com/xRetr00/Marvi-OS/issues/64434
