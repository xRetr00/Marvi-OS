...
cache problem must never block Marvi startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Marvi home used for cache paths.

    ``home_path`` is whatever ``load_marvi_dotenv()`` already resolved;
    falling back to ``$MARVI_MESSAGING_HOME`` / ``~/.marvi`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from marvi_constants import get_marvi_home

        home_path = get_marvi_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<marvi_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if MARVI_MESSAGING_HOME is ever made traversable.
            try:
