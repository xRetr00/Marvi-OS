...
``CommandSecretsProvider`` (marvi-desktop ``src/main/secrets/commandProvider.ts``)
...
* The requested key is passed to the child ONLY via the ``MARVI_MESSAGING_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``MARVI_MESSAGING_SECRET_KEY``) — it is never called per-key in a loop, so a
...
# runtime_support.env_loader can consume both providers identically.
...
def _run_helper(
    ...
    The key is passed as DATA via ``MARVI_MESSAGING_SECRET_KEY`` — never interpolated
    ...
    if source_env is os.environ:
        ...
        env = dict(source_env)
    env["MARVI_MESSAGING_SECRET_KEY"] = secret_key
...
def get_command_secret(
    *,
    ...
    ``MARVI_MESSAGING_SECRET_KEY``.  Returns None on any failure — never raises."""
...
def apply_command_secrets(
    *,
    ...
    # MARVI_MESSAGING_SECRET_KEY and parse its stdout as a dotenv blob.
    stdout = _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
...
class CommandSource(SecretSource):
    ...
    Bulk shape: the helper enumerates a KEY=VALUE blob in one run.  Config::
        ...
        secrets:
          command:
            enabled: true
            command: "cat /run/user/1000/marvi-secrets.env"
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "Marvi discards helper stderr so diagnostics can't leak "
                "secret material."
            )
