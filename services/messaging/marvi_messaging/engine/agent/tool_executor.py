...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Marvi process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    in config.yaml wins, the legacy ``MARVI_MESSAGING_CONCURRENT_TOOL_TIMEOUT_S`` env var
    ...
    return resolve_timeout(
        ...
        env_var="MARVI_MESSAGING_CONCURRENT_TOOL_TIMEOUT_S",
    )
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Marvi process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Marvi policy and dispatch exactly once."""
    from agent import relay_tools
    from runtime_support.middleware import (
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state["dispatched"]:
                raise RuntimeError(
                    "Marvi tool execution callback invoked more than once"
                )
    ...
    def _marvi_pipeline(relay_args: dict[str, Any]) -> Any:
...
def _resolve_sequential_tool_timeout() -> float | None:
    ...
    ``MARVI_MESSAGING_CONCURRENT_TOOL_TIMEOUT_S`` legacy bridge) so the two executor
