...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``MARVI_MESSAGING_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("MARVI_MESSAGING_VERIFY_ON_STOP")
    if env is not None:
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether the workspace has a runtime verify recipe ``marvi verify`` can run.

    True when a saved ``.marvi/environment.json`` manifest exists, or when
    ...
    if verify_commands:
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction += (
                ...
                "start + readiness), prefer `marvi verify --json` — a passing "
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction = (
                ...
                "project has a runnable verification recipe. Run `marvi verify "
            ...
            command_instruction = (
                ...
                "`tempfile` path with a `marvi-verify-` filename prefix, run it "
