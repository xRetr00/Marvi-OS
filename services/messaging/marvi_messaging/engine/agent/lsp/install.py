...
appropriate.  All installs go to a Marvi-owned bin staging dir,
``<MARVI_MESSAGING_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``marvi lsp
  status``.
...
# ``<MARVI_MESSAGING_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `marvi lsp status` reports the host's presence.
...
def marvi_lsp_bin_dir() -> Path:
    """Return the Marvi-owned bin staging dir for LSP servers."""
    from marvi_constants import get_marvi_home

    p = get_marvi_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(marvi_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $MARVI_MESSAGING_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Marvi installed and
    ...
    if npm is None:
        ...
        return None
    staging = marvi_lsp_bin_dir().parent  # <MARVI_MESSAGING_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a marvi-owned target dir.
    ...
    console script.
    """
    pip_target = marvi_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = marvi_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``marvi lsp status`` CLI to give users a quick
