...
Storage
-------
Bundles live in ``~/.marvi/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``MARVI_MESSAGING_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``marvi bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under MARVI_MESSAGING_HOME.

    Honors ``MARVI_MESSAGING_BUNDLES_DIR`` for tests; falls back to
    ``<MARVI_MESSAGING_HOME>/skill-bundles``.
    """
    override = os.environ.get("MARVI_MESSAGING_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_marvi_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `marvi bundles` CLI subcommand.
# ---------------------------------------------------------------------------
