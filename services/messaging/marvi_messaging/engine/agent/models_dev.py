...
Data resolution order:
  ...
  2. Disk cache (~/.marvi/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Marvi ↔ models.dev
# ---------------------------------------------------------------------------

# Marvi provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Marvi' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Marvi ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_marvi_ids(mdev_id: str) -> List[str]:
    """Return the Marvi provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for marvi_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(marvi_id)
...
def _get_cache_path() -> Path:
    ...
    return get_marvi_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from marvi_constants import get_marvi_home
    return get_marvi_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``marvi config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Marvi provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Marvi id.
    for marvi_id in _models_dev_to_marvi_ids(provider_key):
        if marvi_id != provider_key:
            candidates.append(marvi_id)
...
def _get_provider_models(
    ...
    """Resolve a Marvi provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``marvi model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from runtime_support.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Marvi Messaging traffic.
...
def get_provider_info(
    ...
    Accepts either a Marvi provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Marvi ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Marvi or models.dev provider ID.  Tries exact match then
