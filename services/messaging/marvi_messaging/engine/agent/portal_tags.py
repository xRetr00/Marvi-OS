...
Every Marvi request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Marvi Messaging and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=marvi-messaging",
        "client=marvi-client-v<__version__>",
    ]

The version is sourced live from ``runtime_support.__version__`` so it auto-aligns
...
``runtime_support.__version__`` is the canonical source of truth.
"""
...
def _marvi_version() -> str:
    """Return the current Marvi release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``runtime_support`` cannot be imported (should
...
def marvi_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=marvi-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=marvi-client-v{_marvi_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Marvi session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Marvi conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.marvi/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Marvi
    ...
    """
    tags = ["product=marvi-messaging", marvi_client_tag()]
