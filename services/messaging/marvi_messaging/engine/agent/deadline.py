...
(https://github.com/xRetr00/Marvi-OS/issues/85125).
...
* :func:`resolve_timeout` — one config-first resolution path for timeout
  ...
  so new surfaces stop inventing ``MARVI_MESSAGING_*_TIMEOUT`` env vars (".env is for
...
* ``None`` timeout means unbounded, and non-positive resolved values are
  ...
  ``MARVI_MESSAGING_CONCURRENT_TOOL_TIMEOUT_S`` convention).
"""
...
class DeadlineExpired(TimeoutError):
    ...
    was Marvi's own bound — error classification must not attribute it to
...
def clamp_timeout(timeout: Optional[float]) -> Optional[float]:
    ...
    * Non-positive values become ``None`` (unbounded) — matching the existing
      ``MARVI_MESSAGING_CONCURRENT_TOOL_TIMEOUT_S`` "0 disables the bound" convention.
...
def resolve_timeout(
    ...
    *,
    ...
    2. ``env_var`` when set and non-empty (legacy bridge — internal mechanism
       ...
       ``MARVI_MESSAGING_*`` timeout env vars)
    3. ``default``
