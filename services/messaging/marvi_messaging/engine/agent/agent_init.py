...
from runtime_support.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.marvi/.env``. See xRetr00/Marvi-OS#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'marvi memory status'. Note: "
        "systemd/gateway services do not inherit ~/.marvi/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from runtime_support.providers import MARVI_MESSAGING_OVERLAYS, get_provider

        overlay = MARVI_MESSAGING_OVERLAYS.get(provider)
    ...
    try:
        ...
        from runtime_support.models import normalize_provider as normalize_model_provider
        from runtime_support.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: marvi config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$MARVI_MESSAGING_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_marvi_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$MARVI_MESSAGING_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .marvi.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / MARVI_MESSAGING_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.marvi/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from runtime_support.config import load_config_readonly as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind MARVI_MESSAGING_DEV_CREDITS.
    ...
    # both live under ~/.marvi/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from marvi_logging import setup_logging, setup_verbose_logging
    setup_logging(marvi_home=_ra()._marvi_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that marvi_logging.setup_logging() does not
        ...
        # handler level inside marvi_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from runtime_support.auth import build_minimax_oauth_token_provider
        ...
        try:
            from runtime_support.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = marvi_xai_default_headers()
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `marvi model` to "
                        "select a provider, or run `marvi setup` for first-time "
                        "configuration."
                    )
    ...
    # A multiplexed gateway may enter a different MARVI_MESSAGING_HOME after
    ...
    # worker (kanban_show tool is present iff MARVI_MESSAGING_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["MARVI_MESSAGING_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.marvi/sessions/ alongside gateway sessions
    marvi_home = get_marvi_home()
    agent.logs_dir = marvi_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.marvi/sessions/session_{sid}.json)
    ...
    try:
        from runtime_support.config import load_config_readonly as _load_sess_cfg
    ...
    # `marvi --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from runtime_support.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Marvi behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "marvi"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Marvi' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "marvi", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, marvi, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from runtime_support.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from runtime_support.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from runtime_support.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Marvi Messaging.  Choose a model with at least "
    ...
    # Nous Marvi 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from runtime_support.model_switch import _check_marvi_model_warning

            _marvi_warn = _check_marvi_model_warning(agent.model or "")
            if _marvi_warn:
                _user_msg = (
                    "⚠ Nous Research Marvi 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_marvi_warn)
        except Exception:
            pass
