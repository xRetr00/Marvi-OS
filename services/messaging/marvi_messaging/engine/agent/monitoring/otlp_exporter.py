...
configured under ``monitoring.export.otlp``. Lets an operator stream Marvi
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install marvi-messaging[otlp]``), imported lazily so the
...
def _require_sdk(*, auto_install: bool = True, prompt: bool = True):
    ...
    try:
        ...
        raise OTLPUnavailable(
            ...
            "    pip install 'marvi-messaging[otlp]'\n"
            f"(import error: {e})"
        )
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"marvi.event": kind or "unknown"}
    keep_by_kind = {
...
def export_batch(provider, batch: List[Dict[str, Any]]) -> int:
    ...
    tracer = provider.get_tracer("marvi.monitoring")
    n = 0
    for ev in batch:
        try:
            name = f"marvi.{ev.get('event', 'event')}"
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'marvi-messaging[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
