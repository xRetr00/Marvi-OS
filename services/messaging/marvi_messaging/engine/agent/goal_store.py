...
per-session ``/goal`` Ralph loop in ``runtime_support/goals.py`` (that's a
...
Storage: ``~/.marvi/goals.json``. Mirrors ``cron/jobs.py``'s storage style —
...
GOALS_FILE = get_marvi_home().resolve() / "goals.json"
...
def goals_file() -> Path:
    ...
    if GOALS_FILE != _INITIAL_GOALS_FILE:
        return GOALS_FILE
    return get_marvi_home().resolve() / "goals.json"
...
def _save_raw(goals: List[Dict[str, Any]]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"goals": goals, "updated_at": _marvi_now().isoformat()},
                f,
                indent=2,
            )
            f.flush()
    ...
    with _goals_lock:
        ...
        for field in ("title", "detail", "status", "horizon", "origin"):
            if field in updates and updates[field] is not None:
                ...
                target[field] = value
        target["updated"] = _marvi_now().isoformat()
        _save_raw(goals)
        return target
