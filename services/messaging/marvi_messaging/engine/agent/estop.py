...
``marvi pause`` writes a sentinel file at ``$MARVI_MESSAGING_HOME/ESTOP``;
``marvi resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Marvi is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.marvi/ESTOP``.
...
def _marvi_home() -> Path:
    """Resolve the active MARVI_MESSAGING_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active MARVI_MESSAGING_HOME."""
    return _marvi_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on MARVI_MESSAGING_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Marvi is paused ({reason}). New work is on hold; "
            "run `marvi resume` to pick things back up."
        )
    return (
        "⏸️ Marvi is paused. New work is on hold; "
        "run `marvi resume` to pick things back up."
    )
