...
Plugins built on Marvi Messaging often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Marvi
...
:class:`~runtime_support.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Marvi' main config
...
handles every provider, fallback chain, and per-task override Marvi
supports.
"""
...
def _resolve_task_ownership(plugin_id: str) -> tuple[frozenset, frozenset]:
    ...
    try:
        ...
        pass
    try:
        from runtime_support.main import _AUX_TASKS
...
class PluginLlm:
    ...
    Instances are constructed by :class:`runtime_support.plugins.PluginContext`
