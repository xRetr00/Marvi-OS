...
def list_providers(*, scope: Optional[str] = None) -> List[WebSearchProvider]:
    ...
    with _lock:
        ...
        merged.update(_scoped_providers.get(scope or marvi_home_key(), {}))
...
def get_provider(name: str, *, scope: Optional[str] = None) -> Optional[WebSearchProvider]:
    ...
    with _lock:
        key = name.strip()
        return _scoped_providers.get(scope or marvi_home_key(), {}).get(key) or _providers.get(key)
...
def _resolve(configured: Optional[str], *, capability: str) -> Optional[WebSearchProvider]:
    ...
    with _lock:
        ...
        snapshot.update(_scoped_providers.get(marvi_home_key(), {}))
