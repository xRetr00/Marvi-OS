"""Upload a Marvi session transcript to Hugging Face as an agent trace.

Marvi stores sessions in its own SQLite store (``marvi_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``marvi trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Marvi' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "marvi-traces"
_MARVI_MESSAGING_VERSION = "marvi-messaging"
...
# ---------------------------------------------------------------------------
# Conversion: Marvi OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Marvi' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Marvi conversation messages as Claude Code JSONL text.
    ...
    * ``assistant``       -> ``{"type": "assistant", "message": {...}}``
    ...
    def _common(turn_uuid: str) -> Dict[str, Any]:
        return {
            ...
            "version": _MARVI_MESSAGING_VERSION,
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.marvi/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `marvi trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/marvi-traces`` dataset. Returns a
