...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Marvi with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.marvi/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Marvi
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def name(self) -> str:
        ...
        ``mistral``, ``xai``, ``piper``, ``pockettts``) are
        rejected at registration time.
        """

    @property
    def display_name(self) -> str:
        """Human-readable label shown in ``marvi tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``marvi setup`` for
