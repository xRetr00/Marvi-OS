"""Anthropic Messages API adapter for Marvi Messaging.

Translates between Marvi's internal OpenAI-style message format and
...
try:
    ...
    _MARVI_MESSAGING_VERSION = str(_runtime_support.__version__)
except Exception:
    _MARVI_MESSAGING_VERSION = "0.0.0"
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in runtime_support/runtime_provider.py.
    """
...
# Marvi effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See marvi-messaging#13848, #17057.
    """
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to marvi's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to marvi's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + MarviAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://github.com/xRetr00/Marvi-OS")
        headers.setdefault("X-Title", "Marvi Messaging")
        headers.setdefault("User-Agent", f"MarviAgent/{_MARVI_MESSAGING_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Marvi loads that into the process env from
    # ``~/.marvi/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Marvi notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Marvi historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``marvi models``) never mutates ``~/.marvi/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Marvi)
      ...
      5. Anthropic credential_pool OAuth entry (~/.marvi/auth.json)
    ...
    # 1. Marvi-managed OAuth/setup token env var
    ...
    # 5. Marvi credential_pool OAuth entry.
...
# ── Marvi-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.marvi/.anthropic_oauth.json (our own file).
...
def _get_marvi_oauth_file() -> Path:
    return get_marvi_home() / ".anthropic_oauth.json"
...
def run_marvi_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Marvi-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Marvi with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from runtime_support.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_marvi_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Marvi-managed OAuth credentials from ~/.marvi/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Marvi OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See marvi-messaging#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See marvi-messaging: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See marvi-messaging#13848 (Kimi) and
    marvi-messaging#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Marvi Messaging", "Claude Code")
                text = text.replace("Marvi Messaging", "Claude Code")
                text = text.replace("marvi-messaging", "claude-code")
        ...
        #      a) bare Marvi-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Marvi surfaces in its CLI. We
