...
row taxonomy is inferred from the atlas shape so Marvi can render both legacy
...
# rows (1536x1872), while older Marvi/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Marvi' activity state names. They are not always identical to the
...
# Legacy Marvi/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Marvi activity names -> accepted row-name aliases in descending
