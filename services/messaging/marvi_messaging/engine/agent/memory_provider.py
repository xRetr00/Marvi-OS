...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `marvi backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - marvi_home (str): The active MARVI_MESSAGING_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.marvi``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "marvi").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'marvi memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], marvi_home: str) -> None:
        ...
        Called by 'marvi memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``marvi_home`` is the active MARVI_MESSAGING_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE MARVI_MESSAGING_HOME.

        ``marvi backup`` only walks MARVI_MESSAGING_HOME, so any provider state kept
        ...
        of the archive, and ``marvi import`` restores them to their original
