...
_MARVI_MESSAGING_MD_NAMES = (".marvi.md", "MARVI.md")


def _find_marvi_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.marvi.md`` or ``MARVI.md``.
    ...
    # could pick up a .marvi.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _MARVI_MESSAGING_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Marvi Messaging, an intelligent AI assistant created by NeuRetro Labs Research, "
    "built on the Marvi engine. "
...
MARVI_MESSAGING_HELP_GUIDANCE = (
    "You run as Marvi Messaging, the current NeuRetro Labs Research product and persona. "
    "When asked who or what you are, answer directly that you are Marvi Messaging; "
    ...
    "Marvi is the legacy/internal runtime name kept for compatibility: `marvi` "
    "CLI commands, `MARVI_MESSAGING_*` environment variables, `~/.marvi` profile paths, "
    "and the legacy `marvi-messaging` skill all belong to Marvi. Use Marvi in "
    "user-facing explanations unless the user is explicitly asking about legacy "
    "compatibility or an internal path/command that still contains Marvi. When "
    ...
    "`marvi-messaging` skill with skill_view(name='marvi-messaging') for additional "
    "guidance and proven workflows, but treat the docs as the source of truth "
    "when the two differ."
)

MARVI_MESSAGING_AGENT_HELP_GUIDANCE = MARVI_MESSAGING_HELP_GUIDANCE
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.marvi/kanban.db`. Your task id is in "
    "`$MARVI_MESSAGING_KANBAN_TASK`; your workspace is `$MARVI_MESSAGING_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $MARVI_MESSAGING_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $MARVI_MESSAGING_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${MARVI_MESSAGING_KANBAN_BRANCH:-wt/$MARVI_MESSAGING_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$MARVI_MESSAGING_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`marvi profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `marvi kanban <verb>` for board operations. Use "
...
#
# The marvi-messaging runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to marvi-messaging's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when Marvi has none; explicit Marvi YOLO uses a private unrestricted "
        ...
        "run `marvi computer-use doctor` and share the output. That command "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Marvi floating over another application, so the
    user is rarely asking about Marvi — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Marvi "
        ...
        "rather than anything inside Marvi. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Marvi terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
...
# container / remote host rather than on the machine where Marvi itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Marvi runs on.
    """
...
def build_environment_hints() -> str:
    ...
    # Embedder-supplied environment description. Lets a host that wraps Marvi
    ...
    extra = (os.getenv("MARVI_MESSAGING_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_marvi_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("MARVI_MESSAGING_PLATFORM") or os.environ.get("MARVI_MESSAGING_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.marvi/skills/`` directory.  External dirs
    ...
    # MARVI_MESSAGING_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_marvi_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_marvi_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "skills, voice, gateway, plugins, or any feature — load the legacy `marvi-messaging` skill "
            "first. It has the actual commands (e.g. `marvi config set ...`, `marvi tools`, "
            "`marvi setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: marvi setup, marvi setup tools, marvi setup terminal, marvi status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from MARVI_MESSAGING_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the MARVI_MESSAGING_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from runtime_support.config import ensure_marvi_home
        ensure_marvi_home()
    except Exception as e:
        logger.debug("Could not ensure MARVI_MESSAGING_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_marvi_home()
...
def _load_marvi_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.marvi.md / MARVI.md — walk to git root."""
    marvi_md_path = _find_marvi_md(cwd_path)
    if not marvi_md_path:
        return ""
    try:
        content = marvi_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = marvi_md_path.name
        try:
            rel = str(marvi_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".marvi.md", context_length=context_length,
            read_path=str(marvi_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", marvi_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .marvi.md / MARVI.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from MARVI_MESSAGING_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Marvi install/source
    ...
    # explicitly configured cwd is honored verbatim — the Marvi tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Marvi in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Marvi install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_marvi_md(cwd_path, context_length)
    ...
    # SOUL.md from MARVI_MESSAGING_HOME only — skip when already loaded as identity
    if not skip_soul:
