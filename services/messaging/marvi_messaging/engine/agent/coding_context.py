"""Coding-context awareness — base Marvi, every interactive surface.

When the user runs Marvi inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Marvi shifts into a **coding posture**. This module is the
...
from runtime_support._subprocess_compat import bounded_git_probe

logger = logging.getLogger("marvi.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "marvi", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _MARVI_MESSAGING_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``MARVI_MESSAGING_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Marvi should operate in its coding posture right now."""
