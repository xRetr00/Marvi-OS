...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``marvi logs``.
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # Marvi only initiates compaction in "marvi" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Marvi preflight compression for codex app-server "
                "(mode=%s); Marvi will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Marvi (via _emit_status), so it always
