...
(``gateway/platforms/webhook.py``): inbound wakes Marvi when the world
changes; outbound tells the world when Marvi does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Marvi-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``MARVI_MESSAGING_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.marvi/config.yaml``)::

    hooks:
      outbound:
        - url: https://ci.example.com/marvi-events
          events: [on_session_end, subagent_stop]
          # secret literal (discouraged) or env var name (preferred):
          secret_env: MARVI_MESSAGING_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `marvi hooks list`
...
Headers::
    ...
    User-Agent:              Marvi Messaging-Outbound-Webhook
    X-Marvi-Event:          <hook event name>
    X-Marvi-Delivery:       <delivery_id>
    X-Marvi-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("MARVI_MESSAGING_SAFE_MODE"):
        logger.info("MARVI_MESSAGING_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Marvi-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Marvi-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
