...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Marvi starts and finds ``~/.openclaw/``.

    Points users at ``marvi claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``marvi claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Marvi, run "
        "`marvi claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `marvi claw cleanup` (renames it to ~/.openclaw.pre-migration — "
