...
The parent Marvi process may itself be a Kanban dispatcher worker with
MARVI_MESSAGING_KANBAN_* variables in process env. delegate_task children run inside the
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar(
    "marvi_delegated_child_context",
    default=False,
)

# Set for any in-process execution that is NOT the dispatcher-owned worker even
# though the worker's MARVI_MESSAGING_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "marvi_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "MARVI_MESSAGING_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "MARVI_MESSAGING_KANBAN_TASK",
    "MARVI_MESSAGING_KANBAN_RUN_ID",
    "MARVI_MESSAGING_KANBAN_WORKSPACE",
    "MARVI_MESSAGING_KANBAN_WORKSPACES_ROOT",
    "MARVI_MESSAGING_KANBAN_CLAIM_LOCK",
    "MARVI_MESSAGING_KANBAN_BOARD",
    "MARVI_MESSAGING_KANBAN_DB",
)
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``MARVI_MESSAGING_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$MARVI_MESSAGING_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``MARVI_MESSAGING_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``MARVI_MESSAGING_KANBAN_*`` vars while losing the ContextVar in
