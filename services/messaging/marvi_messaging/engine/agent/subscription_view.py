...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    Dev override: when ``MARVI_MESSAGING_DEV_SUBSCRIPTION_FIXTURE`` names a fixture state,
...
def dev_fixture_subscription_state() -> Optional[SubscriptionState]:
    """Return a fixture :class:`SubscriptionState` for ``MARVI_MESSAGING_DEV_SUBSCRIPTION_FIXTURE``.
    ...
    Throwaway scaffolding — mirrors ``MARVI_MESSAGING_DEV_CREDITS_FIXTURE``.
    """
    name = (os.getenv("MARVI_MESSAGING_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return SubscriptionState(logged_in=False, error=f"unknown MARVI_MESSAGING_DEV_SUBSCRIPTION_FIXTURE: {name}")
