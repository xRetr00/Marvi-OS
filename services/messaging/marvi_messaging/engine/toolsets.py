...
_MARVI_MESSAGING_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (MARVI_MESSAGING_KANBAN_TASK env set) or the current
...
_MARVI_MESSAGING_WEBHOOK_SAFE_TOOLS = [
    "web_search",
    "web_extract",
    "vision_analyze",
    "clarify",
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `marvi tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (MARVI_MESSAGING_KANBAN_TASK env "
    ...
    # Coding posture (base Marvi — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in runtime_support/tools_config.py).
        "posture": True,
    },

    # ==========================================================================
    # Full Marvi toolsets (CLI + messaging platforms)
    #
    ...
    # the `marvi send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "marvi-cli": {
        ...
        "tools": _MARVI_MESSAGING_CORE_TOOLS,
        "includes": []
    },

    "marvi-cron": {
        # Mirrors marvi-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `marvi tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as marvi-cli; gated by `marvi tools`",
        "tools": _MARVI_MESSAGING_CORE_TOOLS,
        "includes": []
    },
    ...
    "marvi-email": {
        "description": "Email bot toolset - interact with Marvi via email (IMAP/SMTP)",
        "tools": _MARVI_MESSAGING_CORE_TOOLS,
        "includes": []
    },
    ...
    "marvi-sms": {
        "description": "SMS bot toolset - interact with Marvi via SMS (Twilio)",
        "tools": _MARVI_MESSAGING_CORE_TOOLS,
        "includes": []
    },
    ...
    "marvi-webhook": {
        ...
        "tools": _MARVI_MESSAGING_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "marvi-gateway": {
        ...
        "tools": [],
        "includes": ["marvi-telegram", "marvi-discord", "marvi-whatsapp", "marvi-slack", "marvi-signal", "marvi-bluebubbles", "marvi-homeassistant", "marvi-email", "marvi-sms", "marvi-mattermost", "marvi-matrix", "marvi-dingtalk", "marvi-feishu", "marvi-wecom", "marvi-wecom-callback", "marvi-weixin", "marvi-qqbot", "marvi-webhook", "marvi-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``marvi-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_MARVI_MESSAGING_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``marvi-gateway``
    ...
    """
    core = set(_MARVI_MESSAGING_CORE_TOOLS)
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (marvi-<name>).
        # Gives them _MARVI_MESSAGING_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("marvi-"):
            platform_name = name[len("marvi-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_MARVI_MESSAGING_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
