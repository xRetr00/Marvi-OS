...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with marvi-messaging)
2. User plugins: ``$MARVI_MESSAGING_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``marvi_messaging.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$MARVI_MESSAGING_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$MARVI_MESSAGING_HOME/plugins/model-providers/`` if it exists."""
    try:
        from marvi_constants import get_marvi_home

        d = get_marvi_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple MARVI_MESSAGING_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_marvi_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``marvi_messaging.plugins``
    entry-point group so they self-register.

    A distribution ships::

        [project.entry-points."marvi_messaging.plugins"]
        acme-inference = "acme_marvi_plugin:register"
    ...
    * **Provider targets only.** The ``marvi_messaging.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$MARVI_MESSAGING_HOME``) keep their
    ...
    try:
        from runtime_support.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$MARVI_MESSAGING_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``marvi_messaging.plugins``
    ...
    #    filesystem steps means a bundled or ``$MARVI_MESSAGING_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with marvi-messaging.
    ...
    # 2. User plugins — under $MARVI_MESSAGING_HOME/plugins/model-providers/<name>/.
