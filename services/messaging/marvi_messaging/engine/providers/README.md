...
Registry and ABC for every inference provider Marvi knows about.
...
`$MARVI_MESSAGING_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `runtime_support/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `runtime_support/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `runtime_support/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `runtime_support/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `runtime_support/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$MARVI_MESSAGING_HOME/plugins/model-providers/` for a private plugin).
